(function ($) {
    var frmPersonaValidador;
    var frmPracticasValidador;
    var strMensajes = '';
    var PERFIL_NOMINA_ABASTECIMIENTO = '45';
    var PERFIL_NOMINA_CONTACTO = '46';
    var PERFIL_NOMINA_CONTABILIDAD = '47';
    var data = [
                    { Nombre: "PERIODO LIMITADO", Codigo: "1" },
                    { Nombre: "AL TERMINO DE LA DESIGNACIÓN", Codigo: "2" }
               ];
    var dataTipoContrato = [
                    { Nombre: "FORMATO ESTANDAR", Codigo: "1" }
    ];


    this.ConvocatoriaJS = function () { };
    this.ConvocatoriaJS.prototype.inicializar = function () {
        /* BUSQUEDA */
        $("#ddlEstado_busqueda").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Contrato/ListarEstados",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlTipoFormato").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: dataTipoContrato
        });
        $("#ddlCondicion_busqueda").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdCondicion",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Condicion/ListarCondicion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        data_param.perfil = $("#hdPerfil").val();

                        return $.toDictionary(data_param);
                    }
                }
            }
        });

        $("#ddlSede_busqueda").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlDependencia_busqueda").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlDependencia").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlBases").kendoDropDownList({
            autoBind: true,
            cascadeFrom: "ddlDependencia",
            optionLabel: "--Seleccione--",
            dataTextField: "strNombrePuesto",
            dataValueField: "iCodBasePerfil",
            //filter: "contains",
            //minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Bases/ObtenerBasesPerfilesPuestoConvocatoria",
                        type: "POST",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($operation === "read") {
                            data_param.idDependencia = $('#ddlDependencia').data("kendoDropDownList").value();
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                        }

                        return $.toDictionary(data_param);
                    }
                }
            },
            change: function (e) {
                var estado = this.value();
                
                debugger;
                if (estado != '') {
                    LimpiarModalRegistroConvocatoria();
                    CargarFormularioConvocatoria(estado);
                } else {
                    LimpiarModalRegistroConvocatoria();
                }
            }
        });

        /* CONSULTA */
        $('#divModalEliminacion').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '30%',
            height: 'auto',
            title: 'Confirmar eliminación',
            visible: false,
            position: { top: '5%', left: "35%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");

        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSede").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        //$("#ddlPersonaEstado").kendoDropDownList({
        //    autoBind: false,
        //    //optionLabel: "--Seleccione--",
        //    dataTextField: "Nombre",
        //    dataValueField: "Codigo",
        //    dataSource: {
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Persona/ListarEstado",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            }
        //        }
        //    },
        //    change: function (e) {
        //        debugger;
        //        var estado = this.value();
                
        //        if (estado == '1') {
        //            //$("#divCese").hide();
        //            $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
        //            $("#txtPersonaFechaFinLabores").removeAttr("required");
        //            $("#txtPersonaFechaFinLabores").val('');
        //        }
        //        if (estado == '0') {
        //            //$("#divCese").show();
        //            $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
        //            $("#txtPersonaFechaFinLabores").attr("required", true);
        //            $("#txtPersonaFechaFinLabores").attr("validationmessage", "requerido");
        //        }
        //    }
        //});
        $('#divModalRegistroConvocatoria').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '98%',
            height: 'auto',
            title: 'Agregar Convocatoria',
            visible: false,
            position: { top: '3%', left: "1%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"],
            close: function () {
                frmPersonaValidador.hideMessages();

                $("#hdIdConvocatoria").val('0');
                $("#hdIdPerfil").val('0');
                $("#ddlBases").data("kendoDropDownList").value('');
                $("#ddlDependencia").data("kendoDropDownList").value('');
                $("#txtAIRHSPConvocatoria").val('');
                $("#txtMetaConvocatoria").val('');
                $("#ddlResponsableCurricular").data("kendoDropDownList").value('');
            }
        }).data("kendoWindow");
        frmPersonaValidador = $("#frmRegistroConvocatoria").kendoValidator().data("kendoValidator");

        $('#divModalPostulacion').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Importante',
            visible: false,
            position: { top: '20%', left: "10%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"]
        }).data("kendoWindow");
        $('#divModalPostulacionServir').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Importante',
            visible: false,
            position: { top: '20%', left: "10%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"]
        }).data("kendoWindow");

        //$("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        //$("#txtPersonaFechaInicioLabores").kendoDatePicker({ format: "dd/MM/yyyy", min: new Date() });
        //$("#txtPersonaFechaFinLabores").kendoDatePicker({ format: "dd/MM/yyyy" }); // max: new Date()

        //$("#ddlEstado_busqueda").data("kendoDropDownList").value("-1");
        
        this.inicializarGrid();
        this.inicializarGridServir();
    };
    this.ConvocatoriaJS.prototype.inicializarPostulacion = function () {
        ///* REGISTRO */
        $("#fileAnexo").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            },
            select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        $("#fileHojaVida").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            },
            select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelEstudio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "strDescripcion",
            dataValueField: "iCodGrado",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Perfiles/ListarGradosTodos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelLogradoEstudio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "strDescripcion",
            dataValueField: "iCodNivel",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Perfiles/ListarNivelEducativo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlAnioEstudio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarAnioPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMesEstudio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarMesPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlTipoDocumento").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        $('#divModalVisor').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '98%',
            height: 'auto',
            title: 'Ver documento',
            visible: false,
            position: { top: '1%', left: "1%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");

        //frmPersonaValidador = $("#frmRegistroPostulante").kendoValidator().data("kendoValidator");
        //frmEstudioValidador = $("#frmNuevoEstudio").kendoValidator().data("kendoValidator");
        //frmDocumentoValidador = $("#frmNuevoDocumento").kendoValidator().data("kendoValidator");
        //frmCapacitacionValidador = $("#frmNuevoCapacitacion").kendoValidator().data("kendoValidator");
        //frmExperienciaValidador = $("#frmNuevoExperiencia").kendoValidator().data("kendoValidator");
        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });

        //$("#txtExperienciaInicio").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        //$("#txtExperienciaFin").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        //$("#txtCapacitacionInicio").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        //$("#txtCapacitacionFin").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });

        $('#chkColegiatura').change(OnChangeColegiatura);
        $('#chkPersonaDiscapacidad').change(OnChangeDiscapacidad);
        $('#chkPersonaDeportista').change(OnChangeDeportistaAltoRendimiento);
        $('#chkTrabajarInterior').change(OnChangeTrabajarInterior);
        $('#chkPersonaFFAA').change(OnChangeFuerzasArmadas);

        this.CargarPostulacionPostulante();
        //this.CargarFormularioFamiliares(-1);
        //this.CargarGridExperiencia();
        //this.CargarGridCapacitacion(-1);
    };
    this.ConvocatoriaJS.prototype.inicializarPostulacionServir = function () {
        ///* REGISTRO */
        $("#fileAnexo").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            },
            select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        $("#fileHojaVida").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            },
            select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelEstudio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "strDescripcion",
            dataValueField: "iCodGrado",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Perfiles/ListarGradosTodos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelLogradoEstudio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "strDescripcion",
            dataValueField: "iCodNivel",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Perfiles/ListarNivelEducativo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlAnioEstudio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarAnioPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMesEstudio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarMesPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlTipoDocumento").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        $('#divModalVisor').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '98%',
            height: 'auto',
            title: 'Ver documento',
            visible: false,
            position: { top: '1%', left: "1%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");
        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });

        $('#chkColegiatura').change(OnChangeColegiatura);
        $('#chkPersonaDiscapacidad').change(OnChangeDiscapacidad);
        $('#chkPersonaDeportista').change(OnChangeDeportistaAltoRendimiento);
        $('#chkTrabajarInterior').change(OnChangeTrabajarInterior);
        $('#chkPersonaFFAA').change(OnChangeFuerzasArmadas);

        this.CargarPostulacionPostulanteServir();
        //this.CargarFormularioFamiliares(-1);
        //this.CargarGridExperiencia();
        //this.CargarGridCapacitacion(-1);
    };
    this.ConvocatoriaJS.prototype.inicializarAnonimo = function () {
        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeDocumentoDNI",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#fileDocAnexo").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 5120000
            }//,
            //select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        $("#fileDocRequerimiento").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 4096000
            }//,
            //select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        $("#fileDocCarta").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 4096000
            }//,
            //select: onSelect
            //success: onSuccess,
            //upload: onUpload
        });
        /* CONSULTA */
        $('#divModalPostulacion').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '50%',
            height: 'auto',
            title: 'Importante',
            visible: false,
            position: { top: '20%', left: "25%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"]
        }).data("kendoWindow");
        $('#divModalPostulacionServir').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '50%',
            height: 'auto',
            title: 'Importante',
            visible: false,
            position: { top: '20%', left: "25%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"]
        }).data("kendoWindow");
        $('#divModalRegistroPracticas').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Registro de Postulación',
            visible: false,
            position: { top: '10%', left: "10%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"]
        }).data("kendoWindow");
        frmPracticasValidador = $("#frmRegistroPracticas").kendoValidator().data("kendoValidator");

        var modal = $('#divModalPostulacion').data('kendoWindow');
        modal.open(); //.center();

        this.inicializarGridAnonimo();
        this.inicializarGridServirAnonimo();
        this.inicializarGridPracticasAnonimo();
    };

    function onSelect(e) {
        controlador.CargarRequisitosConvocatoria();
    }
    
    //function onError(e) {
    //    //kendoConsole.log("Error (" + e.operation + ") :: " + getFileInfo(e));
    //}
    //function onComplete(e) {
    //    //$("#divConsole").show();
    //}
    //function onCancel(e) {
    //    //kendoConsole.log("Cancel :: " + getFileInfo(e));
    //}
    //function onRemove(e) {
    //    //kendoConsole.log("Remove :: " + getFileInfo(e));
    //}
    //function onProgress(e) {
    //    //console.log("Upload progress :: " + e.percentComplete + "% :: " + getFileInfo(e));
    //}
    this.ConvocatoriaJS.prototype.CargarPostulacionPostulante = function () {
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());

        $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").readonly();
        $('#ddlPersonaSexo').data("kendoDropDownList").readonly();
        $('#ddlPersonaEstadoCivil').data("kendoDropDownList").readonly();
        $("#txtPersonaFechaNacimiento").data("kendoDatePicker").readonly();


        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulacionPostulante',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;

                $("#hdIdPostulante").val(res.IdPostulante);
                $("#txtPersonaNacionalidad").val(res.Nacionalidad);
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtPersonaNumeroDeDocumento").val(res.NroDocumento);
                $("#txtPersonaNombres").val(res.Nombre);
                $("#txtPersonaApellidoPaterno").val(res.Paterno);
                $("#txtPersonaApellidoMaterno").val(res.Materno);
                $("#ddlPersonaSexo").data("kendoDropDownList").value(res.Sexo);
                $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(res.IdEstadoCivil);
                $("#txtPersonaDireccionDomicilio").val(res.Domicilio);
                $("#txtPersonaUbigeo").val(res.DescripcionUbigeo);
                $("#txtPersonaFechaNacimiento").data("kendoDatePicker").value(res.FechaNacimiento);
                $("#txtTelefonoCelularP").val(res.Celular);
                $("#txtTelefonoFijoP").val(res.Telefono);
                $("#txtPersonaCorreoElectronicoP").val(res.CorreoElectronico);
                $("#txtPersonaRUC").val(res.RUC);
                $("#txtPersonaColegio").val(res.Colegio);
                $("#txtPersonaColegioNro").val(res.NroColegiatura);
                $("#hdIdUbigeoPostulante").val(res.Ubigeo);

                $('#chkColegiatura').prop("checked", res.bColigiatura);
                $('#chkColegiaturaHabilitada').prop("checked", res.bColegiatura_Habilitada);
                $('#txtCarnetConadis').val(res.CarnetConadis);
                $('#txtDocumentoPersonaDeportista').val(res.Acreditacion);
                $('#chkTrabajarInterior').prop("checked", res.bTrabajarProvincia);
                $('#txtTrabajarProvincia').val(res.Trabajar_Provincia);
                $('#txtDomicilioActual').val(res.DireccionActual);
                $('#txtReferencia').val(res.Referencia);
                $('#txtDocumentoFuerzasArmadas').val(res.CarnetFuerzasArmadas);

                if (res.FFAA == 1)
                    $("#chkPersonaFFAA").prop("checked", true);
                if (res.Discapacidad == 1)
                    $("#chkPersonaDiscapacidad").prop("checked", true);
                if (res.Deportista == 1)
                    $("#chkPersonaDeportista").prop("checked", true);

                if (res.Foto != '') {
                    document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,' + res.Foto);
                    $('#hdFoto').val(res.Foto);
                }

                OnChangeColegiatura();
                OnChangeDiscapacidad();
                OnChangeDeportistaAltoRendimiento();
                OnChangeTrabajarInterior();
                OnChangeFuerzasArmadas()

                controlador.CargarGridPostulacionDocumento();
                controlador.CargarGridPostulacionEstudio();
                controlador.CargarGridPostulacionCapacitacion();
                controlador.CargarGridPostulacionExperiencia();

                controlador.CargarGridRequisitosEstudio();
                controlador.CargarGridRequisitosCapacitacion();
                controlador.CargarGridRequisitosExperiencia();
                controlador.CargarRequisitosConvocatoria();
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res.responseText);
            }
        });
    }
    this.ConvocatoriaJS.prototype.CargarPostulacionPostulanteServir = function () {
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());

        $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").readonly();
        $('#ddlPersonaSexo').data("kendoDropDownList").readonly();
        $('#ddlPersonaEstadoCivil').data("kendoDropDownList").readonly();
        $("#txtPersonaFechaNacimiento").data("kendoDatePicker").readonly();


        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulacionPostulanteServir',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;

                $("#hdIdPostulante").val(res.IdPostulante);
                $("#txtPersonaNacionalidad").val(res.Nacionalidad);
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtPersonaNumeroDeDocumento").val(res.NroDocumento);
                $("#txtPersonaNombres").val(res.Nombre);
                $("#txtPersonaApellidoPaterno").val(res.Paterno);
                $("#txtPersonaApellidoMaterno").val(res.Materno);
                $("#ddlPersonaSexo").data("kendoDropDownList").value(res.Sexo);
                $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(res.IdEstadoCivil);
                $("#txtPersonaDireccionDomicilio").val(res.Domicilio);
                $("#txtPersonaUbigeo").val(res.DescripcionUbigeo);
                $("#txtPersonaFechaNacimiento").data("kendoDatePicker").value(res.FechaNacimiento);
                $("#txtTelefonoCelularP").val(res.Celular);
                $("#txtTelefonoFijoP").val(res.Telefono);
                $("#txtPersonaCorreoElectronicoP").val(res.CorreoElectronico);
                $("#txtPersonaRUC").val(res.RUC);
                $("#txtPersonaColegio").val(res.Colegio);
                $("#txtPersonaColegioNro").val(res.NroColegiatura);
                $("#hdIdUbigeoPostulante").val(res.Ubigeo);

                $('#chkColegiatura').prop("checked", res.bColigiatura);
                $('#chkColegiaturaHabilitada').prop("checked", res.bColegiatura_Habilitada);
                $('#txtCarnetConadis').val(res.CarnetConadis);
                $('#txtDocumentoPersonaDeportista').val(res.Acreditacion);
                $('#chkTrabajarInterior').prop("checked", res.bTrabajarProvincia);
                $('#txtTrabajarProvincia').val(res.Trabajar_Provincia);
                $('#txtDomicilioActual').val(res.DireccionActual);
                $('#txtReferencia').val(res.Referencia);
                $('#txtDocumentoFuerzasArmadas').val(res.CarnetFuerzasArmadas);

                if (res.FFAA == 1)
                    $("#chkPersonaFFAA").prop("checked", true);
                if (res.Discapacidad == 1)
                    $("#chkPersonaDiscapacidad").prop("checked", true);
                if (res.Deportista == 1)
                    $("#chkPersonaDeportista").prop("checked", true);

                if (res.Foto != '') {
                    document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,' + res.Foto);
                    $('#hdFoto').val(res.Foto);
                }

                OnChangeColegiatura();
                OnChangeDiscapacidad();
                OnChangeDeportistaAltoRendimiento();
                OnChangeTrabajarInterior();
                OnChangeFuerzasArmadas()

                controlador.CargarGridPostulacionDocumento();
                controlador.CargarGridPostulacionEstudio();
                controlador.CargarGridPostulacionCapacitacion();
                controlador.CargarGridPostulacionExperiencia();

                controlador.CargarGridRequisitosEstudio();
                controlador.CargarGridRequisitosCapacitacion();
                controlador.CargarGridRequisitosExperiencia();
                controlador.CargarRequisitosConvocatoria();
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res.responseText);
            }
        });
    }

    this.ConvocatoriaJS.prototype.CargarGridPostulacionDocumento = function () {
        this.$dataSourceDocumento = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulacionDocumento',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulacion = $("#hdIdPostulacion").val();
                        data_param.IdTipoConvocatoria = $("#hdIdTipoConvocatoria").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdDocumento"
                }
            },
        });

        this.divGridDocumento = $("#divGridDocumento").kendoGrid({
            //toolbar: ["excel"], //"excel", 
            dataSource: this.$dataSourceDocumento,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "NombreTipoDocumento",
                    title: "TIPO DE DOCUMENTO",
                    attributes: { style: "text-align:left;" },
                    width: "200px"
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdDocumento + '\',5)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    }
    this.ConvocatoriaJS.prototype.CargarGridPostulacionEstudio = function () {
        this.$dataSourceEstudio = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulacionEstudio',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulacion = $("#hdIdPostulacion").val();
                        data_param.IdTipoConvocatoria = $("#hdIdTipoConvocatoria").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdPostulacionEstudio",
                    fields: {
                        Requisito: {
                            validation: {
                                required: true,
                                requisitovalidation: function (input) {
                                    if (input.is("[name='Requisito']") && input.val() == "") {
                                        input.attr("data-requisitovalidation-msg", "El requisito es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 0, Nombre: "--" }
                        },
                    }
                }
            },
            change: function (e) {
                if (e.items.length == 0) {
                    controladorApp.notificarMensajeDeInformacion('Debe ingresar estudios de su formación académica en su perfil');
                }
                else {
                    //$("#btnReporte").prop('disabled', true);
                    if (e.action == "itemchange" && e.field == "IdEstudioPerfil") {
                        // ACTUALIZAMOS LOS VALORES ALCANZADOS

                        //alert(e.items[0].IdEstudioPerfil)
                        var data_param = new FormData();
                        data_param.append('IdPostulacionEstudio', e.items[0].IdPostulacionEstudio);
                        data_param.append('IdEstudioPerfil', e.items[0].IdEstudioPerfil);
                        data_param.append('IdTipoConvocatoria', $("#hdIdTipoConvocatoria").val());

                        $.ajax({
                            url: controladorApp.obtenerRutaBase() + 'Postulante/ActualizarPostulacionEstudio',
                            type: 'POST',
                            dataType: 'json',
                            contentType: false,
                            processData: false,
                            data: data_param,
                            success: function (res) {
                                //controladorApp.notificarMensajeSatisfactorio("Se actualizaron los montos correctamente");
                                controlador.CargarGridPostulacionEstudio();
                                controlador.CargarRequisitosConvocatoria();
                            },
                            error: function (res) {
                                debugger;
                            }
                        });
                    }
                }
            }
        });

        this.divGridEstudio = $("#divGridEstudio").kendoGrid({
            //toolbar: ["excel"], //"excel", 
            dataSource: this.$dataSourceEstudio,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "NombreGrado",
                    title: "NIVEL EDUCATIVO",
                    attributes: { style: "text-align:left;" },
                    width: "80px",
                    editable: true
                },
                {
                    field: "NombreNivel",
                    title: "NIVEL ALCANZADO",
                    attributes: { style: "text-align:left;" },
                    width: "100px",
                    editable: true
                },
                {
                    field: "Especialidad",
                    title: "ESPECIALIDAD",
                    attributes: { style: "text-align:left;" },
                    width: "150px",
                    editable: true
                },
                {
                    field: "Institucion",
                    title: "INSTITUCIÓN",
                    attributes: { style: "text-align:left;" },
                    width: "150px",
                    editable: true
                },
                {
                    field: "Ciudad",
                    title: "CIUDAD / PAÍS",
                    width: "100px",
                    attributes: { style: "text-align:left;" },
                    editable: true
                },
                {
                    field: "AnioMes",
                    title: "OBTENCIÓN DEL ESTUDIO",
                    attributes: { style: "text-align:left;" },
                    width: "80px",
                    editable: true
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdEstudio + '\',1)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }

                        return controles;
                    },
                    width: '30px',
                    editable: true
                },
                {
                    field: "IdEstudioPerfil",
                    title: "¿ES REQUISITO PARA<BR>CUMPLIR CON EL PERFIL?",
                    attributes: { style: "text-align:center; background-color: RGB(223, 240, 216); padding: 3px 5px 3px 5px" }, //RGB(252, 248, 227)
                    width: "100px",
                    //editable: true,
                    editor: controlador.RequisitoDropDownEditor,
                    template: "#=Requisito.Nombre#"
                }
            ],
            editable: true
        }).data();
    }
    this.ConvocatoriaJS.prototype.RequisitoDropDownEditor = function (container, options) {
        var data = [
                        { Nombre: "NO", Codigo: "0" },
                        { Nombre: "SI", Codigo: "1" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };
    this.ConvocatoriaJS.prototype.RequisitoExpDropDownEditor = function (container, options) {
        var data = [
                        { Nombre: "GENERAL", Codigo: "0" },
                        { Nombre: "GENERAL Y ESPECÍFICA", Codigo: "1" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };
    this.ConvocatoriaJS.prototype.CargarGridPostulacionExperiencia = function () {
        this.$dataSourceExperiencia = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulacionExperiencia',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulacion = $("#hdIdPostulacion").val();
                        data_param.IdTipoConvocatoria = $("#hdIdTipoConvocatoria").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdPostulacionExperiencia",
                    fields: {
                        Requisito: {
                            validation: {
                                required: true,
                                requisitovalidation: function (input) {
                                    if (input.is("[name='Requisito']") && input.val() == "") {
                                        input.attr("data-requisitovalidation-msg", "El requisito es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 0, Nombre: "GENERAL" }
                        },
                    }
                }
            },
            change: function (e) {
                if (e.items.length == 0) {
                    controladorApp.notificarMensajeDeInformacion('Debe ingresar estudios de capacitación en su perfil');
                }
                else {
                    //$("#btnReporte").prop('disabled', true);
                    if (e.action == "itemchange" && e.field == "IdExperienciaPerfil") {
                        // ACTUALIZAMOS LOS VALORES ALCANZADOS

                        //alert(e.items[0].IdEstudioPerfil)
                        var data_param = new FormData();
                        data_param.append('IdPostulacionExperiencia', e.items[0].IdPostulacionExperiencia);
                        data_param.append('IdExperienciaPerfil', e.items[0].IdExperienciaPerfil);
                        data_param.append('IdTipoConvocatoria', $("#hdIdTipoConvocatoria").val());

                        $.ajax({
                            url: controladorApp.obtenerRutaBase() + 'Postulante/ActualizarPostulacionExperiencia',
                            type: 'POST',
                            dataType: 'json',
                            contentType: false,
                            processData: false,
                            data: data_param,
                            success: function (res) {
                                //controladorApp.notificarMensajeSatisfactorio("Se actualizaron los montos correctamente");
                                controlador.CargarGridPostulacionExperiencia();
                                controlador.CargarRequisitosConvocatoria();
                            },
                            error: function (res) {
                                debugger;
                            }
                        });
                    }
                }
            },
            group: {
                field: "IdExperienciaPerfil", aggregates: [
                   { field: "ExperienciaPerfil", aggregate: "max" },
                   { field: "RangoFecha", aggregate: "sum" }
                ]
            },
            aggregate: [
                    { field: "ExperienciaPerfil", aggregate: "count" },
                    { field: "RangoFecha", aggregate: "sum" }
            ]
        });

        this.divGridExperiencia = $("#divGridExperiencia").kendoGrid({
            //toolbar: ["excel"], //"excel", 
            dataSource: this.$dataSourceExperiencia,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: true,
            dataType: 'json',
            columns: [
                {
                    field: "Empresa",
                    title: "NOMBRE DE LA ENTIDAD O EMPRESA",
                    attributes: { style: "text-align:left;" },
                    width: "300px",
                    editable: true,
                    aggregates: ["count"],
                    //footerTemplate: "<div style='float: left; font-size: 14px'>TOTAL: </div>",
                    footerTemplate: "EXPERIENCIA TOTAL: ",
                    //footerTemplate: "Total: ",
                    //groupFooterTemplate: "Sub Total: #=count#",
                    groupFooterTemplate: "Sub Total: "
                },
                {
                    field: "Cargo",
                    title: "CARGO DESEMPEÑADO",
                    attributes: { style: "text-align:left;" },
                    width: "200px",
                    editable: true
                },
                {
                    field: "FechaInicio",
                    title: "FECHA DE INICIO",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    editable: true
                },
                {
                    field: "FechaFin",
                    title: "FECHA DE TÉRMINO",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    editable: true
                },
                //{
                //    field: "RangoFechaCadena",
                //    title: "TIEMPO DE EXPERIENCIA",
                //    attributes: { style: "text-align:left;" },
                //    width: "100px",
                //    editable: true,
                //    aggregates: ["sum"],
                //    footerTemplate: "#= kendo.toString(sum, 'C') #",
                //    groupFooterTemplate: "#= kendo.toString(sum, 'C') #"
                //},
                {
                    field: "RangoFecha",
                    title: "TIEMPO DE EXPERIENCIA",
                    attributes: { style: "text-align:left;" },
                    width: "50px",
                    editable: true,
                    template: function (item) {
                        return item.RangoFechaCadena;
                    },
                    aggregates: ["sum"],
                    footerTemplate: "#= controlador.MostarRangoFechas(sum) #",
                    groupFooterTemplate: "#= controlador.MostarRangoFechas(sum) #"
                },
                //{
                //    field: "Descripcion",
                //    title: "DESCRIPCIÓN DEL TRABAJO REALIZADO",
                //    attributes: { style: "text-align:left;" },
                //    width: "450px",
                //    editable: true
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdLaboral + '\',3)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    field: "IdExperienciaPerfil",
                    title: "TIPO DE EXPERIENCIA",
                    attributes: { style: "text-align:center; background-color: RGB(223, 240, 216); padding: 3px 5px 3px 5px" }, //252, 248, 227
                    width: "100px",
                    //aggregates: ["count"],
                    //editable: true,
                    editor: controlador.RequisitoExpDropDownEditor,
                    template: "#=Requisito.Nombre#",
                    groupHeaderTemplate: "TIPO DE EXPERIENCIA: #= aggregates.ExperienciaPerfil.max # " //(Items: #= #)
                }
            ],
            editable: true
        }).data();
    }
    this.ConvocatoriaJS.prototype.MostarRangoFechas = function (suma) {
        var years = Math.trunc((suma / 365.25));
        var months = Math.trunc((((suma / 365.25) - years) * 12));
        var days = Math.trunc(((((suma / 365.25) - years) * 12) - months) * 365.25 / 12);

        return years + " años, " + " " + months + " meses" + " " + days + " días";
    }
    this.ConvocatoriaJS.prototype.CargarGridPostulacionCapacitacion = function () {
        this.$dataSourceCapacitacion = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulacionCapacitacion',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulacion = $("#hdIdPostulacion").val();
                        data_param.IdTipoConvocatoria = $("#hdIdTipoConvocatoria").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdPostulacionCapacitacion",
                    fields: {
                        Requisito: {
                            validation: {
                                required: true,
                                requisitovalidation: function (input) {
                                    if (input.is("[name='Requisito']") && input.val() == "") {
                                        input.attr("data-requisitovalidation-msg", "El requisito es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 0, Nombre: "--" }
                        },
                    }
                }
            },
            change: function (e) {
                if (e.items.length == 0) {
                    controladorApp.notificarMensajeDeInformacion('Debe ingresar estudios de capacitación en su perfil');
                }
                else {
                    //$("#btnReporte").prop('disabled', true);
                    if (e.action == "itemchange" && e.field == "IdCapacitacionPerfil") {
                        // ACTUALIZAMOS LOS VALORES ALCANZADOS

                        //alert(e.items[0].IdEstudioPerfil)
                        var data_param = new FormData();
                        data_param.append('IdPostulacionCapacitacion', e.items[0].IdPostulacionCapacitacion);
                        data_param.append('IdCapacitacionPerfil', e.items[0].IdCapacitacionPerfil);
                        data_param.append('IdTipoConvocatoria', $("#hdIdTipoConvocatoria").val());

                        $.ajax({
                            url: controladorApp.obtenerRutaBase() + 'Postulante/ActualizarPostulacionCapacitacion',
                            type: 'POST',
                            dataType: 'json',
                            contentType: false,
                            processData: false,
                            data: data_param,
                            success: function (res) {
                                //controladorApp.notificarMensajeSatisfactorio("Se actualizaron los montos correctamente");
                                controlador.CargarGridPostulacionCapacitacion();
                                controlador.CargarRequisitosConvocatoria();
                            },
                            error: function (res) {
                                debugger;
                            }
                        });
                    }
                }
            }
        });

        this.divGridCapacitacion = $("#divGridCapacitacion").kendoGrid({
            //toolbar: ["create"], //"excel", 
            dataSource: this.$dataSourceCapacitacion,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "Especialidad",
                    title: "NOMBRE DE LA CAPACITACIÓN",
                    attributes: { style: "text-align:left;" },
                    width: "200px",
                    editable: true
                },
                {
                    field: "Institucion",
                    title: "INSTITUCIÓN",
                    attributes: { style: "text-align:left;" },
                    width: "200px",
                    editable: true
                },
                {
                    field: "Ciudad",
                    title: "CIUDAD / PAIS",
                    attributes: { style: "text-align:left;" },
                    width: "100px",
                    editable: true
                },
                {
                    field: "FechaInicio",
                    title: "FECHA DE INICIO",
                    width: "50px",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaInicioEditor,
                    editable: true
                },
                {
                    field: "FechaFin",
                    title: "FECHA DE TÉRMINO",
                    width: "50px",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaFinEditor,
                    editable: true
                },
                {
                    field: "Horas",
                    title: "HORAS LECTIVAS",
                    attributes: { style: "text-align:right;" },
                    width: "50px",
                    editable: true
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdCapacitacion + '\',2)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    field: "IdCapacitacionPerfil",
                    title: "¿ES REQUISITO PARA<BR>CUMPLIR CON EL PERFIL?",
                    attributes: { style: "text-align:center; background-color: RGB(223, 240, 216); padding: 3px 5px 3px 5px" }, //252, 248, 227
                    width: "100px",
                    //editable: true,
                    editor: controlador.RequisitoDropDownEditor,
                    template: "#=Requisito.Nombre#"
                }
            ],
            editable: true
        }).data();
    }

    this.ConvocatoriaJS.prototype.CargarGridRequisitosEstudio = function () {
        var template = kendo.template($("#templateE").html());

        this.$dataSourceRequisitosEstudio = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulacionRequisitosEstudio',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.iCodPerfil = $("#hdIdPerfil").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function () {
                $("#estudios tbody").html(kendo.render(template, this.view())); // populate the table
            }//,
            //schema: {
            //    model: {
            //        id: "iSecuencia"
            //    }
            //},
        });

        this.$dataSourceRequisitosEstudio.read();
        //alert(JSON.stringify(this.$dataSourceRequisitosEstudio.data()));

    }
    this.ConvocatoriaJS.prototype.CargarGridRequisitosCapacitacion = function () {
        var template = kendo.template($("#templateC").html());

        this.$dataSourceRequisitosCapacitacion = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulacionRequisitosCapacitacion',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.iCodPerfil = $("#hdIdPerfil").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function () {
                $("#capacitacion tbody").html(kendo.render(template, this.view())); // populate the table
            }
        });

        this.$dataSourceRequisitosCapacitacion.read();
        //alert(JSON.stringify(this.$dataSourceRequisitosEstudio.data()));
    }
    this.ConvocatoriaJS.prototype.CargarGridRequisitosExperiencia = function () {
        var data_param = new FormData();
        data_param.append('id', $("#hdIdPerfil").val());

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Perfiles/ObtenerPerfilesPuestoPorID',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;
                if (res != null) {
                    $("#spExpGeneral").text(res[0].iAnioExpGeneral);
                    $("#spExpEspecifica").text(res[0].iAnioExpEspecifica);
                    $("#spExpPublico").text(res[0].iAnioExpSectorPublico);
                    if (res[0].iCodNivelMinimo == 1) $("#spExpPerfil").text('Practicante Profesional');
                    if (res[0].iCodNivelMinimo == 2) $("#spExpPerfil").text('Auxiliar o Asistente');
                    if (res[0].iCodNivelMinimo == 3) $("#spExpPerfil").text('Analista');
                    if (res[0].iCodNivelMinimo == 4) $("#spExpPerfil").text('Especialista');
                    if (res[0].iCodNivelMinimo == 5) $("#spExpPerfil").text('Supervisor / Coordinador');
                    if (res[0].iCodNivelMinimo == 6) $("#spExpPerfil").text('Jefe de Area o Departamento');
                    if (res[0].iCodNivelMinimo == 7) $("#spExpPerfil").text('Gerente o Director');
                }
            },
            error: function (res) {
                //alert(res);
            }
        });
    }
    this.ConvocatoriaJS.prototype.CargarRequisitosConvocatoria = function () {
        var data_param = new FormData();
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdTipoConvocatoria', $("#hdIdTipoConvocatoria").val());

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarPostulacionRequisitos',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;
                if (res != null) {
                    var total = 0;
                    if (res.ExisteEstudio == 0) {
                        $("#divCorrectoFO").hide();
                        $("#divPendienteFO").show();
                        $("#iFormacion").addClass("glyphicon-exclamation-sign").addClass("text-danger").removeClass("glyphicon-ok").removeClass("text-success");
                    }
                    else {
                        total = total + 1;
                        $("#divCorrectoFO").show();
                        $("#divPendienteFO").hide();
                        $("#iFormacion").removeClass("glyphicon-exclamation-sign").removeClass("text-danger").addClass("glyphicon-ok").addClass("text-success");
                    }
                    if (res.ExisteCapacitacion == 0) {
                        $("#divCorrectoCO").hide();
                        $("#divPendienteCO").show();
                        $("#iConocimiento").addClass("glyphicon-exclamation-sign").addClass("text-danger").removeClass("glyphicon-ok").removeClass("text-success");
                    }
                    else {
                        total = total + 1;
                        $("#divCorrectoCO").show();
                        $("#divPendienteCO").hide();
                        $("#iConocimiento").removeClass("glyphicon-exclamation-sign").removeClass("text-danger").addClass("glyphicon-ok").addClass("text-success");
                    }
                    if (res.ExisteExperiencia == 0) {
                        $("#divCorrectoEX").hide();
                        $("#divPendienteEX").show();
                        $("#iExperiencia").addClass("glyphicon-exclamation-sign").addClass("text-danger").removeClass("glyphicon-ok").removeClass("text-success");
                    }
                    else {
                        total = total + 1;
                        $("#divCorrectoEX").show();
                        $("#divPendienteEX").hide();
                        $("#iExperiencia").removeClass("glyphicon-exclamation-sign").removeClass("text-danger").addClass("glyphicon-ok").addClass("text-success");
                    }
                    if (res.ExisteAnexo == 1 && $("#fileAnexo").getKendoUpload().getFiles().length > 0) {
                        total = total + 1;
                        $("#divCorrectoAN").show();
                        $("#divPendienteAN").hide();
                        $("#iAnexo").removeClass("glyphicon-exclamation-sign").removeClass("text-danger").addClass("glyphicon-ok").addClass("text-success");
                    }
                    else {
                        $("#divCorrectoAN").hide();
                        $("#divPendienteAN").show();
                        $("#iAnexo").addClass("glyphicon-exclamation-sign").addClass("text-danger").removeClass("glyphicon-ok").removeClass("text-success");
                    }
                    if ($("#fileHojaVida").getKendoUpload().getFiles().length > 0) {
                        total = total + 1;
                        $("#divCorrectoHV").show();
                        $("#divPendienteHV").hide();
                    }
                    else {
                        $("#divCorrectoHV").hide();
                        $("#divPendienteHV").show();
                    }

                    if (total == 5) {
                        $('#btnPostular').prop('disabled', false);
                        $("#iEnviar").removeClass("glyphicon-exclamation-sign").removeClass("text-danger").addClass("glyphicon-ok").addClass("text-success");
                    }
                    else {
                        $('#btnPostular').prop('disabled', true);
                        $("#iEnviar").addClass("glyphicon-exclamation-sign").addClass("text-danger").removeClass("glyphicon-ok").removeClass("text-success");
                    }
                        
                }
            },
            error: function (res) {
                //alert(res);
            }
        });
    }

    this.ConvocatoriaJS.prototype.generarFormatoAnexo = function () {
        var IdPostulacion = $("#hdIdPostulacion").val();
        
        var data_param = new FormData();

        data_param.append('IdPostulacion', IdPostulacion);
        data_param.append('NepotismoRel1', $("#txtPostulanteRel1").val());
        data_param.append('NepotismoRel2', $("#txtPostulanteRel2").val());
        data_param.append('NepotismoRel3', $("#txtPostulanteRel3").val());
        data_param.append('NepotismoApe1', $("#txtPostulanteApe1").val());
        data_param.append('NepotismoApe2', $("#txtPostulanteApe2").val());
        data_param.append('NepotismoApe3', $("#txtPostulanteApe3").val());
        data_param.append('NepotismoNom1', $("#txtPostulanteNom1").val());
        data_param.append('NepotismoNom2', $("#txtPostulanteNom2").val());
        data_param.append('NepotismoNom3', $("#txtPostulanteNom3").val());
        data_param.append('NepotismoAre1', $("#txtPostulanteAre1").val());
        data_param.append('NepotismoAre2', $("#txtPostulanteAre2").val());
        data_param.append('NepotismoAre3', $("#txtPostulanteAre3").val());
        if ($("#chkAcepta").prop("checked")) data_param.append('IdAcepta', 1);
        else data_param.append('IdAcepta', 0);


        controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de generar las declaraciones juradas de postulación?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarPostulacionAnexo',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                //controladorApp.notificarMensajeSatisfactorio("Convocatoria registrado correctamente");

                                window.open(controladorApp.obtenerRutaBase() + "Convocatoria/DescargarAnexo?IdPostulacion=" + IdPostulacion, "_blank");
                                controlador.CargarRequisitosConvocatoria();
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
    }

    this.ConvocatoriaJS.prototype.generarFormatoAnexoServir = function () {
        var IdPostulacion = $("#hdIdPostulacion").val();

        var data_param = new FormData();

        data_param.append('IdPostulacion', IdPostulacion);
        data_param.append('NepotismoRel1', $("#txtPostulanteRel1").val());
        data_param.append('NepotismoRel2', $("#txtPostulanteRel2").val());
        data_param.append('NepotismoRel3', $("#txtPostulanteRel3").val());
        data_param.append('NepotismoApe1', $("#txtPostulanteApe1").val());
        data_param.append('NepotismoApe2', $("#txtPostulanteApe2").val());
        data_param.append('NepotismoApe3', $("#txtPostulanteApe3").val());
        data_param.append('NepotismoNom1', $("#txtPostulanteNom1").val());
        data_param.append('NepotismoNom2', $("#txtPostulanteNom2").val());
        data_param.append('NepotismoNom3', $("#txtPostulanteNom3").val());
        data_param.append('NepotismoAre1', $("#txtPostulanteAre1").val());
        data_param.append('NepotismoAre2', $("#txtPostulanteAre2").val());
        data_param.append('NepotismoAre3', $("#txtPostulanteAre3").val());
        data_param.append('IdTipoConvocatoria', $("#hdIdTipoConvocatoria").val());
        if ($("#chkAcepta").prop("checked")) data_param.append('IdAcepta', 1);
        else data_param.append('IdAcepta', 0);


        controladorApp.abrirMensajeDeConfirmacion(
            '¿Está seguro de generar las declaraciones juradas de postulación?', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarPostulacionAnexo',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            //controladorApp.notificarMensajeSatisfactorio("Convocatoria registrado correctamente");

                            window.open(controladorApp.obtenerRutaBase() + "Convocatoria/DescargarAnexo_Servir?IdPostulacion=" + IdPostulacion, "_blank");
                            controlador.CargarRequisitosConvocatoria();
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);
    }


    function LimpiarModalRegistroConvocatoria() {
        var today = new Date();
        var month = today.getMonth();
        if (today.getMonth() > 1) var month = month - 1;
        var year = today.getFullYear();
        
        $("#txtNroConvocatoria").val("");
        $("#txtCantidadConvocatoria").val("");
        $("#txtRemuneracionConvocatoria").val("");
        $("#txtCargoConvocatoria").val("");
        $("#txtAIRHSPConvocatoria").val("");
        $("#txtMetaConvocatoria").val("");

        $("#fileDocRequerimiento").data("kendoUpload").clearAllFiles();
        $("#fileDocCertificacion").data("kendoUpload").clearAllFiles();
        
        //if ($("#divGridEntrevista").data('kendoGrid') != null)
        //    $("#divGridEntrevista").data('kendoGrid').dataSource.data([]);
    }
    function LimpiarModalRegistroPractica() {
        $("#txtPersonaNumeroDeDocumento").val("");
        $("#txtPersonaNombres").val("");
        $("#txtPersonaApellidoPaterno").val("");
        $("#txtPersonaApellidoMaterno").val("");
        $("#txtPersonaFechaNacimiento").val("");
        $("#txtTelefonoCelularP").val("");
        $("#txtPersonaCorreoElectronicoP").val("");
        $("#txtPersonaUbigeo").val("");
        $("#txtPersonaDireccionDomicilio").val("");
        $('#ddlPersonaSexo').data("kendoDropDownList").value();
        $('#ddlPersonaEstadoCivil').data("kendoDropDownList").value();
        document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,');

        $("#fileDocRequerimiento").data("kendoUpload").clearAllFiles();
        $("#fileDocAnexo").data("kendoUpload").clearAllFiles();
        $("#fileDocCarta").data("kendoUpload").clearAllFiles();

        //if ($("#divGridEntrevista").data('kendoGrid') != null)
        //    $("#divGridEntrevista").data('kendoGrid').dataSource.data([]);
    }
    this.ConvocatoriaJS.prototype.inicializarGrid = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatorias',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 2; // SOLO LAS CONCONVOCATORIAS VIGENTES
                        data_param.IdTipo = 1;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            }
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$grid = $("#divGrid").kendoGrid({
            dataSource: this.$dataSource,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"                    
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }
                        
                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"                    
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }
                        
                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.abrirModalPostulacion(\'' + item.Contrasena + '\')">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }
                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }
                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    };
    this.ConvocatoriaJS.prototype.inicializarGridServir = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatorias',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 2; // SOLO LAS CONCONVOCATORIAS VIGENTES
                        data_param.IdTipo = 3;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            }
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$grid = $("#divGridServir").kendoGrid({
            dataSource: this.$dataSource,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "200px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "200px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "IdTipoApertura",
                    title: "TIPO DE<br>CONVOCATORIA",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var mensajeAbierto = "'abierto'";
                        var mensajeCerrado = "'Para participar en una convocatoria CERRADA debe ser un trabajador activo del Ministerio de Desarrollo e Inclusión Social'";
                        var texto = ''
                        if (item.IdTipoApertura == 1) {
                            texto = 'ABIERTA  <button type="button" class="btn btn-info btn-xs" onclick="controladorApp.notificarMensajeDeAlerta(' + mensajeAbierto + ')">?</button>'
                        }
                        if (item.IdTipoApertura == 2) {
                            texto = 'CERRADA  <button type="button" class="btn btn-info btn-xs" onclick="controladorApp.notificarMensajeDeAlerta(' + mensajeCerrado + ')">?</button>'
                        }

                        return texto;
                    }
                },
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'AVISO DE<br>CONVOCATORIA',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.abrirModalPostulacionServir(\'' + item.Contrasena + '\')">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }
                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }
                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    };
    this.ConvocatoriaJS.prototype.inicializarGridAnonimo = function () {
        this.$dataSource1 = [];
        this.$dataSource1 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 1;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2024" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$dataSource2 = [];
        this.$dataSource2 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 1;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2023"}
            ]
        });

        this.$dataSource3 = [];
        this.$dataSource3 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 1;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2022" }
            ]
        });

        this.$dataSource4 = [];
        this.$dataSource4 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 1;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2021" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });


        this.$dataSource6 = [];
        this.$dataSource6 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatorias_Historica',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 1;
                        //data_param.Anio = $("#ddlAnioCAS").data("kendoDropDownList").value();
                        data_param.Anio = $("#ddlAnioCAS").val();
                        data_param.Mes = $("#ddlMesCAS").val(); 
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            //filter: [
            //    { field: "NroConvocatoria", operator: "contains", value: "-2020" }
            //]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$grid1 = $("#divGrid1").kendoGrid({
            dataSource: this.$dataSource1,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.iniciarSesion()">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid2 = $("#divGrid2").kendoGrid({
            dataSource: this.$dataSource2,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.iniciarSesion()">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid3 = $("#divGrid3").kendoGrid({
            dataSource: this.$dataSource3,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.iniciarSesion()">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid4 = $("#divGrid4").kendoGrid({
            dataSource: this.$dataSource4,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.iniciarSesion()">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid6 = $("#divGrid6").kendoGrid({
            dataSource: this.$dataSource6,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:left;" },
                    width: "100px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "200px"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "200px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.CantidadVacantes == 0)
                            controles = '';
                        else
                            controles = item.CantidadVacantes;

                        return controles;
                    }
                },
                {
                    field: "FechaPublicacion",
                    title: "FECHA <br>PUBLICACIÓN",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "Bases",
                    title: "BASES",
                    width: "50px",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        var vUrl = $("#hdRutaBaseAnte").val();
                        var vNombre = vUrl + "/" + item.Bases;
                        controles += '<a href="' +vNombre + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        return controles;
                   }
                },
                //{
                //    //INGRESAR DETALLE DE LA EVALUACION
                //    title: 'ACCIÓN',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        if (item.Estado == 1) {
                //            if (item.IdMostrarPostular == 1) {
                //                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.iniciarSesion()">'; //GenerarFormatoContrato
                //                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                //                controles += '</button>';
                //            }
                //        }
                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        var vUrl = $("#hdRutaEstadoAnte").val();

                        if (item.DocumentoComunicados != '') {

                            var arrayDeCadenas = item.DocumentoComunicados.split('|');

                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                var vTipDoc = arrayDeCadenas[i].split('#')[0];
                                var vNomDoc = arrayDeCadenas[i].split('#')[1];
                                var vFecha = " [" + arrayDeCadenas[i].split('#')[2] + "]";
                                var vNombre = vUrl + "/" + vNomDoc;
                                controles += '<a href="' + vNombre + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar comunicado"></span>' + vTipDoc +'</a>';
                                controles += vFecha + '<br/>';
                            }
                        }

                        return controles;
                    },
                    width: '40px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        var vUrl = $("#hdRutaResulAnte").val();
                        if (item.DocumentoCurricular != null) {
                            var vNomDoc = item.DocumentoCurricular.split('#')[0];
                            var vFecha = " [" + item.DocumentoCurricular.split('#')[1] + "]";
                            var vNombre = vUrl + "/" + vNomDoc;
                            //controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += '<a href="' + vNombre + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar resultado curricular"></span></a>';
                            controles += '<br />'+vFecha;
                        }
                        return controles;
                    },
                    width: '30px'
                },
                //{
                //    title: 'EXAMEN DE<br>CONOCIMIENTO',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        if (item.TextoConocimientos != null) {
                //            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                //            controles += item.TextoConocimientos.split('#')[2];
                //        }

                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        var vUrl = $("#hdRutaResulAnte").val();

                        if (item.DocumentoResultadoFinal != null) {
                            var vNomDoc = item.DocumentoResultadoFinal.split('#')[0];
                            var vFecha = " [" + item.DocumentoResultadoFinal.split('#')[1] + "]";
                            var vNombre = vUrl + "/" + vNomDoc;
                            //controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += '<a href="' + vNombre + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar resultado final"></span></a>';
                            controles += '<br />'+vFecha;
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    };
    this.ConvocatoriaJS.prototype.inicializarGridServirAnonimo = function () {
        this.$dataSource1 = [];
        this.$dataSource1 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 3;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2024" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$grid1 = $("#divGridServir1").kendoGrid({
            dataSource: this.$dataSource1,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "200px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "200px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "IdTipoApertura",
                    title: "TIPO DE<br>CONVOCATORIA",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var mensajeAbierto = "'abierto'";
                        var mensajeCerrado = "'Para participar en una convocatoria CERRADA debe ser un trabajador activo del Ministerio de Desarrollo e Inclusión Social'";
                        var texto = ''
                        if (item.IdTipoApertura == 1) {
                            texto = 'ABIERTA  <button type="button" class="btn btn-info btn-xs" onclick="controladorApp.notificarMensajeDeAlerta(' + mensajeAbierto +')">?</button>'
                        }
                        if (item.IdTipoApertura == 2) {
                            texto = 'CERRADA  <button type="button" class="btn btn-info btn-xs" onclick="controladorApp.notificarMensajeDeAlerta(' + mensajeCerrado + ')">?</button>'
                        }
                        
                        return texto;
                    }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'AVISO DE<br>CONVOCATORIA',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoBases/?id=' + item.IdBase + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.iniciarSesion()">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EXAMEN DE<br>CONOCIMIENTO',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoConocimientos != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoConocimientos.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    };
    this.ConvocatoriaJS.prototype.inicializarGridPracticasAnonimo = function () {
        this.$dataSource1 = [];
        this.$dataSource1 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 2;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2024" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$dataSource2 = [];
        this.$dataSource2 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 2;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2023" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$dataSource3 = [];
        this.$dataSource3 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 2;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2022" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });

        this.$dataSource4 = [];
        this.$dataSource4 = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Convocatoria/ListarConvocatoriasPublico',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = 0;
                        data_param.IdConvocatoria = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipo = 2;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdConvocatoria"
                }
            },
            filter: [
                { field: "NroConvocatoria", operator: "contains", value: "-2021" }
            ]
            //aggregate: [
            //        { field: "NombreCompleto", aggregate: "count" },
            //        { field: "NombreOficina", aggregate: "count" }
            //]
        });


        this.$grid1 = $("#divGridPracticas1").kendoGrid({
            dataSource: this.$dataSource1,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPractica/?idConvocatoria=' + item.IdConvocatoria + '&idTipo=2' + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.abrirModalPracticas(\'' + item.IdConvocatoria + '\',\'' + item.IdTipo + '\')">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                //{
                //    title: 'EXAMEN DE<br>CONOCIMIENTO',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        //if (item.TextoConocimientos != null) {
                //        //    controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                //        //    controles += item.TextoConocimientos.split('#')[2];
                //        //}

                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid2 = $("#divGridPracticas2").kendoGrid({
            dataSource: this.$dataSource2,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPractica/?idConvocatoria=' + item.IdConvocatoria + '&idTipo=2' + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.abrirModalPracticas(\'' + item.IdConvocatoria + '\',\'' + item.IdTipo + '\')">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                //{
                //    title: 'EXAMEN DE<br>CONOCIMIENTO',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        //if (item.TextoConocimientos != null) {
                //        //    controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                //        //    controles += item.TextoConocimientos.split('#')[2];
                //        //}

                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid3 = $("#divGridPracticas3").kendoGrid({
            dataSource: this.$dataSource3,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPractica/?idConvocatoria=' + item.IdConvocatoria + '&idTipo=2' + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.abrirModalPracticas(\'' + item.IdConvocatoria + '\',\'' + item.IdTipo + '\')">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                //{
                //    title: 'EXAMEN DE<br>CONOCIMIENTO',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        //if (item.TextoConocimientos != null) {
                //        //    controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                //        //    controles += item.TextoConocimientos.split('#')[2];
                //        //}

                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
        this.$grid4 = $("#divGridPracticas4").kendoGrid({
            dataSource: this.$dataSource4,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            //groupable: true,
            //detailTemplate: kendo.template($("#template").html()),
            //detailInit: detailInit,
            dataType: 'json',
            //dataBound: function () {
            //    this.expandRow(this.tbody.find("tr.k-master-row").first());
            //},
            columns: [
                {
                    field: "NroConvocatoria",
                    title: "PROCESO",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Dependencia",
                    title: "OFICINA SOLICITANTE",
                    width: "250px",
                    template: function (item) {
                        var controles = item.Organo;
                        if (item.Dependencia != 'NO APLICA') {
                            controles = item.Dependencia;
                        }

                        return controles;
                    },
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "250px"
                },
                {
                    field: "CantidadVacantes",
                    title: "VACANTES ",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "FechaPublicacionDesde",
                //    title: "INICIO PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                //{
                //    field: "FechaPublicacionHasta",
                //    title: "FIN PUBLICACIÓN",
                //    width: "30px",
                //    attributes: { style: "text-align:center;" }
                //},
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE<br>POSTULACIÓN",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                //{
                //    field: "Estado",
                //    title: "ESTADO",
                //    attributes: { style: "text-align:center;" },
                //    width: "50px",
                //    aggregates: ["count"],
                //    groupHeaderTemplate: "#= value # (Total: #= count#)"
                //},
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'BASES',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.IdTieneRequerimiento == 1) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPractica/?idConvocatoria=' + item.IdConvocatoria + '&idTipo=2' + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-download-alt" title="Descargar bases de convocatoria"></span></a>';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: 'ACCIÓN',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            if (item.IdMostrarPostular == 1) {
                                controles += '<button type="button" class="btn btn-success btn-xs" style="background-color: #449d44; border-color: #398439;" onclick="controlador.abrirModalPracticas(\'' + item.IdConvocatoria + '\',\'' + item.IdTipo + '\')">'; //GenerarFormatoContrato
                                controles += '<span class="glyphicon glyphicon-log-in" aria-hidden="true" data-uib-tooltip="Iniciar postulación" title="Iniciar postulación"></span> POSTULAR';
                                controles += '</button>';
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'COMUNICADOS',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoComunicados != '') {
                            var arrayDeCadenas = item.TextoComunicados.split('|');
                            for (var i = 0; i < arrayDeCadenas.length; i++) {
                                controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + arrayDeCadenas[i].split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span>' + arrayDeCadenas[i].split('#')[0] + '</a>';
                                controles += arrayDeCadenas[i].split('#')[2];
                            }
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: 'EVALUACIÓN<br>CURRICULAR',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //alert(item.TextoCurricular);
                        if (item.TextoCurricular != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoCurricular.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoCurricular.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                },
                //{
                //    title: 'EXAMEN DE<br>CONOCIMIENTO',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        //if (item.TextoConocimientos != null) {
                //        //    controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPorId/?idConvocatoriaDoc=' + item.TextoConocimientos.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                //        //    controles += item.TextoConocimientos.split('#')[2];
                //        //}

                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: 'RESULTADO<br>FINAL',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.TextoFinal != null) {
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Convocatoria/DescargarArchivoPracticaPorId/?idConvocatoriaDoc=' + item.TextoFinal.split('#')[1] + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file" title="Descargar"></span> RESULTADOS </a>';
                            controles += item.TextoFinal.split('#')[2];
                        }

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    };
    function detailInit(e) {
        var detailRow = e.detailRow;

        detailRow.find(".tabstrip").kendoTabStrip({
            animation: {
                open: { effects: "fadeIn" }
            }
        });

        detailRow.find(".orders").kendoGrid({
            dataSource: {
                type: "odata",
                transport: {
                    read: "https://demos.telerik.com/kendo-ui/service/Northwind.svc/Orders"
                },
                serverPaging: true,
                serverSorting: true,
                serverFiltering: true,
                pageSize: 7,
                filter: { field: "EmployeeID", operator: "eq", value: e.data.EmployeeID }
            },
            scrollable: false,
            sortable: true,
            pageable: true,
            columns: [
                { field: "OrderID", title: "ID", width: "70px" },
                { field: "ShipCountry", title: "Ship Country", width: "110px" },
                { field: "ShipAddress", title: "Ship Address" },
                { field: "ShipName", title: "Ship Name", width: "300px" }
            ]
        });
    }

    this.ConvocatoriaJS.prototype.buscar = function (e) {
        e.preventDefault();

        var grilla = $('#divGrid').data("kendoGrid");
        grilla.dataSource._sort = undefined;
        grilla.dataSource.page(1);

        //$("#lblTotal").html(grilla.dataSource.total());
    };

    function CargarFormularioConvocatoria(id) {
        var data_param = new FormData();
        data_param.append('id', id);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Bases/ObtenerBasesPerfilesConvocatoriaPorID',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                $("#hdIdPerfil").val(res.iCodPerfil);
                $("#txtNroConvocatoria").val(res.strNroCAS);
                //$("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtCantidadConvocatoria").val(res.iCantPersonalRequerido);
                $("#txtRemuneracionConvocatoria").val(res.decRemuneracion);
                $("#txtCargoConvocatoria").val(res.strNombrePuesto);

                //$("#spCronograma1").text(kendo.toString(kendo.parseDate(res.dFechaDesdePubMIDIS), 'dd/MM/yyyy') + ' - ' + kendo.toString(kendo.parseDate(res.dFechaHastaPubMIDIS), 'dd/MM/yyyy'));
                //$("#spCronograma2").text(kendo.toString(kendo.parseDate(res.dFechaRegCVPostulante), 'dd/MM/yyyy'));
                //$("#spCronograma3").text(kendo.toString(kendo.parseDate(res.dFechaDesdeEvaCV), 'dd/MM/yyyy') + ' - ' + kendo.toString(kendo.parseDate(res.dFechaDesdeEvaCV), 'dd/MM/yyyy'));
                //$("#spCronograma4").text(kendo.toString(kendo.parseDate(res.dFechaPubResultadoMIDIS), 'dd/MM/yyyy'));
                //$("#spCronograma5").text(kendo.toString(kendo.parseDate(res.dFechaDesdeEntrevista), 'dd/MM/yyyy') + ' - ' + kendo.toString(kendo.parseDate(res.dFechaDesdeEntrevista), 'dd/MM/yyyy'));
                //$("#spCronograma6").text(kendo.toString(kendo.parseDate(res.dFechaPubResultadoFinalMIDIS), 'dd/MM/yyyy'));
                //$("#spCronograma7").text(kendo.toString(kendo.parseDate(res.dFechaDesdeSuscripcionContrato), 'dd/MM/yyyy') + ' - ' + kendo.toString(kendo.parseDate(res.dFechaHastaSuscripcionContrato), 'dd/MM/yyyy'));
                
                
                PDFObject.embed(res.strNombreArchivo, "#app");
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res[0].responseText);
            }
        });
    }

    this.ConvocatoriaJS.prototype.abrirModalRegistroConvocatoria = function (id) {
        var modal = $('#divModalRegistroConvocatoria').data('kendoWindow');
        
        LimpiarModalRegistroConvocatoria();

        if (id == 0) {           
            //$("#ddlPersonaEstado").data("kendoDropDownList").value('1');
            //$("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value('01');
            
            $('#tab2').removeClass('in active');
            $('#tab1').addClass('in active');
            $('#liTab2').removeClass('active');
            $('#liTab1').addClass('active');
            
            //controlador.CargarFormularioCuentasBancarias(-1);
            //controlador.CargarFormularioOrdenesServicio(-1);
            //$('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
            //$("#txtPersonaCargo").attr("required", true);
            //$("#txtRUC").removeAttr("required");
            
            //if ($("#btnGuardarAdmin") != null) $("#btnGuardarAdmin").text("Guardar");
            //if ($("#btnGuardarAbastecimiento") != null) $("#btnGuardarAbastecimiento").text("Guardar");
            //if ($("#btnGuardarContacto") != null) $("#btnGuardarContacto").text("Guardar");

            modal.title("Ingresar Proceso de Convocatoria");
            modal.open().center();
        }
        else {
            $("#hdIdConvocatoria").val(id);

            $("#_tab4").attr("data-toggle", "tab");
            $('#_tab4').prop("onclick", null).off("click");

            controlador.CargarFormularioTrabajador(id);
            controlador.CargarFormularioCuentasBancarias(id);
            controlador.CargarFormularioOrdenesServicio(id);

            if ($("#btnGuardarAdmin") != null) $("#btnGuardarAdmin").text("Actualizar");
            if ($("#btnGuardarAbastecimiento") != null) $("#btnGuardarAbastecimiento").text("Actualizar");
            if ($("#btnGuardarContacto") != null) $("#btnGuardarContacto").text("Actualizar");

            modal.title("Actualizar Trabajador");
            modal.open();
        }
    }

    this.ConvocatoriaJS.prototype.cerrarModalRegistroConvocatoria = function () {
        var modal = $('#divModalRegistroConvocatoria').data('kendoWindow');
        modal.close();
    }

    this.ConvocatoriaJS.prototype.CargarFormularioTrabajador = function (id) {
        var data_param = new FormData();
        data_param.append('IdEmpleado', id);
        
        //$('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").readonly();
        //$('#txtPersonaNumeroDeDocumento').prop('readonly', true);
        //$('#btnBuscarP').prop('disabled', true);
        //$('#ddlPersonaSexo').data("kendoDropDownList").readonly();
        //$('#ddlPersonaEstadoCivil').data("kendoDropDownList").readonly();
        //$('#txtPersonaFechaNacimiento').data("kendoDatePicker").readonly();
        //$('#txtPersonaRUC').prop('readonly', true);
        //$('#txtPersonaCorreoElectronicoP').prop('readonly', true);
        //$('#txtTelefonoCelularP').prop('readonly', true);
        //$('#txtTelefonoFijoP').prop('readonly', true);
        
        $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").enable(true);
        $('#txtPersonaNumeroDeDocumento').prop('readonly', false);
        $('#btnBuscarP').prop('disabled', false);
        $('#ddlPersonaSexo').data("kendoDropDownList").enable(true);
        $('#ddlPersonaEstadoCivil').data("kendoDropDownList").enable(true);
        $('#txtPersonaFechaNacimiento').data("kendoDatePicker").enable(true);
        $('#txtPersonaRUC').prop('readonly', false);
        $('#txtPersonaCorreoElectronicoP').prop('readonly', false);
        $('#txtTelefonoCelularP').prop('readonly', false);
        $('#txtTelefonoFijoP').prop('readonly', false);
        
        

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Empleado/ObtenerParaEditar',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtPersonaNumeroDeDocumento").val(res.NroDocumento);
                $("#txtPersonaNombres").val(res.Nombre);
                $("#txtPersonaApellidoPaterno").val(res.Paterno);
                $("#txtPersonaApellidoMaterno").val(res.Materno);
                $("#ddlPersonaSexo").data("kendoDropDownList").value(res.IdGenero);
                $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(res.IdEstadoCivil);
                $("#ddlPersonaGrupoSan").data("kendoDropDownList").value(res.IdGrupoSanguineo);
                $("#txtPersonaDireccionDomicilio").val(res.Domicilio);
                $("#txtPersonaUbigeo").val(res.DescripcionUbigeo);
                $("#txtPersonaCorreoElectronicoP").val(res.CorreoElectronico);
                $("#txtTelefonoCelularP").val(res.Celular);
                $("#txtTelefonoFijoP").val(res.Telefono);
                $("#txtPersonaCorreoElectronico").val(res.CorreoElectronicoLaboral);
                $("#txtTelefonoCelular").val(res.CelularLaboral);
                $("#txtTelefonoLaboral").val(res.TelefonoLaboral);
                $("#txtTelefonoAnexo").val(res.AnexoLaboral);
                $("#ddlPersonaEstado").data("kendoDropDownList").value(res.Estado);
                $("#ddlPersonaCondicion").data("kendoDropDownList").value(res.IdCondicion);

                if (res.Estado == 1) {
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaFechaFinLabores").removeAttr("required");
                    $("#txtPersonaFechaFinLabores").val('');
                }//$("#divCese").hide();
                if (res.Estado == 0) {
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaFechaFinLabores").attr("required", true);
                    $("#txtPersonaFechaFinLabores").attr("validationmessage", "requerido");
                    $("#txtPersonaFechaFinLabores").data("kendoDatePicker").value(res.FechaCese);
                }//$("#divCese").show();

                if (res.IdCondicion == '5') {
                    $("#divOrdenes").show();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaCargo").removeAttr("required");
                    $("#txtRUC").attr("required", true);
                }
                else {
                    $("#divOrdenes").hide();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaCargo").attr("required", true);
                    $("#txtRUC").removeAttr("required");
                } 

                $("#ddlPersonaSede").data("kendoDropDownList").value(res.IdSede);
                $("#ddlPersonaDependencia").data("kendoDropDownList").value(res.IdDependencia);
                $("#txtPersonaCargo").val(res.NombreCargo);
                $("#txtRUC").val(res.RUC);
                $("#hdIdUbigeoEmpleado").val(res.Ubigeo);
                
                $("#txtPersonaFechaNacimiento").data("kendoDatePicker").value(kendo.parseDate(res.FechaNacimiento));
                $("#txtPersonaFechaInicioLabores").data("kendoDatePicker").value(res.FechaInicio);
                
                if (res.Foto != '') {
                    document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,' + res.Foto);
                    $('#hdFoto').val(res.Foto);
                }
                else {
                    if (($("#hdPerfil").val() != PERFIL_NOMINA_CONTACTO) && ($("#hdPerfil").val() != PERFIL_NOMINA_CONTABILIDAD)) 
                        $("#btnBuscarFoto").show();
                }   
            },
            error: function (res) {
                debugger;
            }
        });
    }

    this.ConvocatoriaJS.prototype.CargarFormularioCuentasBancarias = function (id) {
        this.$dataSourceCuentas = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarCuentasEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarCuentaEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarCuentaEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarCuentaEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.IdEmpleado = id;
                            data_param.Estado = 0;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            //if (frmCompromiso.validate()) {                                
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdBanco = $options.Banco.IdBanco;
                            data_param.CCI = $options.CCI;
                            data_param.NroCuenta = '';
                            data_param.IdEstado = $options.Estado.Codigo;
                            //}
                            break;
                        case "update":
                            data_param.IdEmpleadoBanco = $options.IdEmpleadoBanco;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdBanco = $options.Banco.IdBanco;
                            data_param.CCI = $options.CCI;
                            data_param.NroCuenta = '';
                            data_param.IdEstado = $options.Estado.Codigo;
                            break;
                        case "destroy":
                            data_param.IdEmpleadoBanco = $options.IdEmpleadoBanco;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdBanco = $options.Banco.IdBanco;
                            data_param.CCI = $options.CCI;
                            data_param.NroCuenta = '';
                            data_param.IdEstado = 0;
                            break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                switch (e.type) {
                    case "create": case "update": case "destroy":
                        var grilla = $('#divGridCuentas').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleadoBanco",
                    fields: {
                        IdEmpleadoBanco: { editable: false, nullable: true },
                        CCI: { validation: { required: true } },
                        Banco: {
                            validation: {
                                required: true,
                                bancovalidation: function (input) {
                                    if (input.is("[name='Banco']") && input.val() == "") {
                                        input.attr("data-bancovalidation-msg", "Entidad bancaria es requerida");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { IdBanco: 8, Nombre: "Nombre" }
                        },
                        Estado: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Estado']") && input.val() == "") {
                                        input.attr("data-estadovalidation-msg", "El estado es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ACTIVO" }
                        },
                    }
                }
            },
        });

        this.divGridCuentas = $("#divGridCuentas").kendoGrid({
            toolbar: ["excel", "create"],
            excel: {
                fileName: "Listado de cuentas bancarias del trabajador.xlsx",
                filterable: false
            },
            dataSource: this.$dataSourceCuentas,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "Banco",
                    title: "Entidad Bancaria",
                    width: "200px",
                    editor: controlador.BancoDropDownEditor,
                    template: "#=Banco.Nombre#"
                },
                {
                    field: "CCI",
                    title: "Código de Cuenta Interbancaria (CCI)",
                    attributes: { style: "text-align:left;" },

                    width: "200px"
                },
                {
                    field: "Estado",
                    title: "Estado",
                    width: "50px",
                    editor: controlador.EstadoDropDownEditor,
                    template: "#=Estado.Nombre#"
                },
                { title: "Acciones", command: ["edit", "destroy"], width: "250px" },
            ],
            editable: "inline"
        }).data();
    }

    this.ConvocatoriaJS.prototype.BancoDropDownEditor = function (container, options) {
        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "IdBanco",
                optionLabel: "-- Seleccione --",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Persona/ListarBancos",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = 0;
                            data_param.Grilla.OrdenarPor = "Nombre";
                            data_param.Grilla.OrdenarDeForma = "ASC";

                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.ConvocatoriaJS.prototype.EstadoDropDownEditor = function (container, options) {
        var data = [
                        { Nombre: "ACTIVO", Codigo: "1" },
                        { Nombre: "INACTIVO", Codigo: "0" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };

    this.ConvocatoriaJS.prototype.CargarFormularioOrdenesServicio = function (id) {
        this.$dataSourceOrdenes = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarOrdenesEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarOrdenEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarOrdenEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarOrdenEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.IdEmpleado = id;
                            data_param.Estado = 0;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.NroOrden = $options.NroOrden;
                            data_param.NroSIAF = $options.NroSIAF;
                            data_param.Nombre = $options.Nombre.toUpperCase();
                            data_param.Duracion = $options.Duracion;
                            data_param.FechaInicio = $options.FechaInicio;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.Monto = $options.Monto;
                            data_param.IdEstado = $options.Estado.Codigo;
                            break;
                        case "update":
                            data_param.IdEmpleadoOrden = $options.IdEmpleadoOrden;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.NroOrden = $options.NroOrden;
                            data_param.NroSIAF = $options.NroSIAF;
                            data_param.Nombre = $options.Nombre.toUpperCase();
                            data_param.Duracion = $options.Duracion;
                            data_param.FechaInicio = $options.FechaInicio;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.Monto = $options.Monto;
                            data_param.IdEstado = $options.Estado.Codigo;
                            break;
                        case "destroy":
                            data_param.IdEmpleadoOrden = $options.IdEmpleadoOrden;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.NroOrden = $options.NroOrden;
                            data_param.Nombre = $options.Nombre.toUpperCase();
                            data_param.Duracion = $options.Duracion;
                            data_param.FechaInicio = $options.FechaInicio;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.IdEstado = 0;
                            break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                switch (e.type) {
                    case "create": case "update": case "destroy":
                        var grilla = $('#divGridOrdenes').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleadoOrden",
                    fields: {
                        IdEmpleadoOrden: { editable: false, nullable: true },
                        NroOrden: { validation: { required: true, maxlength: "15" } },
                        NroSIAF: { validation: { required: true, maxlength: "15" } },
                        Duracion: { validation: { required: true, type: "number" } },
                        Nombre: { validation: { required: true } },
                        FechaInicio: {
                            validation: {
                                required: true,
                                type: "date",
                                format: "dd/MM/yyyy",
                                fechainiciovalidation: function (input) {
                                    if (input.is("[name='FechaInicio']") && input.val() == "") {
                                        input.attr("data-fechainiciovalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        FechaFin: {
                            validation: {
                                required: true,
                                type: "date",
                                format: "dd/MM/yyyy",
                                fechafinvalidation: function (input) {
                                    if (input.is("[name='FechaFin']") && input.val() == "") {
                                        input.attr("data-fechafinvalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        Monto: {
                            validation: {
                                required: true,
                                type: "number",
                                montovalidation: function (input) {
                                    if (input.is("[name='Monto']") && input.val() == "") {
                                        input.attr("data-montovalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        Estado: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Estado']") && input.val() == "") {
                                        input.attr("data-estadovalidation-msg", "El estado es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ACTIVO" }
                        },
                    }
                }
            },
        });

        this.divGridOrdenes = $("#divGridOrdenes").kendoGrid({
            toolbar: ["excel", "create"], //, { text: "Add New Email", className: "k-grid-addEmail", imageClass: "k-add" }, { text: "Add New Cell Text Message", className: "k-grid-addText", imageClass: "k-add" }
            excel: {
                fileName: "Listado de ordenes de servicio del trabajador.xlsx",
                filterable: false
            },
            dataSource: this.$dataSourceOrdenes,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            dataBound: function (e) {
                $('.k-grid-add').unbind("click");
                $('.k-grid-add').bind("click", function () {
                    if ($("#hdIdEmpleado").val() == 0) {
                        controladorApp.notificarMensajeDeAlerta('Primero debe completar el registro del empleado');
                        return false;
                    }
                });
            },
            columns: [
                {
                    field: "NroOrden",
                    title: "Nro Orden",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "NroSIAF",
                    title: "Nro SIAF",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Nombre",
                    title: "Nombre de la Orden de Servicio",
                    attributes: { style: "text-align:left;" },
                    editor: controlador.NombreOrdenEditor,
                    width: "200px"
                },
                {
                    field: "Duracion",
                    title: "Plazo",
                    attributes: { style: "text-align:center;" },

                    width: "30px"
                },
                {
                    field: "FechaInicio",
                    title: "Fecha Inicio",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaInicioEditor,
                    width: "70px"
                },
                {
                    field: "FechaFin",
                    title: "Fecha Fin",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaFinEditor,
                    width: "80px"
                },
                {
                    field: "Monto",
                    title: "Monto",
                    attributes: { style: "text-align:right;" },
                    format: "{0:c}",
                    width: "50px"
                },
                {
                    field: "Estado",
                    title: "Estado",
                    width: "30px",
                    editor: controlador.EstadoDropDownEditor,
                    template: "#=Estado.Nombre#"
                },
                { title: "Acciones", command: ["edit", "destroy"], width: "50px" },
            ],
            editable: "inline"
        }).data();
                
    }
     

    this.ConvocatoriaJS.prototype.NombreOrdenEditor = function (container, options) {
        $('<textarea required name="' + options.field + '" style="width: ' + (container.width() + 30) + 'px;height:' + (container.height() + 30) + 'px" />')
            .appendTo(container);
    };
    this.ConvocatoriaJS.prototype.FechaInicioEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width: ' + container.width() + 'px" data-bind="value:FechaInicio">')
            .appendTo(container);
    };
    this.ConvocatoriaJS.prototype.FechaFinEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width: ' + container.width() + 'px" data-bind="value:FechaFin">')
            .appendTo(container);
    };
    this.ConvocatoriaJS.prototype.agregarConvocatoria = function (e) {
        e.preventDefault();
        var metodo = 'Registrar';
        
        if (frmPersonaValidador.validate()) {
            var modal = $('#divModalRegistroConvocatoria').data('kendoWindow');
            var esValido = true;
            var mensajeValidacion = '';

            var data_param = new FormData();
            
            data_param.append('IdBase', $("#ddlBases").data("kendoDropDownList").value());
            data_param.append('IdPerfil', $("#hdIdPerfil").val());
            data_param.append('IdDependencia', $("#ddlDependencia").data("kendoDropDownList").value());
            data_param.append('NroAIRHSP', $("#txtAIRHSPConvocatoria").val());
            data_param.append('Meta', $("#txtMetaConvocatoria").val());
            data_param.append('ResponsableCurricular', $("#ddlResponsableCurricular").data("kendoDropDownList").value());
            data_param.append('Estado', 1);
            
            var existeFile = false;
            var upload1 = $("#fileDocCertificacion").getKendoUpload();
            var file1 = upload1.getFiles();
            if (file1.length == 0) {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar el documento de sustento de la certificación (Formato PDF)');
                return false;
            } else {
                if (file1.length == 0) {
                    data_param.append('formatos[0]', null);
                }
                else {
                    if (file1[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[0]', file1[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }
            var upload2 = $("#fileDocRequerimiento").getKendoUpload();
            var file2 = upload2.getFiles();
            if (file2.length == 0) {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar el documento de sustento del requerimiento (Formato PDF)');
                return false;
            } else {
                if (file2.length == 0) {
                    data_param.append('formatos[1]', null);
                }
                else {
                    if (file2[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[1]', file2[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }
            
            controladorApp.abrirMensajeDeConfirmacion(
                '<span>¿Está seguro de ingresar la convocatoria?.</span></br></br><div class="alert alert-warning" style="padding: 5px; border: 1px solid #f7cf0aa0;">Recuerde que al confirmar esta operación la convocatoria automaticamente será publicada</div>', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Convocatoria/' + metodo,
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Convocatoria registrado correctamente");

                                // REFRESCAR INFORMACION DEL TRABAJADOR
                                $("#hdIdConvocatoria").val(res.responseText);

                                modal.close();
                                $('#divGrid').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }
    this.ConvocatoriaJS.prototype.agregarPracticas = function (e) {
        e.preventDefault();
        
        if (frmPracticasValidador.validate()) {
            //alert('En construccion...')
            
            var modal = $('#divModalRegistroPracticas').data('kendoWindow');
            var esValido = true;
            var mensajeValidacion = '';

            var data_param = new FormData();

            //data_param.append('IdBase', $("#ddlBases").data("kendoDropDownList").value());
            data_param.append('IdConvocatoria', $('#hdIdConvocatoria').val());
            data_param.append('TipoDocumento', $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value());
            data_param.append('NroDocumento', $("#txtPersonaNumeroDeDocumento").val());
            data_param.append('TelefonoCelular', $("#txtTelefonoCelularP").val());
            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('Estado', 1);

            var existeFile = false;
            var upload1 = $("#fileDocAnexo").getKendoUpload();
            var file1 = upload1.getFiles();
            if (file1.length == 0) {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar el documento de anexos de prácticas completo (Formato PDF)');
                return false;
            } else {
                if (file1.length == 0) {
                    data_param.append('formatos[0]', null);
                }
                else {
                    if (file1[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[0]', file1[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }
            var upload2 = $("#fileDocRequerimiento").getKendoUpload();
            var file2 = upload2.getFiles();
            if (file2.length == 0 ) {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar su hoja de vida documentada (Formato PDF)');
                return false;
            } else {
                if (file2.length == 0) {
                    data_param.append('formatos[1]', null);
                }
                else {
                    if (file2[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[1]', file2[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }
            var upload3 = $("#fileDocCarta").getKendoUpload();
            var file3 = upload3.getFiles();
            if (file3.length == 0) {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar el documento de acreditación como estudiante (Formato PDF)');
                return false;
            } else {
                if (file3.length == 0) {
                    data_param.append('formatos[2]', null);
                }
                else {
                    if (file3[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[2]', file3[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }

            controladorApp.abrirMensajeDeConfirmacion(
                '<span>¿Está seguro de enviar su postulación?.</span></br></br><div class="alert alert-warning" style="padding: 5px; border: 1px solid #f7cf0aa0;">Recuerde que al confirmar su postulación quedará registrada y NO SE PODRÁ REALIZAR NINGÚN CAMBIO</div>', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/IngresarPostulacionPractica',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Postulación registrada correctamente");

                                // REFRESCAR INFORMACION DEL TRABAJADOR
                                $("#hdIdConvocatoria").val(res.responseText);

                                modal.close();
                                //$('#divGrid').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }

    this.ConvocatoriaJS.prototype.generarFormatoHojaVida = function () {
        var IdPostulante = $("#hdIdPostulante").val();
        var IdPostulacion = $("#hdIdPostulacion").val();
        var IdConvocatoria = $("#hdIdConvocatoria").val();

        window.open(controladorApp.obtenerRutaBase() + "Postulante/HojaVida?IdPostulante=" + IdPostulante + "&IdPostulacion=" + IdPostulacion + "&IdConvocatoria=" + IdConvocatoria, "_blank");
    }
    this.ConvocatoriaJS.prototype.generarFormatoHojaVidaServir = function () {
        var IdPostulante = $("#hdIdPostulante").val();
        var IdPostulacion = $("#hdIdPostulacion").val();
        var IdConvocatoria = $("#hdIdConvocatoria").val();

        window.open(controladorApp.obtenerRutaBase() + "Postulante/HojaVidaServir?IdPostulante=" + IdPostulante + "&IdPostulacion=" + IdPostulacion + "&IdConvocatoria=" + IdConvocatoria, "_blank");
    }
    this.ConvocatoriaJS.prototype.GenerarFormatoContrato = function () {
        var IdContrato = $('#hdIdContrato').val();

        window.open(controladorApp.obtenerRutaBase() + "Contrato/Ficha?idContrato=" + IdContrato, "_blank");
        $("#btnGenerarFicha").css("display", "");
    }
    this.ConvocatoriaJS.prototype.agregarContratoArchivo = function (e) {
        e.preventDefault();
        
        var modal = $('#divModalFormato').data('kendoWindow');
        var esValido = true;
        var mensajeValidacion = '';

        var data_param = new FormData();
        data_param.append('IdContrato', $('#hdIdContrato').val());
        
        var upload1 = $("#fileFormato").getKendoUpload();
        var firmas = upload1.getFiles();
        if (firmas.length == 0) {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar el contrato que será enviado al postulante ganador del proceso (Formato PDF)');
            return false;
        } else {
            for (var j = 0; j < firmas.length; j++) {
                if (firmas[0].extension.toLowerCase() == '.pdf')
                    data_param.append('formatos[' + j.toString() + ']', firmas[j].rawFile);
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }

        controladorApp.abrirMensajeDeConfirmacion('¿Desea actualizar el contrato y enviarlo al postulante ganador del proceso para su revisión?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Contrato/RegistrarContratoArchivo',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                $("#console").append(res.responseText);
                                controladorApp.notificarMensajeDeAlerta("La actualización del contrato no se pudo realizar");
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("El contrato se actualizó de forma correcta");
                                modal.close();
                                $('#divGrid').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
    }
    this.ConvocatoriaJS.prototype.actualizarTrabajadorContacto = function (e) {
        e.preventDefault();
        
        if (frmPersonaValidador.validate()) {
            var modal = $('#divModalRegistroConvocatoria').data('kendoWindow');
            var esValido = true;
            var mensajeValidacion = '';

            //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid($('#hdnUid').val());
            var data_param = new FormData();
            if ($("#hdIdEmpleado").val() != 0) 
                data_param.append('IdEmpleado', $("#hdIdEmpleado").val());
            
            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('Celular', $("#txtTelefonoCelularP").val());
            data_param.append('Telefono', $("#txtTelefonoFijoP").val());
            data_param.append('CorreoElectronicoLaboral', $("#txtPersonaCorreoElectronico").val());
            data_param.append('CelularLaboral', $("#txtTelefonoCelular").val());
            data_param.append('TelefonoLaboral', $("#txtTelefonoLaboral").val());
            data_param.append('AnexoLaboral', $("#txtTelefonoAnexo").val());
            
            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de actualizar el trabajador?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Empleado/GuardarContacto',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                if (($("#hdPerfil").val() == PERFIL_NOMINA_CONTACTO) || ($("#hdPerfil").val() == PERFIL_NOMINA_CONTABILIDAD)) {
                                    modal.close();
                                    $('#divGrid').data("kendoGrid").dataSource.page(1);
                                }
                                else {
                                    controladorApp.notificarMensajeSatisfactorio("Trabajador registrado correctamente");

                                    // REFRESCAR INFORMACION DEL TRABAJADOR
                                    LimpiarModalRegistroConvocatoria();

                                    $("#hdIdEmpleado").val(res.responseText);

                                    $("#_tab4").attr("data-toggle", "tab");
                                    $('#_tab4').prop("onclick", null).off("click");

                                    controlador.CargarFormularioTrabajador(res.responseText);
                                    controlador.CargarFormularioCuentasBancarias(res.responseText);

                                    modal.title("Actualizar Trabajador");
                                    $('#divGrid').data("kendoGrid").dataSource.page(1);
                                }
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }

    }

    this.ConvocatoriaJS.prototype.cerrarModalFormato = function () {
        $('#divModalFormato').data('kendoWindow').close();
    }
    this.ConvocatoriaJS.prototype.cerrarModalVisor = function () {
        $('#divModalVisor').data('kendoWindow').close();
    }
    this.ConvocatoriaJS.prototype.cerrarModalRegistroConvocatoria = function () {
        $('#divModalRegistroPracticas').data('kendoWindow').close();
    }

    this.ConvocatoriaJS.prototype.ingresarDetalleEvaluacion = function (idCad, idEvaluacion) {
        //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
        window.open(controladorApp.obtenerRutaBase() + 'EvaluacionDetalle/?IdCad=' + idCad + "&IdEvaluacion=" + idEvaluacion, '_blank');
    }
    this.ConvocatoriaJS.prototype.iniciarPostulacion = function (idConvocatoria) {
        //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
        window.open(controladorApp.obtenerRutaBase() + 'Convocatoria/Postular?id=' + $('#hdIdConvocatoria').val());
    }
    this.ConvocatoriaJS.prototype.iniciarPostulacionServir = function (e) {
        e.preventDefault();
        // RECUPERAMOS LA CONVOCATORIA Y CONSULTAMOS SI SE TRATA DE UNA CONVOCATORIA CERRADA
        var data_param = new FormData();
        data_param.append('id', $('#hdIdConvocatoria').val());

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Convocatoria/ValidarPostulacionServir',
            type: 'POST',
            dataType: 'json',
            async: false,
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                //alert(res.responseText)
                if (res.responseText.startsWith("1")) {
                    controladorApp.notificarMensajeSatisfactorio("Usted se encuentra permitido para postular a este proceso de selección");
                    setTimeout(() => {
                        //console.log('Esperando para postular ...');
                        window.open(controladorApp.obtenerRutaBase() + 'Convocatoria/PostularServir?id=' + $('#hdIdConvocatoria').val());

                        var modal = $('#divModalPostulacionServir').data('kendoWindow');
                        modal.close();
                    }, 3000);
                }
                else {
                    controladorApp.notificarMensajeDeAlerta('NO cumple con lo requerido para participar en esta convocatoria cerrada');
                }
                
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res);
            }
        });
    }
    this.ConvocatoriaJS.prototype.iniciarSesion = function () {
        //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
        window.open(controladorApp.obtenerRutaBase() + 'Postulante');
    }
    this.ConvocatoriaJS.prototype.cancelarPostulacion = function () {
        window.location = controladorApp.obtenerRutaBase() + 'Postulante';
    }
    //controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Postulante/Ingresar/?id=' + item.Contrasena + '" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file"> </span> Validar Información</a>';
    this.ConvocatoriaJS.prototype.abrirModalPostulacion = function (IdConvocatoria) {
        $('#hdIdConvocatoria').val(IdConvocatoria);

        var modal = $('#divModalPostulacion').data('kendoWindow');
        modal.title("Importante");

        modal.open();
    }
    this.ConvocatoriaJS.prototype.abrirModalPostulacionServir = function (IdConvocatoria) {
        $('#hdIdConvocatoria').val(IdConvocatoria);

        var modal = $('#divModalPostulacionServir').data('kendoWindow');
        modal.title("Importante");

        modal.open();
    }
    this.ConvocatoriaJS.prototype.cerrarModalPostulacion = function () {
        var modal = $('#divModalPostulacion').data('kendoWindow');
        modal.close();
    }
    this.ConvocatoriaJS.prototype.cerrarModalPostulacionServir = function () {
        var modal = $('#divModalPostulacionServir').data('kendoWindow');
        modal.close();
    }

    this.ConvocatoriaJS.prototype.abrirModalPracticas = function (IdConvocatoria, IdTipo) {
        $('#hdIdConvocatoria').val(IdConvocatoria);
        debugger;
        if (IdTipo == '2')
            $('#lblCarta').html('Certificado o constancia de egresado');

        var modal = $('#divModalRegistroPracticas').data('kendoWindow');
        modal.title("Registro de Postulación");

        LimpiarModalRegistroPractica();
        modal.center().open();
    }
    this.ConvocatoriaJS.prototype.abrirModalVisor = function (idDetalle, idTipo) {
        var modal = $('#divModalVisor').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Ver documento");

        debugger;
        //var options = {
        //    width: "80rem",
        //    height: "100rem"
        //};
        var data_param = new FormData();
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdDetalle', idDetalle);
        data_param.append('IdTipo', idTipo);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivoPostulacion',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                if (res.success == "True") {
                    PDFObject.embed(res.responseText, "#app");
                }

                modal.center().open();
            },
            error: function (res) {

            }
        });

        //initPDFViewer("https://cdn.www.gob.pe/uploads/document/file/575243/RD_050-2020-MIDISP65-DE.pdf");
        //initPDFViewer("http://localhost:36205/temp/RM_076_2020MIDIS.pdf"); //575243/RD_050-2020-MIDISP65-DE
        //debugger;
        //modal.open();
    }
    this.ConvocatoriaJS.prototype.cerrarModalEliminacion = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        $('#hdnUid').val('');
        modal.close();
    }

    this.ConvocatoriaJS.prototype.eliminar = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        var grilla = $('#divGrid').data("kendoGrid");
        var dr = grilla.dataSource.getByUid($('#hdnUid').val());

        var data_param = new FormData();
        data_param.append('IdCad', dr.IdCad);
        data_param.append('IdPropuesta', dr.IdPropuesta);
        
        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Propuesta/Eliminar',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                controladorApp.notificarMensajeSatisfactorio("Registro eliminado correctamente");
                grilla.dataSource.page(1);
                modal.close();
            },
            error: function (res) {

            }
        });
    }
    this.ConvocatoriaJS.prototype.enviarPostulacion = function (e) {
        e.preventDefault();

        var data_param = new FormData();

        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());
        data_param.append('Estado', 1);

        var existeFile = false;
        var upload1 = $("#fileAnexo").getKendoUpload();
        var file1 = upload1.getFiles();
        if (file1.length == 0) {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar el Anexo 06 firmado (Formato PDF)');
            return false;
        } else {
            if (file1.length == 0) {
                data_param.append('formatos[0]', null);
            }
            else {
                if (file1[0].extension.toLowerCase() == '.pdf') {
                    existeFile = true;
                    data_param.append('formatos[0]', file1[0].rawFile);
                }
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }
        var upload2 = $("#fileHojaVida").getKendoUpload();
        var file2 = upload2.getFiles();
        if (file2.length == 0) {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar su hoja de vida firmada (Formato PDF)');
            return false;
        } else {
            if (file2.length == 0) {
                data_param.append('formatos[1]', null);
            }
            else {
                if (file2[0].extension.toLowerCase() == '.pdf') {
                    existeFile = true;
                    data_param.append('formatos[1]', file2[0].rawFile);
                }
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }
        
        controladorApp.abrirMensajeDeConfirmacion(
                '<span>¿Está seguro de enviar su postulación?.</span></br></br><div class="alert alert-warning" style="padding: 5px; border: 1px solid #f7cf0aa0;">Recuerde que al confirmar su postulación quedará registrada y NO SE PODRÁ REALIZAR NINGÚN CAMBIO</div>', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/ActualizarRegistroPostulacion',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Postulación registrada correctamente");

                                // REFRESCAR INFORMACION DEL TRABAJADOR
                                $("#hdIdConvocatoria").val(res.responseText);

                                window.location = controladorApp.obtenerRutaBase() + 'Postulante/Listar';
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);

        //if (frmPersonaValidador.validate()) {
        //    var modal = $('#divModalRegistroConvocatoria').data('kendoWindow');
        //    var esValido = true;
        //    var mensajeValidacion = '';

        //}
        //else {
        //    controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        //}
    }
    this.ConvocatoriaJS.prototype.enviarPostulacionServir = function (e) {
        e.preventDefault();

        var data_param = new FormData();

        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());
        data_param.append('Estado', 1);

        var existeFile = false;
        var upload1 = $("#fileAnexo").getKendoUpload();
        var file1 = upload1.getFiles();
        if (file1.length == 0) {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar el Anexo 06 firmado (Formato PDF)');
            return false;
        } else {
            if (file1.length == 0) {
                data_param.append('formatos[0]', null);
            }
            else {
                if (file1[0].extension.toLowerCase() == '.pdf') {
                    existeFile = true;
                    data_param.append('formatos[0]', file1[0].rawFile);
                }
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }
        var upload2 = $("#fileHojaVida").getKendoUpload();
        var file2 = upload2.getFiles();
        if (file2.length == 0) {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar su hoja de vida firmada (Formato PDF)');
            return false;
        } else {
            if (file2.length == 0) {
                data_param.append('formatos[1]', null);
            }
            else {
                if (file2[0].extension.toLowerCase() == '.pdf') {
                    existeFile = true;
                    data_param.append('formatos[1]', file2[0].rawFile);
                }
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }

        controladorApp.abrirMensajeDeConfirmacion(
            '<span>¿Está seguro de enviar su postulación?.</span></br></br><div class="alert alert-warning" style="padding: 5px; border: 1px solid #f7cf0aa0;">Recuerde que al confirmar su postulación quedará registrada y NO SE PODRÁ REALIZAR NINGÚN CAMBIO</div>', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ActualizarRegistroPostulacionServir',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            controladorApp.notificarMensajeSatisfactorio("Postulación registrada correctamente");

                            // REFRESCAR INFORMACION DEL TRABAJADOR
                            $("#hdIdConvocatoria").val(res.responseText);

                            window.location = controladorApp.obtenerRutaBase() + 'Postulante/Listar';
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);

        //if (frmPersonaValidador.validate()) {
        //    var modal = $('#divModalRegistroConvocatoria').data('kendoWindow');
        //    var esValido = true;
        //    var mensajeValidacion = '';

        //}
        //else {
        //    controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        //}
    }
    /* Adicionado el 04.09.2024  */
    this.ConvocatoriaJS.prototype.buscarConvocatoriaAnterior = function () {
        var grilla = $('#divGrid6').data("kendoGrid");
        grilla.dataSource._sort = undefined;
        grilla.dataSource.page(1);
    }

}(jQuery));

