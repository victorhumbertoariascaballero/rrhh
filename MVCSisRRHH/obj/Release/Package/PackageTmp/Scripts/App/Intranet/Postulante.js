(function ($) {
    var frmPersonaValidador;
    var frmEstudioValidador;
    var frmDocumentoValidador;
    var frmCapacitacionValidador;
    var frmExperienciaValidador;
    var strMensajes = '';
    
    var myState = {
        pdf: null,
        currentPage: 1,
        zoom: 1
    }

    this.PostulanteJS = function () { };
    this.PostulanteJS.prototype.inicializar = function () {
        
        /* REGISTRO */
        $("#fileHojaVida").kendoUpload({
            multiple: false,
            //async: {
            //    saveUrl: controladorApp.obtenerRutaBase() + 'Boletas/Validar',
            //    removeUrl: controladorApp.obtenerRutaBase() + 'Boletas/Remover',
            //    autoUpload: true
            //},
            validation: {
                allowedExtensions: [".jpg"],
                maxFileSize: 1024000
            }
        });
        $("#fileSustento").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            }
        });
        $("#fileDDJJ").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 5120000
            }
        });
        //$("#fileFormato").kendoUpload({
        //    multiple: false,
        //    validation: {
        //        allowedExtensions: [".pdf"],
        //        maxFileSize: 1024000
        //    }
        //});
        
        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoBrevete").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeBrevete",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoVivienda").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoVivienda",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoPension").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDePension",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }//,
            //change: function (e) {
            //    var estado = this.value();
            //    if (estado == '1') {
            //        $("#txtPersonaNombrePension").show();
            //        $("#txtPersonaNombrePension").attr("required", true);
            //    }
            //    else {
            //        $("#txtPersonaNombrePension").hide();
            //        $("#txtPersonaNombrePension").removeAttr("required");
            //    }
            //}
        });
        $("#ddlPersonaNuevoTipoPension").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDePensionConRegimen",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaBanco").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "IdBanco",
            //optionLabel: "-- Seleccione --",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarBancos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaNuevoBanco").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarNuevoBanco",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaGrupoSan").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarGrupoSanguineo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        
        $("#ddlPersonaSede").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstado").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstado",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            },
            change: function (e) {
                debugger;
                var estado = this.value();
                //if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO)
                //{
                    if (estado == '1') {
                        //$("#divCese").hide();
                        $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                        $("#txtPersonaFechaFinLabores").removeAttr("required");
                        $("#txtPersonaFechaFinLabores").val('');
                    }
                    if (estado == '0') {
                        //$("#divCese").show();
                        $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                        $("#txtPersonaFechaFinLabores").attr("required", true);
                        $("#txtPersonaFechaFinLabores").attr("validationmessage", "requerido");
                    }
                //}
            }
        });
        $("#ddlPersonaDependencia").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlPersonaTipoPension").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDePension",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }//,
            //change: function (e) {
            //    var estado = this.value();
            //    if (estado == '1') {
            //        $("#txtPersonaNombrePension").show();
            //        $("#txtPersonaNombrePension").attr("required", true);
            //    }
            //    else {
            //        $("#txtPersonaNombrePension").hide();
            //        $("#txtPersonaNombrePension").removeAttr("required");
            //    }
            //}
        });
        $("#ddlPersonaTipoAFP").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeAFP",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoComisionAFP").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeComisionAFP",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        frmPersonaValidador = $("#frmRegistroPostulante").kendoValidator().data("kendoValidator");

        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        
        debugger;
        
        $("#tienepension1").prop("checked", true);
        $("#tienebanco1").prop("checked", true);
        $("#ddlPersonaTipoPension").data("kendoDropDownList").value(1);

        $("#tienealergias2").prop("checked", true);
        $("#tieneenfermedad2").prop("checked", true);
        $("#tienemedicamento2").prop("checked", true);
        $("#tienediscapacidad2").prop("checked", true);

        $("#declaraIncompatibilidad2").prop("checked", true);
        $("#declaraNepotismo1").prop("checked", true);
        $("#declaraNormas1").prop("checked", true);
        $("#declaraInteres1").prop("checked", true);

        this.inicializarGridNotificaciones();
        this.CargarFormularioPostulante();
        this.CargarFormularioFamiliares(-1);

        OnChangeTienePension();
        OnChangeTieneBanco();
        OnChangeAlergias();
        OnChangeEnfermedades();
        OnChangeMedicamento();
        OnChangeDiscapacidad();
        OnChangeFuerzasArmadas();
        OnChangeDeclaraNepotismo();
        OnChangeTieneAFP();
    };
    this.PostulanteJS.prototype.inicializarPostulante = function () {

        ///* REGISTRO */
        $("#fileEstudio").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            }
        });
        $("#fileCapacitacion").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            }
        });
        $("#fileExperiencia").kendoUpload({
            multiple: false,
            //async: {
            //    saveUrl: controladorApp.obtenerRutaBase() + 'Boletas/Validar',
            //    removeUrl: controladorApp.obtenerRutaBase() + 'Boletas/Remover',
            //    autoUpload: true
            //},
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            }
        });
        $("#fileDocumento").kendoUpload({
            multiple: false,
            validation: {
                allowedExtensions: [".pdf"],
                maxFileSize: 1024000
            }
        });

        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelEstudio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "strDescripcion",
            dataValueField: "iCodGrado",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Perfiles/ListarGradosTodos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelLogradoEstudio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "strDescripcion",
            dataValueField: "iCodNivel",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Perfiles/ListarNivelEducativo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlAnioEstudio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarAnioPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMesEstudio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarMesPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlAnioFechaInicio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarAnioPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMesFechaInicio").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarMesPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlAnioFechaFin").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarAnioPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMesFechaFin").kendoDropDownList({
            autoBind: true,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarMesPostulacion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlTipoDocumento").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlTipoMateria").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoMateria",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            },
            change: function (e) {
                OnChangeTipoMateria();
            }
        });
        $("#ddlTipoMateriaOtros").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            enable: false,
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoMateriaOtros",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlNivelTipoMateriaOtros").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            enable: false,
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarNivelMateria",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlSector").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarSector",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            },
            change: function (e) {
                OnChangeSector();
            }
        });
        $("#ddlRegimen").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarRegimen",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        $('#divModalDocumento').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '60%',
            height: 'auto',
            title: 'Agregar documento de sustento',
            visible: false,
            position: { top: '20%', left: "20%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");
        $('#divModalEstudio').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Agregar estudio ',
            visible: false,
            position: { top: '10%', left: "10%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");
        $('#divModalCapacitacion').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Agregar capacitación',
            visible: false,
            position: { top: '10%', left: "10%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");
        $('#divModalExperiencia').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Agregar experiencia laboral',
            visible: false,
            position: { top: '10%', left: "10%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");
        $('#divModalVisor').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '98%',
            height: 'auto',
            title: 'Ver documento',
            visible: false,
            position: { top: '1%', left: "1%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");

        frmPersonaValidador = $("#frmRegistroPostulante").kendoValidator().data("kendoValidator");
        frmEstudioValidador = $("#frmNuevoEstudio").kendoValidator().data("kendoValidator");
        frmDocumentoValidador = $("#frmNuevoDocumento").kendoValidator().data("kendoValidator");
        frmCapacitacionValidador = $("#frmNuevoCapacitacion").kendoValidator().data("kendoValidator");
        frmExperienciaValidador = $("#frmNuevoExperiencia").kendoValidator().data("kendoValidator");
        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });

        $("#txtExperienciaInicio").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#txtExperienciaFin").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#txtCapacitacionInicio").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#txtCapacitacionFin").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });

        $('#chkColegiatura').change(OnChangeColegiatura);
        $('#chkPersonaDiscapacidad').change(OnChangeDiscapacidad);
        $('#chkPersonaDeportista').change(OnChangeDeportistaAltoRendimiento);
        $('#chkTrabajarInterior').change(OnChangeTrabajarInterior);
        $('#chkPersonaFFAA').change(OnChangeFuerzasArmadas);
        //$('#ddlSector').change(OnChangeSector)

        this.CargarPostulante();
        //this.CargarFormularioFamiliares(-1);
        //this.CargarGridExperiencia();
        //this.CargarGridCapacitacion(-1);
    };

    this.PostulanteJS.prototype.inicializarGridPostulaciones = function (idPostulante) {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulaciones',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.IdPostulante = idPostulante;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdPostulantePostulacion"
                }
            }
        });


        this.$grid = $("#divGridPostulaciones").kendoGrid({
            toolbar: ["excel"], //{ template: kendo.template('<div>Esto es una prueba ' + this.$dataSource.total() + ' </div>') }
            excel: {
                fileName: "Listado de Postulaciones.xlsx",
                filterable: false
            },
            dataSource: this.$dataSource,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,

            dataType: 'json',
            columns: [
                {
                    field: "Nro",
                    title: "Nro",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "NombreCompleto",
                    title: "POSTULANTE",
                    width: "300px"
                },
                {
                    field: "NombreProceso",
                    title: "Nro PROCESO",
                    width: "50px"
                },
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "300px"
                },
                {
                    field: "FechaPostulacion",
                    title: "FECHA DE POSTULACIÓN",
                    attributes: { style: "text-align:center;" },
                    width: "40px"
                },
                {
                    field: "AptoCurricular",
                    title: "ETAPA CURRICULAR",
                    attributes: { style: "text-align:center;" },
                    width: "40px",
                    template: function (item) {
                        var controles = "";
                        if (item.AptoCurricular == 1) {
                            controles += '<span class="k-icon k-i-check k-i-checkmark" style="color: #00b5dc;"></span>';
                        }

                        return controles;
                    }
                },
                {
                    field: "AptoResultado",
                    title: "RESULTADO FINAL",
                    attributes: { style: "text-align:center;" },
                    width: "40px",
                    template: function (item) {
                        var controles = "";
                        if (item.AptoResultado == 1) {
                            controles += '<span class="k-icon k-i-check k-i-checkmark" style="color: #00b5dc;"></span>';
                        }

                        return controles;
                    }
                },
                {
                    field: "AptoContrato",
                    title: " ",
                    attributes: { style: "text-align:center;" },
                    width: "40px",
                    template: function (item) {
                        var controles = "";
                        if (item.AptoContrato == 1) {
                            //controles += '<button type="button" class="btn btn-default btn-xs" onclick="controlador.abrirModalEnvioMail(\'' + item.Contrasena + '\')">';
                            //controles += '<span class="glyphicon glyphicon-envelope" aria-hidden="true" data-uib-tooltip="Editar"></span>';
                            //controles += '</button>';
                            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Postulante/Ingresar/?id=' + item.Contrasena + '" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-check"> </span> SUSCRIBIR CONTRATO LABORAL</a>';
                        }

                        return controles;
                    }
                }
                //{
                //    //INGRESAR DETALLE DE LA EVALUACION
                //    title: '',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        if (item.Contrasena != '')
                //            controles += '<a href="' + controladorApp.obtenerRutaBase() + 'Boletas/DescargarArchivo/?id=' + item.Contrasena + '" target="_blank" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-file"></span></a>';

                //        return controles;
                //    },
                //    width: '30px'
                //}
            ]
        }).data();

        debugger;
    };

    this.PostulanteJS.prototype.inicializarGridNotificaciones = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarNotificaciones',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.IdPostulante = $("#hdIdPostulante").val();
                        data_param.IdPostulacion = $("#hdIdPostulacion").val();
                        data_param.IdConvocatoria = $("#hdIdConvocatoria").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdNotificacion"
                }
            }
        });


        this.$grid = $("#divGridNotificacion").kendoGrid({
            toolbar: ["excel"], 
            excel: {
                fileName: "Listado de Notificaciones.xlsx",
                filterable: false
            },
            dataSource: this.$dataSource,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,

            dataType: 'json',
            columns: [
                {
                    field: "FechaRegistro",
                    title: "FECHA",
                    attributes: { style: "text-align:center;" },
                    width: "80px"
                },
                {
                    field: "Descripcion",
                    title: "DETALLE",
                    width: "500px"
                },
                {
                    field: "IdUsuarioRegistro",
                    title: "USUARIO",
                    attributes: { style: "text-align:center;" },
                    width: "50px"
                }
            ]
        }).data();

        debugger;
    };

    function LimpiarModalRegistroPersona() {
        var today = new Date();
        var month = today.getMonth();
        if (today.getMonth() > 1) var month = month - 1;
        var year = today.getFullYear();
        $("#txtPersonaFechaNacimiento").kendoDatePicker().data("kendoDatePicker").max(new Date());
        $("#txtPersonaFechaInicioLabores").kendoDatePicker().data("kendoDatePicker").max(new Date());
        //$("#txtPersonaFechaFinLabores").kendoDatePicker().data("kendoDatePicker").max(new Date());
        document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,');

        $("#hdIdEmpleado").val(0);
        $("#txtPersonaNumeroDeDocumento").val('');
        $("#txtPersonaNombres").val('');
        $("#txtPersonaApellidoPaterno").val('');
        $("#txtPersonaApellidoMaterno").val('');
        $("#ddlPersonaSexo").data("kendoDropDownList").value(0);
        $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(0);
        $("#ddlPersonaGrupoSan").data("kendoDropDownList").value(0);
        $("#txtPersonaUbigeo").val('');
        $("#txtPersonaDireccionDomicilio").val('');
        $("#ddlPersonaCondicion").data("kendoDropDownList").value(0);
        $("#ddlPersonaSede").data("kendoDropDownList").value(0);
        $("#txtRUC").val('');
        $("#txtPersonaFechaNacimiento").val('');
        $("#txtPersonaFechaInicioLabores").val('');
        $("#txtPersonaFechaFinLabores").val('');
        $("#ddlPersonaDependencia").data("kendoDropDownList").value(0);
        $("#txtPersonaCargo").val('');
        $("#txtPersonaCorreoElectronicoP").val('');
        $("#txtTelefonoCelularP").val('');
        $("#txtTelefonoFijoP").val('');
        $("#txtPersonaCorreoElectronico").val('');
        $("#txtTelefonoCelular").val('');
        $("#txtTelefonoLaboral").val('');
        $("#txtTelefonoAnexo").val('');
        $('#hdFoto').val('');
        $("#hdIdUbigeoEmpleado").val('');

        $("#divExiste").hide();
        $("#btnBuscarFoto").hide();

    }
    this.PostulanteJS.prototype.CargarPostulante = function () {
        var data_param = new FormData();
        data_param.append('NroDocumento', $("#hdnNroDocumento").val());
        
        $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").readonly();
        
        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulante',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;

                $("#hdIdPostulante").val(res.IdPostulante);
                $("#txtPersonaNacionalidad").val(res.Nacionalidad);
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtPersonaNumeroDeDocumento").val(res.NroDocumento);
                $("#txtPersonaNombres").val(res.Nombre);
                $("#txtPersonaApellidoPaterno").val(res.Paterno);
                $("#txtPersonaApellidoMaterno").val(res.Materno);
                $("#ddlPersonaSexo").data("kendoDropDownList").value(res.Sexo);
                $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(res.IdEstadoCivil);
                $("#txtPersonaDireccionDomicilio").val(res.Domicilio);
                $("#txtPersonaUbigeo").val(res.DescripcionUbigeo);
                $("#txtPersonaFechaNacimiento").data("kendoDatePicker").value(res.FechaNacimiento);
                $("#txtTelefonoCelularP").val(res.Celular);
                $("#txtTelefonoFijoP").val(res.Telefono);
                $("#txtPersonaCorreoElectronicoP").val(res.CorreoElectronico);
                $("#txtPersonaRUC").val(res.RUC);
                $("#txtPersonaColegio").val(res.Colegio);
                $("#txtPersonaColegioNro").val(res.NroColegiatura);
                $("#hdIdUbigeoPostulante").val(res.Ubigeo);
                $('#chkColegiatura').prop("checked", res.bColigiatura);
                $('#chkColegiaturaHabilitada').prop("checked", res.bColegiatura_Habilitada);
                $('#txtCarnetConadis').val(res.CarnetConadis);
                $('#txtDocumentoPersonaDeportista').val(res.Acreditacion);
                $('#chkTrabajarInterior').prop("checked", res.bTrabajarProvincia);
                $('#txtTrabajarProvincia').val(res.Trabajar_Provincia);
                $('#txtDomicilioActual').val(res.DireccionActual);
                $('#txtReferencia').val(res.Referencia);
                $('#txtDocumentoFuerzasArmadas').val(res.CarnetFuerzasArmadas);

                if (res.FFAA == 1)
                    $("#chkPersonaFFAA").prop("checked", true);
                if (res.Discapacidad == 1)
                    $("#chkPersonaDiscapacidad").prop("checked", true);
                if (res.Deportista == 1)
                    $("#chkPersonaDeportista").prop("checked", true);

                if (res.Foto != '') {
                    document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,' + res.Foto);
                    $('#hdFoto').val(res.Foto);
                }

                if (res.TipoDocumento == 1) {   //EN CASO DE TIPO DE DOCUMENTO DNI
                    $('#ddlPersonaSexo').data("kendoDropDownList").readonly();
                    $('#ddlPersonaEstadoCivil').data("kendoDropDownList").readonly();
                    $('#txtPersonaFechaNacimiento').data("kendoDatePicker").readonly();
                }

                controlador.CargarGridDocumento();
                controlador.CargarGridEstudio();
                controlador.CargarGridCapacitacion();
                controlador.CargarGridExperiencia(); //res.IdPostulante

                OnChangeColegiatura();
                OnChangeDiscapacidad();
                OnChangeFuerzasArmadas();
                OnChangeDeportistaAltoRendimiento();
                OnChangeTrabajarInterior();
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res.responseText);
            }
        });
    }

    this.PostulanteJS.prototype.CargarFormularioPostulante = function () {
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());

        $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").readonly();
        $('#ddlPersonaSexo').data("kendoDropDownList").readonly();
        $('#ddlPersonaEstadoCivil').data("kendoDropDownList").readonly();
        $('#txtPersonaFechaNacimiento').data("kendoDatePicker").readonly();

        $("#fileHojaVida").data("kendoUpload").clearAllFiles();
        $("#fileSustento").data("kendoUpload").clearAllFiles();
        $("#fileDDJJ").data("kendoUpload").clearAllFiles();
        //$("#fileFormato").data("kendoUpload").clearAllFiles();

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulanteFicha',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;
                
                $("#txtPersonaNacionalidad").val(res.Nacionalidad);
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtPersonaNumeroDeDocumento").val(res.NroDocumento);
                $("#txtPersonaNombres").val(res.Nombre);
                $("#txtPersonaApellidoPaterno").val(res.Paterno);
                $("#txtPersonaApellidoMaterno").val(res.Materno);
                $("#ddlPersonaSexo").data("kendoDropDownList").value(res.Sexo);
                $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(res.IdEstadoCivil);
                $("#txtPersonaDireccionDomicilio").val(res.Domicilio);
                $("#txtPersonaUbigeo").val(res.DescripcionUbigeo);
                $("#txtPersonaFechaNacimiento").data("kendoDatePicker").value(res.FechaNacimiento);
                $("#txtPersonaLugarNacimiento").val(res.LugarNacimiento);
                $("#txtPersonaEdad").val(res.Edad);
                $("#ddlPersonaTipoVivienda").data("kendoDropDownList").value(res.IdTipoVivienda);
                $("#ddlPersonaTipoBrevete").data("kendoDropDownList").value(res.IdTipoBrevete);
                $("#txtPersonaNroLicencia").val(res.NroLicencia);
                $("#txtTelefonoCelularP").val(res.Celular);
                $("#txtTelefonoFijoP").val(res.Telefono);
                $("#txtPersonaCorreoElectronicoP").val(res.CorreoElectronico);
                $("#txtPersonaRUC").val(res.RUC);
                $("#txtTelefonoEmergencia1").val(res.TelefonoEmergencia1);
                $("#txtTelefonoEmergencia2").val(res.TelefonoEmergencia2);
                $("#txtNombreEmergencia1").val(res.ContactoEmergencia1);
                $("#txtNombreEmergencia2").val(res.ContactoEmergencia2);
                $("#hdIdUbigeoPostulante").val(res.Ubigeo);

                if (res.IdEstaAfiliadoPensiones == 1) 
                    $("#tienepension1").prop("checked", true);
                else
                    $("#tienepension2").prop("checked", true);
                if (res.IdAFPAfiliada > 0) $("#ddlPersonaTipoPension").data("kendoDropDownList").value(res.IdAFPAfiliada);
                $("#txtPersonaCodigoAfp").val(res.CodigoAFP);
                if (res.IdAFPAfiliada > 0) $("#ddlPersonaTipoPension").data("kendoDropDownList").value(res.IdAFPAfiliada);
                if (res.IdTipoAFP > 0) $("#ddlPersonaTipoAFP").data("kendoDropDownList").value(res.IdTipoAFP);
                if (res.IdTipoComisionAFP > 0) $("#ddlPersonaTipoComisionAFP").data("kendoDropDownList").value(res.IdTipoComisionAFP);
                if (res.IdTipoPensionDeseaAfiliar > 0) $("#ddlPersonaNuevoTipoPension").data("kendoDropDownList").value(res.IdTipoPensionDeseaAfiliar);

                if (res.IdEstaAfiliadoBanco == 1)
                    $("#tienebanco1").prop("checked", true);
                else
                    $("#tienebanco2").prop("checked", true);
                if (res.IdBancoAfiliado > 0) $("#ddlPersonaBanco").data("kendoDropDownList").value(res.IdBancoAfiliado);
                $("#txtPersonaBanco").val(res.CuentaBancoAfiliado);
                $("#txtPersonaBancoCCI").val(res.CuentaBancoCCIAfiliado);
                if (res.IdBancoDeseaAfiliar > 0) $("#ddlPersonaNuevoBanco").data("kendoDropDownList").value(res.IdBancoDeseaAfiliar);

                if (res.IdSituacionAcademicaS == 1) $("#situacionAcademicaS").prop("checked", true);
                if (res.IdSituacionAcademicaT == 1) $("#situacionAcademicaT").prop("checked", true);
                if (res.IdSituacionAcademicaU == 1) $("#situacionAcademicaU").prop("checked", true);
                if (res.IdSituacionAcademicaO == 1) $("#situacionAcademicaO").prop("checked", true);
                if (res.IdCentroEstudiosPU == 1) $("#centroestudiosPU").prop("checked", true);
                if (res.IdCentroEstudiosPR == 1) $("#centroestudiosPR").prop("checked", true);
                if (res.IdCentroEstudiosEX == 1) $("#centroestudiosEX").prop("checked", true);

                $("#txtsituacionAcademicaT").val(res.SituacionAcademicaT);
                $("#txtsituacionAcademicaU").val(res.SituacionAcademicaU);
                $("#txtsituacionAcademicaO").val(res.SituacionAcademicaO);
                $("#txtcentroestudiosPU").val(res.CentroEstudiosPU);
                $("#txtcentroestudiosPR").val(res.CentroEstudiosPR);
                $("#txtcentroestudiosEX").val(res.CentroEstudiosEX);

                if (res.IdGradoAcademicoES == 1) $("#gradoacademicoES").prop("checked", true);
                if (res.IdGradoAcademicoEG == 1) $("#gradoacademicoEG").prop("checked", true);
                if (res.IdGradoAcademicoBA == 1) $("#gradoacademicoBA").prop("checked", true);
                if (res.IdGradoAcademicoTI == 1) $("#gradoacademicoTI").prop("checked", true);
                if (res.IdPostgradoM == 1) $("#postgradoM").prop("checked", true);
                if (res.IdPostgradoD == 1) $("#postgradoD").prop("checked", true);
                if (res.IdPostgradoO == 1) $("#postgradoO").prop("checked", true);
                $("#txtpostgradoO").val(res.PostgradoO);
                $("#txtpostgradoCE").val(res.PostgradoCE);
                $("#txtpostgradoGrado").val(res.PostgradoGrado);

                if (res.IdPresentaAlergias == 1)
                    $("#tienealergias1").prop("checked", true);
                else
                    $("#tienealergias2").prop("checked", true);
                if (res.IdPresentaAlergias1 == 1) $("#alergia1").prop("checked", true);
                if (res.IdPresentaAlergias2 == 1) $("#alergia2").prop("checked", true);
                $("#txtAlergiaOtro").val(res.PresentaAlergiasOtro);

                if (res.IdPresentaEnfermedades == 1)
                    $("#tieneenfermedad1").prop("checked", true);
                else
                    $("#tieneenfermedad2").prop("checked", true);
                if (res.IdPresentaEnfermedadesD == 1) $("#tieneenefermedadD").prop("checked", true);
                if (res.IdPresentaEnfermedadesH == 1) $("#tieneenefermedadH").prop("checked", true);
                if (res.IdPresentaEnfermedadesA == 1) $("#tieneenefermedadA").prop("checked", true);
                if (res.IdPresentaEnfermedadesE == 1) $("#tieneenefermedadE").prop("checked", true);
                $("#txtEnfermedadOtro").val(res.PresentaEnfermedadesOtro);

                if (res.IdConsumeMedicamentos == 1)
                    $("#tienemedicamento1").prop("checked", true);
                else
                    $("#tienemedicamento2").prop("checked", true);
                $("#txtMedicamentoOtro").val(res.ConsumeMedicamentosOtro);
                $("#ddlPersonaGrupoSan").data("kendoDropDownList").value(res.IdGrupoSanguineo);
                $("#txtSaludAdicional").val(res.InformacionAdicionalSalud);

                if (res.IdPresentaDiscapacidad == 1)
                    $("#tienediscapacidad1").prop("checked", true);
                else
                    $("#tienediscapacidad2").prop("checked", true);
                if (res.IdPresentaDiscapacidadA == 1) $("#tienediscapacidadA").prop("checked", true);
                if (res.IdPresentaDiscapacidadV == 1) $("#tienediscapacidadV").prop("checked", true);
                if (res.IdPresentaDiscapacidadH == 1) $("#tienediscapacidadH").prop("checked", true);
                if (res.IdPresentaDiscapacidadC == 1) $("#tienediscapacidadC").prop("checked", true);
                if (res.IdPresentaDiscapacidadF == 1) $("#tienediscapacidadF").prop("checked", true);
                $("#txtDiscapacidadConducta").val(res.PresentaDiscapacidadC);
                $("#txtDiscapacidadFisica").val(res.PresentaDiscapacidadF);

                if (res.IdCertificadoDiscapacidad == 1)
                    $("#tienecertdiscapacidad1").prop("checked", true);
                else
                    $("#tienecertdiscapacidad2").prop("checked", true);

                if (res.IdDeclaraIncompatibilidad == 1)
                    $("#declaraIncompatibilidad1").prop("checked", true);
                else
                    $("#declaraIncompatibilidad2").prop("checked", true);
                if (res.IdDeclaraNepotismo == 0) {
                    $("#declaraNepotismo2").prop("checked", true);
                    $("#txtNepotismoRel1").val(res.NepotismoRel1);
                    $("#txtNepotismoRel2").val(res.NepotismoRel2);
                    $("#txtNepotismoRel3").val(res.NepotismoRel3);
                    $("#txtNepotismoApe1").val(res.NepotismoApe1);
                    $("#txtNepotismoApe2").val(res.NepotismoApe2);
                    $("#txtNepotismoApe3").val(res.NepotismoApe3);
                    $("#txtNepotismoNom1").val(res.NepotismoNom1);
                    $("#txtNepotismoNom2").val(res.NepotismoNom2);
                    $("#txtNepotismoNom3").val(res.NepotismoNom3);
                    $("#txtNepotismoAre1").val(res.NepotismoAre1);
                    $("#txtNepotismoAre2").val(res.NepotismoAre2);
                    $("#txtNepotismoAre3").val(res.NepotismoAre3);
                }
                else 
                    $("#declaraNepotismo1").prop("checked", true);

                if (res.IdDeclaraNormas == 1)
                    $("#declaraNormas1").prop("checked", true);
                else
                    $("#declaraNormas2").prop("checked", true);

                if (res.IdDeclaraInteres == 1)
                    $("#declaraInteres1").prop("checked", true);
                else
                    $("#declaraInteres2").prop("checked", true);

                if (res.Foto != '') {
                    document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,' + res.Foto);
                    $('#hdFoto').val(res.Foto);
                }

                //alert(res.IdEstadoRegistro)
                if (res.IdEstadoRegistro == 1) {
                    $("#step1").removeAttr("disabled");
                    $("#step1").removeClass("btn-default").addClass("btn-info");
                    $("#liTab4").hide();
                } 
                if (res.IdEstadoRegistro == 2) {
                    $("#step2").removeAttr("disabled");
                    $("#step1").removeClass("btn-info");
                    $("#step2").removeClass("btn-default").addClass("btn-info");
                    $("#liTab4").hide();
                } 
                if (res.IdEstadoRegistro == 3) {
                    $("#step3").removeAttr("disabled");
                    $("#step2").removeClass("btn-info");
                    $("#step3").removeClass("btn-default").addClass("btn-info");

                    $("#btnGuardar").hide();
                    $("#btnEnviar").hide();
                    $("#btnAprobar").hide();
                    $("#liTab4").hide();
                }
                if (res.IdEstadoRegistro == 31) {
                    $("#step3").removeAttr("disabled");
                    $("#step2").removeClass("btn-info");
                    $("#step3").removeClass("btn-default").addClass("btn-info");

                    $("#btnActualContrato").show();
                    $("#btnActualContrato").attr("href", controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivo/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria + '&idTipo=9');
                    $("#btnGuardar").hide();
                    $("#btnEnviar").hide();                    
                }
                if (res.IdEstadoRegistro == 4) {
                    $("#step4").removeAttr("disabled");
                    $("#step3").removeClass("btn-info");
                    $("#step4").removeClass("btn-default").addClass("btn-info");

                    $("#btnGuardar").hide();
                    $("#btnEnviar").hide();
                    $("#btnAprobar").hide();
                    $("#liTab2").hide();
                    $("#liTab3").hide();
                    $("#liTab4").hide();
                }

                var newUrl = '';
                if (res.IdTieneHojaVida == 1) {
                    $("#IdTieneHojaVida").val("1");
                    newUrl = controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivo/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria + '&idTipo=1'
                    $("#btnActualHojaVida").attr("href", newUrl);
                    $("#btnActualHojaVida").show();
                    //$("#btnActualHojaVida").click(function () { alert('Hoja de vida'); });
                }
                if (res.IdTieneSustento == 1) {
                    $("#IdTieneSustento").val("1");
                    newUrl = controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivo/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria + '&idTipo=2'
                    $("#btnActualSustento").attr("href", newUrl);
                    $("#btnActualSustento").show();
                }
                if (res.IdTieneDDJJ == 1) {
                    $("#IdTieneDDJJ").val("1");
                    newUrl = controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivo/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria + '&idTipo=3'
                    $("#btnActualDDJJ").attr("href", newUrl);
                    $("#btnActualDDJJ").show();
                }
                //if (res.IdTieneFormato == 1) {
                //    $("#IdTieneFormato").val("1");
                //    newUrl = controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivo/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria + '&idTipo=4'
                //    $("#btnActualFormato").attr("href", newUrl);
                //    $("#btnActualFormato").show();
                //}
                
                if(res.TotalAlertas > 0) $("#alertas").show();

                OnChangeTienePension();
                OnChangeTieneBanco();
                OnChangeAlergias();
                OnChangeEnfermedades();
                OnChangeMedicamento();
                OnChangeDiscapacidad();
                OnChangeDeclaraNepotismo();
                OnChangeTieneAFP();
                OnChangeFuerzasArmadas();
                OnChangeDeportistaAltoRendimiento();
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res.responseText);
            }
        });
    }

    this.PostulanteJS.prototype.CargarFormularioFamiliares = function (id) {
        this.$dataSourceFamiliares = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulanteFamiliares', 
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Postulante/ActualizarPostulanteFamiliar",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Postulante/EliminarPostulanteFamiliar",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Postulante/RegistrarPostulanteFamiliar",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.IdPostulante = $("#hdIdPostulante").val();
                            data_param.IdPostulacion = $("#hdIdPostulacion").val();
                            data_param.IdConvocatoria = $("#hdIdConvocatoria").val();
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            //if (frmCompromiso.validate()) {                                
                            data_param.IdPostulante = $("#hdIdPostulante").val();
                            data_param.IdPostulacion = $("#hdIdPostulacion").val();
                            data_param.IdConvocatoria = $("#hdIdConvocatoria").val();
                            data_param.Nombre = $options.Nombre.toUpperCase(); //Banco.IdBanco;
                            data_param.IdParentesco = $options.Parentesco.Codigo;
                            data_param.NroDocumento = $options.NroDocumento;
                            data_param.Edad = $options.Edad;
                            data_param.FechaNacimiento = kendo.toString(kendo.parseDate($options.FechaNacimiento), 'dd/MM/yyyy');
                            data_param.Ocupacion = $options.Ocupacion;
                            data_param.IdSexo = $options.Sexo.Nombre;
                            //}
                            break;
                        case "update":
                            data_param.IdPostulante = $("#hdIdPostulante").val();
                            data_param.IdPostulacion = $("#hdIdPostulacion").val();
                            data_param.IdConvocatoria = $("#hdIdConvocatoria").val();
                            data_param.IdFamiliar = $options.IdFamiliar;
                            data_param.Nombre = $options.Nombre.toUpperCase(); //Banco.IdBanco;
                            data_param.IdParentesco = $options.Parentesco.Codigo;
                            data_param.NroDocumento = $options.NroDocumento;
                            data_param.Edad = $options.Edad;
                            data_param.FechaNacimiento = kendo.toString(kendo.parseDate($options.FechaNacimiento), 'dd/MM/yyyy');
                            data_param.Ocupacion = $options.Ocupacion;
                            data_param.IdSexo = $options.Sexo.Nombre;
                            break;
                        case "destroy":
                            data_param.IdPostulante = $("#hdIdPostulante").val();
                            data_param.IdPostulacion = $("#hdIdPostulacion").val();
                            data_param.IdConvocatoria = $("#hdIdConvocatoria").val();
                            data_param.IdFamiliar = $options.IdFamiliar;
                            break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                switch (e.type) {
                    case "create": case "update": case "destroy":
                        var grilla = $('#divGridFamiliares').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdFamiliar",
                    fields: {
                        IdFamiliar: { editable: false, nullable: true },
                        Nombre: { validation: { required: true, style: "text-transform: uppercase" } }, //, class: "text-uppercase"
                        Edad: { validation: { required: true, type: "number" } },
                        DNI: { validation: { required: true, maxlength: "8" } },
                        Parentesco: {
                            validation: {
                                required: true,
                                parentescovalidation: function (input) {
                                    if (input.is("[name='Parentesco']") && input.val() == "") {
                                        input.attr("data-parentescovalidation-msg", "Parentesco es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ESPOSO (A)" }
                        },
                        FechaNacimiento: {
                            validation: {
                                required: true,
                                type: "date",
                                format: "dd/MM/yyyy",
                                fechanacimientovalidation: function (input) {
                                    if (input.is("[name='FechaNacimiento']") && input.val() == "") {
                                        input.attr("data-fechanacimientovalidation-msg", "requerido");
                                        return false;
                                    }
                                    
                                    return true;
                                }
                            }
                        },
                        Sexo: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Sexo']") && input.val() == "") {
                                        input.attr("data-sexovalidation-msg", "El sexo es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 0, Nombre: "F" }
                        },
                    }
                }
            },
        });

        this.divGridFamiliares = $("#divGridFamiliares").kendoGrid({
            toolbar: ["create"], //"excel", 
            dataSource: this.$dataSourceFamiliares,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "Parentesco",
                    title: "Parentesco",
                    width: "80px",
                    editor: controlador.ParentescoDropDownEditor,
                    template: "#=Parentesco.Nombre#"
                },
                {
                    field: "Nombre",
                    title: "Apellidos y Nombres",
                    attributes: { style: "text-align:left;" },
                    width: "150px"
                },
                {
                    field: "NroDocumento",
                    title: "DNI",
                    attributes: { style: "text-align:right;" },
                    width: "70px"
                },
                {
                    field: "FechaNacimiento",
                    title: "Fec Nacimiento",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaNacimientoEditor
                },
                {
                    field: "Edad",
                    title: "Edad",
                    attributes: { style: "text-align:right;" },
                    width: "30px"
                },
                {
                    field: "Ocupacion",
                    title: "Ocupación: Trabajo/Estudios",
                    attributes: { style: "text-align:left;" },
                    width: "150px"
                },
                {
                    field: "Sexo",
                    title: "Sexo",
                    width: "20px",
                    editor: controlador.SexoDropDownEditor,
                    template: "#=Sexo.Nombre#"
                },
                { title: "Acciones", command: ["edit", "destroy"], width: "150px" },
            ],
            editable: "inline"
        }).data();

    }

    this.PostulanteJS.prototype.ParentescoDropDownEditor = function (container, options) {
        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                optionLabel: "-- Seleccione --",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Postulante/ListarParentesco",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = 0;
                            data_param.Grilla.OrdenarPor = "Nombre";
                            data_param.Grilla.OrdenarDeForma = "ASC";

                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.PostulanteJS.prototype.SexoDropDownEditor = function (container, options) {
        var data = [
                        { Nombre: "F", Codigo: "0" },
                        { Nombre: "M", Codigo: "1" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };

    this.PostulanteJS.prototype.FechaNacimientoEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width_: ' + container.width() + 'px" data-bind="value:FechaNacimiento">')
            .appendTo(container);
    };
 
    this.PostulanteJS.prototype.DescripcionTrabajoEditor = function (container, options) {
        $('<textarea required name="' + options.field + '" style="width: 98%; " />') //height:' + (container.height()) + 'px
            .appendTo(container);
    };
    this.PostulanteJS.prototype.FechaInicioEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width_: ' + container.width() + 'px" data-bind="value:FechaInicio">')
            .appendTo(container);
    };
    this.PostulanteJS.prototype.FechaFinEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width_: ' + container.width() + 'px" data-bind="value:FechaFin">')
            .appendTo(container);
    };

    this.PostulanteJS.prototype.CargarGridDocumento = function () {
        this.$dataSourceDocumento = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulanteDocumento',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulante = $("#hdIdPostulante").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdDocumento"
                }
            },
        });

        this.divGridDocumento = $("#divGridDocumento").kendoGrid({
            //toolbar: ["excel"], //"excel", 
            dataSource: this.$dataSourceDocumento,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "NombreTipoDocumento",
                    title: "TIPO DE DOCUMENTO",
                    attributes: { style: "text-align:left;" },
                    width: "200px"
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1 && item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdDocumento + '\',5)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                //{
                //    //INGRESAR DETALLE DE LA EVALUACION
                //    title: '',
                //    attributes: { style: "text-align:center;" },
                //    template: function (item) {
                //        var controles = "";
                //        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalEditarDocumento(\'' + item.IdDocumento + '\')">'; //GenerarFormatoContrato
                //        controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar documento de sustento"></span>';
                //        controles += '</button>';

                //        return controles;
                //    },
                //    width: '30px'
                //},
                {
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //var items = [item.iCodPerfil, item.iSecuencia, item.iCodGrado];
                        //controles += '<a href="perfiles/nuevo">';
                        controles += '<button type="button" class="btn btn-danger btn-xs" onclick="controlador.EliminarDocumento(\'' + item.IdDocumento + '\')">';
                        controles += '<span class="glyphicon glyphicon-remove" aria-hidden="true" data-uib-tooltip="Eliminar" title="Eliminar tipo de documento"></span>';
                        controles += '</button>';
                        //controles += '</a>';

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    }
    this.PostulanteJS.prototype.CargarGridEstudio = function () {
        this.$dataSourceEstudio = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulanteEstudio',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulante = $("#hdIdPostulante").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEstudio"
                }
            },
        });

        this.divGridEstudio = $("#divGridEstudio").kendoGrid({
            //toolbar: ["excel"], //"excel", 
            dataSource: this.$dataSourceEstudio,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "NombreGrado",
                    title: "NIVEL EDUCATIVO",
                    attributes: { style: "text-align:left;" },
                    width: "80px"
                },
                {
                    field: "NombreNivel",
                    title: "NIVEL ALCANZADO",
                    attributes: { style: "text-align:left;" },
                    width: "100px"
                },
                {
                    field: "Especialidad",
                    title: "ESPECIALIDAD",
                    attributes: { style: "text-align:left;" },
                    width: "150px"
                },
                {
                    field: "Institucion",
                    title: "INSTITUCIÓN",
                    attributes: { style: "text-align:left;" },
                    width: "150px"
                },
                {
                    field: "Ciudad",
                    title: "CIUDAD / PAÍS",
                    width: "100px",
                    attributes: { style: "text-align:left;" }
                },
                {
                    field: "AnioMes",
                    title: "OBTENCIÓN DEL ESTUDIO",
                    attributes: { style: "text-align:left;" },
                    width: "80px"
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1 && item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdEstudio + '\',1)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }

                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalEditarEstudio(\'' + item.IdEstudio + '\')">'; //GenerarFormatoContrato
                        controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar estudio"></span>';
                        controles += '</button>';

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //var items = [item.iCodPerfil, item.iSecuencia, item.iCodGrado];
                        //controles += '<a href="perfiles/nuevo">';
                        controles += '<button type="button" class="btn btn-danger btn-xs" onclick="controlador.EliminarEstudio(\'' + item.IdEstudio + '\')">';
                        controles += '<span class="glyphicon glyphicon-remove" aria-hidden="true" data-uib-tooltip="Eliminar" title="Eliminar estudio"></span>';
                        controles += '</button>';
                        //controles += '</a>';

                        return controles;
                    },
                    width: '30px'
                }
            ]
        }).data();
    }
    this.PostulanteJS.prototype.EliminarEstudio = function (IdEstudio) {
        var data_param = new FormData();        
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdEstudio', IdEstudio);

        controladorApp.abrirMensajeDeConfirmacion(
            '¿Está seguro de eliminar el estudio?', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Postulante/EliminarPostulanteEstudio',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            controladorApp.notificarMensajeSatisfactorio("Estudio eliminado correctamente");
                            controlador.CargarGridEstudio(event);
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);
    };
    this.PostulanteJS.prototype.EliminarExperiencia = function (IdLaboral) {
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdLaboral', IdLaboral);

        controladorApp.abrirMensajeDeConfirmacion(
            '¿Está seguro de eliminar la experiencia laboral?', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Postulante/EliminarPostulanteExperiencia',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            controladorApp.notificarMensajeSatisfactorio("Experiencia laboral eliminada correctamente");
                            controlador.CargarGridExperiencia(event);
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);
    };
    this.PostulanteJS.prototype.EliminarCapacitacion = function (IdCapacitacion) {
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdCapacitacion', IdCapacitacion);

        controladorApp.abrirMensajeDeConfirmacion(
            '¿Está seguro de eliminar la capacitación?', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Postulante/EliminarPostulanteCapacitacion',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            controladorApp.notificarMensajeSatisfactorio("Capacitación eliminada correctamente");
                            controlador.CargarGridCapacitacion(event);
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);
    };
    this.PostulanteJS.prototype.EliminarDocumento = function (IdDocumento) {
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdDocumento', IdDocumento);

        controladorApp.abrirMensajeDeConfirmacion(
            '¿Está seguro de eliminar el tipo de documento?', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Postulante/EliminarPostulanteDocumento',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            controladorApp.notificarMensajeSatisfactorio("Tipo de documento eliminado correctamente");
                            controlador.CargarGridDocumento(event);
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);
    };

    this.PostulanteJS.prototype.CargarGridExperiencia = function () {
        this.$dataSourceExperiencia = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulanteExperiencia',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulante = $("#hdIdPostulante").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdLaboral"
                }
            },
        });

        this.divGridExperiencia = $("#divGridExperiencia").kendoGrid({
            //toolbar: ["excel"], //"excel", 
            dataSource: this.$dataSourceExperiencia,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "Empresa",
                    title: "NOMBRE DE LA ENTIDAD O EMPRESA",
                    attributes: { style: "text-align:left;" },
                    width: "200px"
                },
                {
                    field: "Cargo",
                    title: "CARGO DESEMPEÑADO",
                    attributes: { style: "text-align:left;" },
                    width: "200px"
                },
                {
                    field: "FechaInicio",
                    title: "FECHA DE INICIO",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "FechaFin",
                    title: "FECHA DE TÉRMINO",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "Descripcion",
                    title: "DESCRIPCIÓN DEL TRABAJO REALIZADO",
                    attributes: { style: "text-align:left;" },
                    width: "250px"
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1 && item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdLaboral + '\',3)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }
                        
                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalEditarExperiencia(\'' + item.IdLaboral + '\')">'; //GenerarFormatoContrato
                        controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Ver documento adjunto"></span>';
                        controles += '</button>';

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //var items = [item.iCodPerfil, item.iSecuencia, item.iCodGrado];
                        //controles += '<a href="perfiles/nuevo">';
                        controles += '<button type="button" class="btn btn-danger btn-xs" onclick="controlador.EliminarExperiencia(\'' + item.IdLaboral + '\')">';
                        controles += '<span class="glyphicon glyphicon-remove" aria-hidden="true" data-uib-tooltip="Eliminar" title="Eliminar experiencia laboral"></span>';
                        controles += '</button>';
                        //controles += '</a>';

                        return controles;
                    },
                    width: '30px'
                },
            ]
        }).data();
    }

    this.PostulanteJS.prototype.CargarGridCapacitacion = function () {
        this.$dataSourceCapacitacion = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarPostulanteCapacitacion',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    if ($operation == "read") {
                        data_param.IdPostulante = $("#hdIdPostulante").val();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdCapacitacion"
                }
            },
        });

        this.divGridCapacitacion = $("#divGridCapacitacion").kendoGrid({
            //toolbar: ["create"], //"excel", 
            dataSource: this.$dataSourceCapacitacion,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "Especialidad",
                    title: "NOMBRE DE LA CAPACITACIÓN",
                    attributes: { style: "text-align:left;" },
                    width: "200px"
                },
                {
                    field: "Institucion",
                    title: "INSTITUCIÓN",
                    attributes: { style: "text-align:left;" },
                    width: "200px"
                },
                {
                    field: "Ciudad",
                    title: "CIUDAD / PAIS",
                    attributes: { style: "text-align:left;" },
                    width: "100px"
                },
                {
                    field: "FechaInicio",
                    title: "FECHA DE INICIO",
                    width: "50px",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaInicioEditor
                },
                {
                    field: "FechaFin",
                    title: "FECHA DE TÉRMINO",
                    width: "50px",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaFinEditor
                },
                {
                    field: "Horas",
                    title: "HORAS LECTIVAS",
                    attributes: { style: "text-align:right;" },
                    width: "50px"
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1 && item.IdTieneArchivo == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalVisor(\'' + item.IdCapacitacion + '\',2)">'; //GenerarFormatoContrato
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Ver documento adjunto" title="Ver documento adjunto"></span>';
                            controles += '</button>&nbsp;&nbsp;';
                        }
                        
                        return controles;
                    },
                    width: '30px'
                },
                {
                    //INGRESAR DETALLE DE LA CAPACITACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalEditarCapacitacion(\'' + item.IdCapacitacion + '\')">'; //GenerarFormatoContrato
                        controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Ver documento adjunto"></span>';
                        controles += '</button>';

                        return controles;
                    },
                    width: '30px'
                },
                {
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        //var items = [item.iCodPerfil, item.iSecuencia, item.iCodGrado];
                        //controles += '<a href="perfiles/nuevo">';
                        controles += '<button type="button" class="btn btn-danger btn-xs" onclick="controlador.EliminarCapacitacion(\'' + item.IdCapacitacion + '\')">';
                        controles += '<span class="glyphicon glyphicon-remove" aria-hidden="true" data-uib-tooltip="Eliminar" title="Eliminar capacitación"></span>';
                        controles += '</button>';
                        //controles += '</a>';

                        return controles;
                    },
                    width: '30px'
                },
            ]
        }).data();

    }

    this.PostulanteJS.prototype.actualizarPostulante = function (e) {
        e.preventDefault();

        if (frmPersonaValidador.validate()) {
            var esValido = true;
            var mensajeValidacion = '';

            var data_param = new FormData();
            data_param.append('IdPostulante', $("#hdIdPostulante").val());
            data_param.append('Nacionalidad', $("#txtPersonaNacionalidad").val());
            data_param.append('TipoDocumento', $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value());
            data_param.append('NroDocumento', $("#txtPersonaNumeroDeDocumento").val());
            data_param.append('Nombre', $("#txtPersonaNombres").val());
            data_param.append('Paterno', $("#txtPersonaApellidoPaterno").val());
            data_param.append('Materno', $("#txtPersonaApellidoMaterno").val());
            data_param.append('Sexo', $("#ddlPersonaSexo").data("kendoDropDownList").value());
            data_param.append('IdEstadoCivil', $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value());
            data_param.append('Domicilio', $("#txtPersonaDireccionDomicilio").val());
            data_param.append('DescripcionUbigeo', $("#txtPersonaUbigeo").val());
            data_param.append('FechaNacimiento', kendo.toString(kendo.parseDate($("#txtPersonaFechaNacimiento").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('Celular', $("#txtTelefonoCelularP").val());
            data_param.append('Telefono', $("#txtTelefonoFijoP").val());
            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('RUC', $("#txtPersonaRUC").val());

         

            if ($("#chkPersonaDiscapacidad").prop("checked")) {
                if ($("#txtCarnetConadis").val().trim() == "") {
                    controladorApp.notificarMensajeDeAlerta('El carnet CONADIS es obligatorio');
                    $("#txtCarnetConadis").focus();
                    return;
                }

                data_param.append('Discapacidad', 1);
                data_param.append('CarnetConadis', $("#txtCarnetConadis").val());
            } else {
                data_param.append('Discapacidad', 0);
                data_param.append('CarnetConadis', "");
            }

            if ($("#chkPersonaDeportista").prop("checked")) {
                if ($("#txtDocumentoPersonaDeportista").val().trim() == "") {
                    controladorApp.notificarMensajeDeAlerta('El documento es obligatorio');
                    $("#txtDocumentoPersonaDeportista").focus();
                    return;
                }

                data_param.append('Deportista', 1);
                data_param.append('Acreditacion', $("#txtDocumentoPersonaDeportista").val());
            } else {
                data_param.append('Deportista', 0);
                data_param.append('Acreditacion', "");
            }

            data_param.append('Ubigeo', $("#hdIdUbigeoPostulante").val());
            data_param.append('Foto', $("#hdFoto").val());

            var nacimiento = kendo.toString(kendo.parseDate($("#txtPersonaFechaNacimiento").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            if (nacimiento == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de nacimiento no es válida');
                $("#txtPersonaFechaNacimiento").focus();
                return;
            }

            data_param.append('DireccionActual', $('#txtDomicilioActual').val());
            data_param.append('Referencia', $('#txtReferencia').val());

            if ($('#chkColegiatura').prop("checked")) {
                if ($("#txtPersonaColegio").val().trim() == "") {
                    controladorApp.notificarMensajeDeAlerta('El colegio profesional es obligatorio');
                    $("#txtPersonaColegioNro").focus();
                    return;
                }

                if ($("#txtPersonaColegioNro").val().trim() == "") {
                    controladorApp.notificarMensajeDeAlerta('El número de colegiatura es obligatorio');
                    $("#txtPersonaColegioNro").focus();
                    return;
                }

                data_param.append('bColigiatura', true);
                data_param.append('Colegio', $("#txtPersonaColegio").val());
                data_param.append('NroColegiatura', $("#txtPersonaColegioNro").val());
                data_param.append('bColegiatura_Habilitada', $('#bColegiatura_Habilitada').prop("checked") ? 1 : true)
            } else {
                data_param.append('bColigiatura', false);
                data_param.append('Colegio', "");
                data_param.append('NroColegiatura', "");
                data_param.append('bColegiatura_Habilitada', false)
            }

            if ($('#chkPersonaFFAA').prop("checked")) {
                if ($("#txtDocumentoFuerzasArmadas").val().trim() == "") {
                    controladorApp.notificarMensajeDeAlerta('El Carnet de las FFAA es obligatorio');
                    $("#txtDocumentoFuerzasArmadas").focus();
                    return;
                }
                data_param.append('FFAA', 1);
                data_param.append('CarnetFuerzasArmadas', $("#txtDocumentoFuerzasArmadas").val());
            } else {
                data_param.append('FFAA', 0);
                data_param.append('CarnetFuerzasArmadas', "");
            }

            if ($('#chkTrabajarInterior').prop("checked")) {
                if ($("#txtTrabajarProvincia").val().trim() == "") {
                    controladorApp.notificarMensajeDeAlerta('La provincia es obligatoria');
                    $("#txtTrabajarProvincia").focus();
                    return;
                }

                data_param.append('bTrabajarProvincia', true);
                data_param.append('Trabajar_Provincia', $("#txtTrabajarProvincia").val());
            } else {
                data_param.append('bTrabajarProvincia', false);
                data_param.append('Trabajar_Provincia', "");
            }

            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de guardar la información ingresada en su perfil?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/ActualizarPostulante',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: data_param,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Información registrada correctamente");
                                controlador.CargarPostulante();
                                //$('#divGrid').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }

    this.PostulanteJS.prototype.registrarPostulanteCapacitacion = function (e) {
        e.preventDefault();

        if (frmCapacitacionValidador.validate()) {
            var esValido = true;
            var mensajeValidacion = '';
            var modal = $('#divModalCapacitacion').data('kendoWindow');

            var data_param = new FormData();
            data_param.append('IdPostulante', $("#hdIdPostulante").val());
            data_param.append('IdCapacitacion', $("#hdIdCapacitacion").val());
            data_param.append('Especialidad', $("#txtCapacitacionNombre").val());
            data_param.append('Institucion', $("#txtCapacitacionInstitucion").val());
            data_param.append('FechaInicio', kendo.toString(kendo.parseDate($("#txtCapacitacionInicio").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('FechaFin', kendo.toString(kendo.parseDate($("#txtCapacitacionFin").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('Ciudad', $("#txtCapacitacionCiudad").val());
            data_param.append('Horas', $("#txtCapacitacionHoras").val());
            data_param.append('Estado', 1);
            data_param.append('TipoEstudio', $("#ddlTipoMateria").data("kendoDropDownList").value());
            data_param.append('TipoEstudioOtro', $("#ddlTipoMateriaOtros").data("kendoDropDownList").value());
            data_param.append('NivelTipoEstudioOtro', $("#ddlNivelTipoMateriaOtros").data("kendoDropDownList").value());

            var inicio = kendo.toString(kendo.parseDate($("#txtCapacitacionInicio").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            var fin = kendo.toString(kendo.parseDate($("#txtCapacitacionFin").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            if (inicio == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de inicio no es válida');
                $("#txtCapacitacionInicio").focus();
                return;
            }
            if (fin == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de fin no es válida');
                $("#txtCapacitacionFin").focus();
                return;
            }

            if ($("#ddlTipoMateria").data("kendoDropDownList").value() == "7") {
                if ($("#ddlTipoMateriaOtros").data("kendoDropDownList").value() == "") {
                    controladorApp.notificarMensajeDeAlerta('Debe seleccionar otro tipo de Estudio');
                    return;
                }

                if ($("#ddlNivelTipoMateriaOtros").data("kendoDropDownList").value() == "") {
                    controladorApp.notificarMensajeDeAlerta('Debe seleccionar el nivel de estudio');
                    return;
                }
            }

            var existeFile = false;
            var upload1 = $("#fileCapacitacion").getKendoUpload();
            var file1 = upload1.getFiles();
            if (file1.length == 0 && $("#IdTieneArchivo").val() == "0") {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar su documento de sustento (Formato PDF)');
                return false;
            } else {
                if (file1.length > 0) {
                    if (file1[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[0]', file1[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }

            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de guardar la información ingresada?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarPostulanteCapacitacion',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: data_param,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Capacitación registrada correctamente");
                                controlador.CargarGridCapacitacion();

                                modal.close();
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }
    this.PostulanteJS.prototype.registrarPostulanteDocumento = function (e) {
        e.preventDefault();

        if (frmDocumentoValidador.validate()) {
            var esValido = true;
            var mensajeValidacion = '';
            var modal = $('#divModalDocumento').data('kendoWindow');

            var data_param = new FormData();
            data_param.append('IdPostulante', $("#hdIdPostulante").val());
            data_param.append('IdDocumento', $("#hdIdDocumento").val());
            data_param.append('IdTipoDocumento', $("#ddlTipoDocumento").data("kendoDropDownList").value());
            data_param.append('Estado', 1);

            var existeFile = false;
            var upload1 = $("#fileDocumento").getKendoUpload();
            var file1 = upload1.getFiles();
            if (file1.length == 0 && $("#IdTieneArchivo").val() == "0") {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar su documento de sustento (Formato PDF)');
                return false;
            } else {
                if (file1.length > 0) {
                    if (file1[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[0]', file1[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }

            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de guardar la información ingresada?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarPostulanteDocumento',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: data_param,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Documento de sustento registrado correctamente");
                                controlador.CargarGridDocumento();

                                modal.close();
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }
    this.PostulanteJS.prototype.registrarPostulanteEstudio = function (e) {
        e.preventDefault();

        if (frmEstudioValidador.validate()) {
            var esValido = true;
            var mensajeValidacion = '';
            var modal = $('#divModalEstudio').data('kendoWindow');

            var data_param = new FormData();
            data_param.append('IdPostulante', $("#hdIdPostulante").val());
            data_param.append('IdEstudio', $("#hdIdEstudio").val());
            data_param.append('IdNivel', $("#ddlNivelEstudio").data("kendoDropDownList").value());
            data_param.append('IdNoTieneTitulo', $("#ddlNivelLogradoEstudio").data("kendoDropDownList").value());
            data_param.append('Especialidad', $("#txtEspecialidadEstudio").val());
            data_param.append('Institucion', $("#txtInstitucionEstudio").val());
            data_param.append('IdMes', $("#ddlMesEstudio").data("kendoDropDownList").value());
            data_param.append('IdAnio', $("#ddlAnioEstudio").data("kendoDropDownList").value());
            data_param.append('Ciudad', $("#txtCiudadEstudio").val());
            data_param.append('Estado', 1);
            data_param.append('IdFechaInicioMes', $("#ddlMesFechaInicio").data("kendoDropDownList").value());
            data_param.append('IdFechaInicioAnio', $("#ddlAnioFechaInicio").data("kendoDropDownList").value());
            data_param.append('IdFechaFinMes', $("#ddlMesFechaFin").data("kendoDropDownList").value());
            data_param.append('IdFechaFinAnio', $("#ddlAnioFechaFin").data("kendoDropDownList").value());
            
            var existeFile = false;
            var upload1 = $("#fileEstudio").getKendoUpload();
            var file1 = upload1.getFiles();
            if (file1.length == 0 && $("#IdTieneArchivo").val() == "0") {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar su documento de sustento (Formato PDF)');
                return false;
            } else {
                if (file1.length > 0) {
                    if (file1[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[0]', file1[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }

            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de guardar la información ingresada?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarPostulanteEstudio',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: data_param,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Estudio registrado correctamente");
                                controlador.CargarGridEstudio();
                                
                                modal.close();
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }
    this.PostulanteJS.prototype.registrarPostulanteExperiencia = function (e) {
        e.preventDefault();

        if (frmExperienciaValidador.validate()) {
            var esValido = true;
            var mensajeValidacion = '';
            var modal = $('#divModalExperiencia').data('kendoWindow');

            var data_param = new FormData();
            data_param.append('IdPostulante', $("#hdIdPostulante").val());
            data_param.append('IdLaboral', $("#hdIdLaboral").val());
            data_param.append('Empresa', $("#txtExperienciaNombre").val());
            data_param.append('Cargo', $("#txtExperienciaCargo").val());
            data_param.append('FechaInicio', kendo.toString(kendo.parseDate($("#txtExperienciaInicio").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('FechaFin', kendo.toString(kendo.parseDate($("#txtExperienciaFin").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('Descripcion', $("#txtExperienciaDescripcion").val());
            data_param.append('Estado', 1);

            //Sector Público
            if ($("#ddlSector").data("kendoDropDownList").value() == "2") {
                if ($("#ddlRegimen").data("kendoDropDownList").value() == "") {
                    controladorApp.notificarMensajeDeAlerta('El régimen es obligatorio');
                    $("#ddlRegimen").focus();
                    return;
                }

                data_param.append('Regimen', $("#ddlRegimen").data("kendoDropDownList").value());

            } else {
                data_param.append('Regimen', 0);
            }

            data_param.append('Sector', $("#ddlSector").data("kendoDropDownList").value());
            data_param.append('Regimen', $("#ddlRegimen").data("kendoDropDownList").value());
            data_param.append('NombreJefeDirecto', $('#txtNombreJefeDirecto').val());
            data_param.append('PuestoJefeDirecto', $('#txtCargoJefeDirecto').val());
            data_param.append('MotivoCambio', $('#txtMotivoCambio').val());
            data_param.append('Remuneracion', $('#txtRemuneracion').val());

            var inicio = kendo.toString(kendo.parseDate($("#txtExperienciaInicio").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            var fin = kendo.toString(kendo.parseDate($("#txtExperienciaFin").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            if (inicio == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de inicio no es válida');
                $("#txtExperienciaInicio").focus();
                return;
            }
            if (fin == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de fin no es válida');
                $("#txtExperienciaFin").focus();
                return;
            }

            data_param.append('ReferenciaNombre', $('#txtNombreReferencia').val());
            data_param.append('ReferenciaPuesto', $('#txtPuestoReferencia').val());
            data_param.append('ReferenciaTelefono', $('#txtTelefonoReferencia').val());
            data_param.append('ReferenciaCorreo', $('#txtCorreoReferencia').val());

            var existeFile = false;
            var upload1 = $("#fileExperiencia").getKendoUpload();
            var file1 = upload1.getFiles();
            if (file1.length == 0 && $("#IdTieneArchivo").val() == "0") {
                controladorApp.notificarMensajeDeAlerta('Debe seleccionar su documento de sustento (Formato PDF)');
                return false;
            } else {
                if (file1.length > 0) {
                    if (file1[0].extension.toLowerCase() == '.pdf') {
                        existeFile = true;
                        data_param.append('formatos[0]', file1[0].rawFile);
                    }
                    else {
                        controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                        return false;
                    }
                }
            }

            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de guardar la información ingresada?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarPostulanteExperiencia',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: data_param,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Experiencia laboral registrada correctamente");
                                controlador.CargarGridExperiencia();

                                modal.close();
                            }
                        },
                        error: function (res) {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }
    this.PostulanteJS.prototype.registrarPostulante = function (e) {
        e.preventDefault();
        
        if (frmPersonaValidador.validate()) {
            var esValido = true;
            var mensajeValidacion = '';

            var data_param = new FormData();
            data_param.append('IdPostulante', $("#hdIdPostulante").val());
            data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
            data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());
            data_param.append('Nacionalidad', $("#txtPersonaNacionalidad").val());
            data_param.append('TipoDocumento', $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value());
            data_param.append('NroDocumento', $("#txtPersonaNumeroDeDocumento").val());
            data_param.append('Nombre', $("#txtPersonaNombres").val());
            data_param.append('Paterno', $("#txtPersonaApellidoPaterno").val());
            data_param.append('Materno', $("#txtPersonaApellidoMaterno").val());
            data_param.append('Sexo', $("#ddlPersonaSexo").data("kendoDropDownList").value());
            data_param.append('IdEstadoCivil', $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value());
            data_param.append('Domicilio', $("#txtPersonaDireccionDomicilio").val());
            data_param.append('DescripcionUbigeo', $("#txtPersonaUbigeo").val());
            data_param.append('FechaNacimiento', kendo.toString(kendo.parseDate($("#txtPersonaFechaNacimiento").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('LugarNacimiento', $("#txtPersonaLugarNacimiento").val());
            data_param.append('Edad', $("#txtPersonaEdad").val());
            data_param.append('IdTipoVivienda', $("#ddlPersonaTipoVivienda").data("kendoDropDownList").value());
            data_param.append('IdTipoBrevete', $("#ddlPersonaTipoBrevete").data("kendoDropDownList").value());
            data_param.append('NroLicencia', $("#txtPersonaNroLicencia").val());
            data_param.append('Celular', $("#txtTelefonoCelularP").val());
            data_param.append('Telefono', $("#txtTelefonoFijoP").val());
            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('RUC', $("#txtPersonaRUC").val());
            data_param.append('TelefonoEmergencia1', $("#txtTelefonoEmergencia1").val());
            data_param.append('TelefonoEmergencia2', $("#txtTelefonoEmergencia2").val());
            data_param.append('ContactoEmergencia1', $("#txtNombreEmergencia1").val());
            data_param.append('ContactoEmergencia2', $("#txtNombreEmergencia2").val());
            if ($("#tienepension1").prop("checked")) {
                data_param.append('IdEstaAfiliadoPensiones', 1);
                data_param.append('IdAFPAfiliada', $("#ddlPersonaTipoPension").data("kendoDropDownList").value());
                //data_param.append('NombreAFPAfiliada', $("#txtPersonaNombreAfp").val());
                //data_param.append('IdTipoPension', $("#ddlPersonaTipoPension").data("kendoDropDownList").value());
                data_param.append('IdTipoAfp', $("#ddlPersonaTipoAFP").data("kendoDropDownList").value());
                data_param.append('CodigoAfp', $("#txtPersonaCodigoAfp").val());
                data_param.append('IdTipoComisionAfp', $("#ddlPersonaTipoComisionAFP").data("kendoDropDownList").value()); 
            }
            else {
                data_param.append('IdEstaAfiliadoPensiones', 0);
                data_param.append('IdTipoPensionDeseaAfiliar', $("#ddlPersonaNuevoTipoPension").data("kendoDropDownList").value());
            } 
            
            if ($("#tienebanco1").prop("checked")) data_param.append('IdEstaAfiliadoBanco', 1);
            else data_param.append('IdEstaAfiliadoBanco', 0);
            data_param.append('IdBancoAfiliado', $("#ddlPersonaBanco").data("kendoDropDownList").value());
            data_param.append('CuentaBancoAfiliado', $("#txtPersonaBanco").val());
            data_param.append('CuentaBancoCCIAfiliado', $("#txtPersonaBancoCCI").val());
            data_param.append('IdBancoDeseaAfiliar', $("#ddlPersonaNuevoBanco").data("kendoDropDownList").value());

            if ($("#situacionAcademicaS").prop("checked")) data_param.append('IdSituacionAcademicaS', 1);
            else data_param.append('IdSituacionAcademicaS', 0);
            if ($("#situacionAcademicaT").prop("checked")) data_param.append('IdSituacionAcademicaT', 1);
            else data_param.append('IdSituacionAcademicaT', 0);
            if ($("#situacionAcademicaU").prop("checked")) data_param.append('IdSituacionAcademicaU', 1);
            else data_param.append('IdSituacionAcademicaU', 0);
            if ($("#situacionAcademicaO").prop("checked")) data_param.append('IdSituacionAcademicaO', 1);
            else data_param.append('IdSituacionAcademicaO', 0);
            if ($("#centroestudiosPU").prop("checked")) data_param.append('IdCentroEstudiosPU', 1);
            else data_param.append('IdCentroEstudiosPU', 0);
            if ($("#centroestudiosPR").prop("checked")) data_param.append('IdCentroEstudiosPR', 1);
            else data_param.append('IdCentroEstudiosPR', 0);
            if ($("#centroestudiosEX").prop("checked")) data_param.append('IdCentroEstudiosEX', 1);
            else data_param.append('IdCentroEstudiosEX', 0);
            data_param.append('SituacionAcademicaT', $("#txtsituacionAcademicaT").val());
            data_param.append('SituacionAcademicaU', $("#txtsituacionAcademicaU").val());
            data_param.append('SituacionAcademicaO', $("#txtsituacionAcademicaO").val());
            data_param.append('CentroEstudiosPU', $("#txtcentroestudiosPU").val());
            data_param.append('CentroEstudiosPR', $("#txtcentroestudiosPR").val());
            data_param.append('CentroEstudiosEX', $("#txtcentroestudiosEX").val());

            if ($("#gradoacademicoES").prop("checked")) data_param.append('IdGradoAcademicoES', 1);
            else data_param.append('IdGradoAcademicoES', 0);
            if ($("#gradoacademicoEG").prop("checked")) data_param.append('IdGradoAcademicoEG', 1);
            else data_param.append('IdGradoAcademicoEG', 0);
            if ($("#gradoacademicoBA").prop("checked")) data_param.append('IdGradoAcademicoBA', 1);
            else data_param.append('IdGradoAcademicoBA', 0);
            if ($("#gradoacademicoTI").prop("checked")) data_param.append('IdGradoAcademicoTI', 1);
            else data_param.append('IdGradoAcademicoTI', 0);
            if ($("#postgradoM").prop("checked")) data_param.append('IdPostgradoM', 1);
            else data_param.append('IdPostgradoM', 0);
            if ($("#postgradoD").prop("checked")) data_param.append('IdPostgradoD', 1);
            else data_param.append('IdPostgradoD', 0);
            if ($("#postgradoO").prop("checked")) data_param.append('IdPostgradoO', 1);
            else data_param.append('IdPostgradoO', 0);
            data_param.append('PostgradoO', $("#txtpostgradoO").val());
            data_param.append('PostgradoCE', $("#txtpostgradoCE").val());
            data_param.append('PostgradoGrado', $("#txtpostgradoGrado").val());

            if ($("#tienealergias1").prop("checked")) data_param.append('IdPresentaAlergias', 1);
            else data_param.append('IdPresentaAlergias', 0);
            if ($("#alergia1").prop("checked")) data_param.append('IdPresentaAlergias1', 1);
            if ($("#alergia2").prop("checked")) data_param.append('IdPresentaAlergias2', 1);
            data_param.append('PresentaAlergiasOtro', $("#txtAlergiaOtro").val());
            if ($("#tieneenfermedad1").prop("checked")) data_param.append('IdPresentaEnfermedades', 1);
            else data_param.append('IdPresentaEnfermedades', 0);
            if ($("#tieneenefermedadD").prop("checked")) data_param.append('IdPresentaEnfermedadesD', 1);
            if ($("#tieneenefermedadH").prop("checked")) data_param.append('IdPresentaEnfermedadesH', 1);
            if ($("#tieneenefermedadA").prop("checked")) data_param.append('IdPresentaEnfermedadesA', 1);
            if ($("#tieneenefermedadE").prop("checked")) data_param.append('IdPresentaEnfermedadesE', 1);
            data_param.append('PresentaEnfermedadesOtro', $("#txtEnfermedadOtro").val());

            if ($("#tienemedicamento1").prop("checked")) data_param.append('IdConsumeMedicamentos', 1);
            else data_param.append('IdConsumeMedicamentos', 0);
            data_param.append('ConsumeMedicamentosOtro', $("#txtMedicamentoOtro").val());
            data_param.append('IdGrupoSanguineo', $("#ddlPersonaGrupoSan").data("kendoDropDownList").value());
            data_param.append('InformacionAdicionalSalud', $("#txtSaludAdicional").val());

            if ($("#tienediscapacidad1").prop("checked")) data_param.append('IdPresentaDiscapacidad', 1);
            else data_param.append('IdPresentaDiscapacidad', 0);
            if ($("#tienediscapacidadA").prop("checked")) data_param.append('IdPresentaDiscapacidadA', 1);
            else data_param.append('IdPresentaDiscapacidadA', 0);
            if ($("#tienediscapacidadV").prop("checked")) data_param.append('IdPresentaDiscapacidadV', 1);
            else data_param.append('IdPresentaDiscapacidadV', 0);
            if ($("#tienediscapacidadH").prop("checked")) data_param.append('IdPresentaDiscapacidadH', 1);
            else data_param.append('IdPresentaDiscapacidadH', 0);
            if ($("#tienediscapacidadC").prop("checked")) data_param.append('IdPresentaDiscapacidadC', 1);
            else data_param.append('IdPresentaDiscapacidadC', 0);
            if ($("#tienediscapacidadF").prop("checked")) data_param.append('IdPresentaDiscapacidadF', 1);
            else data_param.append('IdPresentaDiscapacidadF', 0);
            data_param.append('PresentaDiscapacidadC', $("#txtDiscapacidadConducta").val());
            data_param.append('PresentaDiscapacidadF', $("#txtDiscapacidadFisica").val());

            if ($("#tienecertdiscapacidad1").prop("checked")) data_param.append('IdCertificadoDiscapacidad', 1);
            else data_param.append('IdCertificadoDiscapacidad', 0);

            if ($("#declaraIncompatibilidad1").prop("checked")) data_param.append('IdDeclaraIncompatibilidad', 1);
            else data_param.append('IdDeclaraIncompatibilidad', 0);
            if ($("#declaraNepotismo1").prop("checked")) data_param.append('IdDeclaraNepotismo', 1);
            else data_param.append('IdDeclaraNepotismo', 0);
            if ($("#declaraNormas1").prop("checked")) data_param.append('IdDeclaraNormas', 1);
            else data_param.append('IdDeclaraNormas', 0);
            if ($("#declaraInteres1").prop("checked")) data_param.append('IdDeclaraInteres', 1);
            else data_param.append('IdDeclaraInteres', 0);

            data_param.append('NepotismoRel1', $("#txtNepotismoRel1").val());
            data_param.append('NepotismoRel2', $('#txtNepotismoRel2').val());
            data_param.append('NepotismoRel3', $("#txtNepotismoRel3").val());
            data_param.append('NepotismoApe1', $("#txtNepotismoApe1").val());
            data_param.append('NepotismoApe2', $('#txtNepotismoApe2').val());
            data_param.append('NepotismoApe3', $("#txtNepotismoApe3").val());
            data_param.append('NepotismoNom1', $("#txtNepotismoNom1").val());
            data_param.append('NepotismoNom2', $('#txtNepotismoNom2').val());
            data_param.append('NepotismoNom3', $("#txtNepotismoNom3").val());
            data_param.append('NepotismoAre1', $("#txtNepotismoAre1").val());
            data_param.append('NepotismoAre2', $('#txtNepotismoAre2').val());
            data_param.append('NepotismoAre3', $("#txtNepotismoAre3").val());

            data_param.append('NombreProceso', $("#hdNombreProceso").val());

            data_param.append('Ubigeo', $("#hdIdUbigeoPostulante").val());
            data_param.append('Foto', $("#hdFoto").val());

            var nacimiento = kendo.toString(kendo.parseDate($("#txtPersonaFechaNacimiento").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            if (nacimiento == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de nacimiento no es válida');
                $("#txtPersonaFechaNacimiento").focus();
                return;
            }
            
            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de guardar la información ingresada?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/Guardar',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: data_param,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio("Información registrada correctamente");
                                controlador.CargarFormularioPostulante();
                                //$('#divGrid').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }
    }
    this.PostulanteJS.prototype.confirmarEnvio = function (e) {
        e.preventDefault();
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());

        var existeFile = false;
        var upload1 = $("#fileHojaVida").getKendoUpload();
        var file1 = upload1.getFiles();
        if (file1.length == 0 && $("#IdTieneHojaVida").val() == "0") {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar su fotografía en formato digital a color con fondo blanco y vestimenta formal (Formato JPG)');
            return false;
        } else {
            if (file1.length == 0) {
                data_param.append('formatos[0]', null);
            }
            else {
                if (file1[0].extension.toLowerCase() == '.jpg'){
                    existeFile = true;
                    data_param.append('formatos[0]', file1[0].rawFile);
                }   
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo de fotografía con extensión JPG');
                    return false;
                }
            }
        }
        var upload2 = $("#fileSustento").getKendoUpload();
        var file2 = upload2.getFiles();
        if (file2.length == 0 && $("#IdTieneSustento").val() == "0") {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar su copia de voucher de entidad bancaria, cuenta bancaria y código de cuenta intebancaria - CCI (Formato PDF)');
            return false;
        } else {
            if (file2.length == 0) {
                data_param.append('formatos[1]', null);
            }
            else {
                if (file2[0].extension.toLowerCase() == '.pdf'){
                    existeFile = true;
                    data_param.append('formatos[1]', file2[0].rawFile);
                }   
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }
        var upload3 = $("#fileDDJJ").getKendoUpload();
        var file3 = upload3.getFiles();
        if (file3.length == 0 && $("#IdTieneDDJJ").val() == "0") {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar su declaración jurada de formatos y declaraciones (Formato PDF)');
            return false;
        } else {
            if (file3.length == 0) {
                data_param.append('formatos[2]', null);
            }
            else {
                if (file3[0].extension.toLowerCase() == '.pdf'){
                    existeFile = true;
                    data_param.append('formatos[2]', file3[0].rawFile);
                }   
                else {
                    controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
                    return false;
                }
            }
        }
        //var upload4 = $("#fileFormato").getKendoUpload();
        //var file4 = upload4.getFiles();
        //if (file4.length == 0 && $("#IdTieneFormato").val() == "0") {
        //    controladorApp.notificarMensajeDeAlerta('Debe seleccionar su declaración jurada de formatos y declaraciones (Formato PDF)');
        //    return false;
        //} else {
        //    if (file4.length == 0) {
        //        data_param.append('formatos[3]', null);
        //    }
        //    else {
        //        if (file4[0].extension.toLowerCase() == '.pdf'){
        //            existeFile = true;
        //            data_param.append('formatos[3]', file4[0].rawFile);
        //        }   
        //        else {
        //            controladorApp.notificarMensajeDeAlerta('Sólo se admite archivo con extensión PDF');
        //            return false;
        //        }
        //    }
        //}
        
        if (!existeFile) {
            controladorApp.notificarMensajeDeAlerta('Debe seleccionar algún archivo antes de enviarlo para su evaluación');
            return false;
        }
        controladorApp.abrirMensajeDeConfirmacion(
               '<span>¿Está seguro de haber revisado toda la información ingresada para iniciar su evaluación?.</span></br></br><div class="alert alert-warning" style="padding: 5px; border: 1px solid #f7cf0aa0;">Recuerde que la información ingresada tiene carácter de <strong>DECLARACIÓN JURADA</strong></div>', 'SI', 'NO'
               , function (arg) {
                   $.ajax({
                       url: controladorApp.obtenerRutaBase() + 'Postulante/GuardarDocumentosPostulante',
                       type: 'POST',
                       dataType: 'json',
                       contentType: false,
                       processData: false,
                       data: arg,
                       success: function (res) {
                           debugger;
                           if (res.success == 'False') {
                               controladorApp.notificarMensajeDeAlerta(res.responseText);
                           }
                           else {
                               controladorApp.notificarMensajeSatisfactorio("Información registrada correctamente");
                               controlador.inicializarGridNotificaciones();
                               controlador.CargarFormularioPostulante();
                               //$('#divGrid').data("kendoGrid").dataSource.page(1);
                           }
                       },
                       error: function (res) {
                           alert(res);
                       }
                   });
               }, data_param);
    }
    this.PostulanteJS.prototype.confirmarAprobacion = function (e) {
        e.preventDefault();
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdPostulacion', $("#hdIdPostulacion").val());
        data_param.append('IdConvocatoria', $("#hdIdConvocatoria").val());

        controladorApp.abrirMensajeDeConfirmacion(
               '<div class="alert alert-warning" style="padding: 5px; border: 1px solid #f7cf0aa0;">¿Está seguro de haber <strong>REVISADO y ACEPTADO</strong> todas las condiciones establecidas en el contrato laboral generado?</div>', 'SI', 'NO'
               , function (arg) {
                   $.ajax({
                       url: controladorApp.obtenerRutaBase() + 'Postulante/AprobarContratoPostulante',
                       type: 'POST',
                       dataType: 'json',
                       contentType: false,
                       processData: false,
                       data: arg,
                       success: function (res) {
                           debugger;
                           if (res.success == 'False') {
                               controladorApp.notificarMensajeDeAlerta(res.responseText);
                           }
                           else {
                               controladorApp.notificarMensajeSatisfactorio("Aprobación registrada correctamente");
                               controlador.inicializarGridNotificaciones();
                               controlador.CargarFormularioPostulante();
                               //$('#divGrid').data("kendoGrid").dataSource.page(1);
                           }
                       },
                       error: function (res) {
                           alert(res);
                       }
                   });
               }, data_param);
    }
    this.PostulanteJS.prototype.cerrarModalVisor = function () {
        $('#divModalVisor').data('kendoWindow').close();
    }
    this.PostulanteJS.prototype.cerrarModalDocumento = function () {
        $('#divModalDocumento').data('kendoWindow').close();
    }
    this.PostulanteJS.prototype.cerrarModalEstudio = function () {
        $('#divModalEstudio').data('kendoWindow').close();
    }
    this.PostulanteJS.prototype.cerrarModalCapacitacion = function () {
        $('#divModalCapacitacion').data('kendoWindow').close();
    }
    this.PostulanteJS.prototype.cerrarModalExperiencia = function () {
        $('#divModalExperiencia').data('kendoWindow').close();
    }

    //this.PostulanteJS.prototype.ingresarDetalleEvaluacion = function (idCad, idEvaluacion) {
    //    //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
    //    window.open(controladorApp.obtenerRutaBase() + 'EvaluacionDetalle/?IdCad=' + idCad + "&IdEvaluacion=" + idEvaluacion, '_blank');
    //}

    this.PostulanteJS.prototype.generarFormatoFicha = function () {
        var IdPostulante = $("#hdIdPostulante").val();
        var IdPostulacion = $("#hdIdPostulacion").val();
        var IdConvocatoria = $("#hdIdConvocatoria").val();

        window.open(controladorApp.obtenerRutaBase() + "Postulante/FichaPersonal?IdPostulante=" + IdPostulante + "&IdPostulacion=" + IdPostulacion + "&IdConvocatoria=" + IdConvocatoria, "_blank");
    }

    this.PostulanteJS.prototype.abrirModalRegistroDocumento = function () {
        $("#hdIdDocumento").val("0");
        $("#fileDocumento").data("kendoUpload").clearAllFiles();

        var modal = $('#divModalDocumento').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Agregar documento de sustento");
        modal.open().center();
    }
    //this.PostulanteJS.prototype.abrirModalEditarDocumento = function (idDocumento) {
    //    debugger;
    //    $("#hdIdDocumento").val(idDocumento)
    //    var modal = $('#divModalDocumento').data('kendoWindow');
    //    modal.title("Actualizar documento");

    //    var data_param = new FormData();
    //    data_param.append('IdPostulante', $("#hdIdPostulante").val());
    //    data_param.append('IdDetalle', idDocumento);

    //    $.ajax({
    //        url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulanteEstudio',
    //        type: 'POST',
    //        dataType: 'json',
    //        contentType: false,
    //        processData: false,
    //        data: data_param,
    //        success: function (res) {
                
    //            modal.open().center();
    //        },
    //        error: function (res) {

    //        }
    //    });
    //}

    this.PostulanteJS.prototype.abrirModalRegistroEstudio = function () {
        $("#hdIdEstudio").val("0");
        $("#IdTieneArchivo").val("0");
        $("#txtEspecialidadEstudio").val("");
        $("#txtInstitucionEstudio").val("");
        $("#txtCiudadEstudio").val("LIMA / PERU");
        $("#fileEstudio").data("kendoUpload").clearAllFiles();

        var modal = $('#divModalEstudio').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Agregar estudio");
        modal.open().center();
    }
    this.PostulanteJS.prototype.abrirModalEditarEstudio = function (idEstudio) {
        debugger;
        $("#hdIdEstudio").val(idEstudio)
        var modal = $('#divModalEstudio').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Actualizar estudio");

        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdDetalle', idEstudio);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulanteEstudio',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                $("#ddlNivelEstudio").data("kendoDropDownList").value(res.IdNivel);
                $("#ddlNivelLogradoEstudio").data("kendoDropDownList").value(res.IdNoTieneTitulo);
                $("#txtEspecialidadEstudio").val(res.Especialidad);
                $("#txtInstitucionEstudio").val(res.Institucion);
                $("#txtCiudadEstudio").val(res.Ciudad);
                $("#ddlAnioEstudio").data("kendoDropDownList").value(res.IdAnio);
                $("#ddlMesEstudio").data("kendoDropDownList").value(res.IdMes);
                $("#fileEstudio").data("kendoUpload").clearAllFiles();
                $("#IdTieneArchivo").val(res.IdTieneArchivo);
                $("#ddlAnioFechaInicio").data("kendoDropDownList").value(res.IdFechaInicioAnio);
                $("#ddlMesFechaInicio").data("kendoDropDownList").value(res.IdFechaInicioMes);
                $("#ddlAnioFechaFin").data("kendoDropDownList").value(res.IdFechaFinAnio);
                $("#ddlMesFechaFin").data("kendoDropDownList").value(res.IdFechaFinMes);

                modal.open().center();
            },
            error: function (res) {

            }
        });
    }

    this.PostulanteJS.prototype.abrirModalRegistroCapacitacion = function () {
        $("#hdIdCapacitacion").val("0");
        $("#IdTieneArchivo").val("0");
        $("#txtCapacitacionNombre").val("");
        $("#txtCapacitacionInstitucion").val("");
        $("#txtCapacitacionHoras").val("");
        $("#txtCapacitacionInicio").val("");
        $("#txtCapacitacionFin").val("");
        $("#txtCapacitacionCiudad").val("LIMA / PERU");
        $("#fileCapacitacion").data("kendoUpload").clearAllFiles();
        $("#ddlTipoMateria").data("kendoDropDownList").value("");
        $("#ddlTipoMateriaOtros").data("kendoDropDownList").value("");
        $("#ddlNivelTipoMateriaOtros").data("kendoDropDownList").value("");

        OnChangeTipoMateria();

        var modal = $('#divModalCapacitacion').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Agregar capacitación");
        modal.open().center();
    }
    this.PostulanteJS.prototype.abrirModalEditarCapacitacion = function (idCapacitacion) {
        debugger;
        $("#hdIdCapacitacion").val(idCapacitacion)
        var modal = $('#divModalCapacitacion').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Actualizar conocimiento");

        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdDetalle', idCapacitacion);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulanteCapacitacion',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                $("#txtCapacitacionNombre").val(res.Especialidad);
                $("#txtCapacitacionInstitucion").val(res.Institucion);
                $("#txtCapacitacionCiudad").val(res.Ciudad);
                $("#txtCapacitacionHoras").val(res.Horas);
                $("#txtCapacitacionInicio").data("kendoDatePicker").value(res.FechaInicio);
                $("#txtCapacitacionFin").data("kendoDatePicker").value(res.FechaFin);
                $("#fileCapacitacion").data("kendoUpload").clearAllFiles();
                $("#IdTieneArchivo").val(res.IdTieneArchivo);
                $("#ddlTipoMateria").data("kendoDropDownList").value(res.TipoEstudio);
                $("#ddlTipoMateriaOtros").data("kendoDropDownList").value(res.TipoEstudioOtro);
                $("#ddlNivelTipoMateriaOtros").data("kendoDropDownList").value(res.NivelTipoEstudioOtro);

                OnChangeTipoMateria();

                modal.open().center();
            },
            error: function (res) {

            }
        });
    }

    this.PostulanteJS.prototype.abrirModalRegistroExperiencia = function () {
        $("#hdIdLaboral").val("0");
        $("#IdTieneArchivo").val("0");
        $("#txtExperienciaNombre").val("");
        $("#txtExperienciaCargo").val("");
        $("#txtExperienciaInicio").val("");
        $("#txtExperienciaFin").val("");
        $("#txtExperienciaDescripcion").val("");
        $("#fileExperiencia").data("kendoUpload").clearAllFiles();

        $("#ddlSector").data("kendoDropDownList").value("");
        $("#ddlRegimen").data("kendoDropDownList").value("");
        $('#txtNombreJefeDirecto').val("")
        $('#txtCargoJefeDirecto').val("")
        $('#txtMotivoCambio').val("")
        $('#txtRemuneracion').val("")
        $('#txtNombreReferencia').val("")
        $('#txtPuestoReferencia').val("")
        $('#txtTelefonoReferencia').val("")
        $('#txtCorreoReferencia').val("")

        OnChangeSector();

        var modal = $('#divModalExperiencia').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Agregar experiencia laboral");
        modal.open().center();
    }
    this.PostulanteJS.prototype.abrirModalEditarExperiencia = function (idLaboral) {
        debugger;
        $("#hdIdLaboral").val(idLaboral)
        var modal = $('#divModalExperiencia').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Actualizar experiencia laboral");
        
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdDetalle', idLaboral);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/ObtenerPostulanteExperiencia',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                $("#txtExperienciaNombre").val(res.Empresa);
                $("#txtExperienciaCargo").val(res.Cargo);
                $("#txtExperienciaInicio").data("kendoDatePicker").value(res.FechaInicio);
                $("#txtExperienciaFin").data("kendoDatePicker").value(res.FechaFin);
                $("#txtExperienciaDescripcion").val(res.Descripcion);
                $("#fileExperiencia").data("kendoUpload").clearAllFiles();
                $("#IdTieneArchivo").val(res.IdTieneArchivo);

                $("#ddlSector").data("kendoDropDownList").value(res.Sector);
                $("#ddlRegimen").data("kendoDropDownList").value(res.Regimen);
                $('#txtNombreJefeDirecto').val(res.NombreJefeDirecto);
                $('#txtCargoJefeDirecto').val(res.PuestoJefeDirecto);
                $('#txtMotivoCambio').val(res.MotivoCambio);
                $('#txtRemuneracion').val(res.Remuneracion);
                $('#txtNombreReferencia').val(res.ReferenciaNombre);
                $('#txtPuestoReferencia').val(res.ReferenciaPuesto);
                $('#txtTelefonoReferencia').val(res.ReferenciaTelefono);
                $('#txtCorreoReferencia').val(res.ReferenciaCorreo);

                OnChangeSector();

                modal.open().center();
            },
            error: function (res) {

            }
        });
    }
    this.PostulanteJS.prototype.abrirModalVisor = function (idDetalle, idTipo) {
        var modal = $('#divModalVisor').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(uid);
        modal.title("Ver documento");
        
        debugger;
        //var options = {
        //    width: "80rem",
        //    height: "100rem"
        //};
        var data_param = new FormData();
        data_param.append('IdPostulante', $("#hdIdPostulante").val());
        data_param.append('IdDetalle', idDetalle);
        data_param.append('IdTipo', idTipo);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/DescargarArchivoPostulante',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                if (res.success == "True") {
                    PDFObject.embed(res.responseText, "#app"); 
                }
                
                modal.center().open();
            },
            error: function (res) {

            }
        });
        
        //initPDFViewer("https://cdn.www.gob.pe/uploads/document/file/575243/RD_050-2020-MIDISP65-DE.pdf");
        //initPDFViewer("http://localhost:36205/temp/RM_076_2020MIDIS.pdf"); //575243/RD_050-2020-MIDISP65-DE
        //debugger;
        //modal.open();
    }

    this.PostulanteJS.prototype.cerrarModalEliminacion = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        $('#hdnUid').val('');
        modal.close();
    }

    this.PostulanteJS.prototype.eliminar = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        var grilla = $('#divGrid').data("kendoGrid");
        var dr = grilla.dataSource.getByUid($('#hdnUid').val());

        var data_param = new FormData();
        data_param.append('IdCad', dr.IdCad);
        data_param.append('IdPropuesta', dr.IdPropuesta);
        
        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Propuesta/Eliminar',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                controladorApp.notificarMensajeSatisfactorio("Registro eliminado correctamente");
                grilla.dataSource.page(1);
                modal.close();
            },
            error: function (res) {

            }
        });
    }

}(jQuery));