(function ($) {
    ...
    this.LegajoJS = function () { };
    this.LegajoJS.prototype.inicializar = function () {
    ...
    function LimpiarModalRegistroLegajo() {
    ...
    this.LegajoJS.prototype.inicializarGrid = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Legajo/ListarLegajos',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    ...
                    if ($operation === "read") {
                        data_param.Estado = 0; //$("#ddlEstado_busqueda").data("kendoDropDownList").value();
                        data_param.IdTipoDocumento = 0; //$("#ddlDependencia_busqueda").data("kendoDropDownList").value();
        ...
        this.$grid = $("#divGrid").kendoGrid({
                ...
                excel: {
                    fileName: "Listado de Legajos.xlsx",
                    filterable: false
                },
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        template: function (item) {
                            var controles = "";
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalRegistroLegajo(\'' + item.IdLegajo + '\')">';
                            controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del legajo de personal"></span>';
                            controles += '</button>';
    ...
    this.LegajoJS.prototype.buscar = function (e) {
        e.preventDefault();
    ...
    this.LegajoJS.prototype.abrirModalRegistroLegajo = function (id) {
        var modal = $('#divModalRegistroPersona').data('kendoWindow');

        LimpiarModalRegistroLegajo();
        ...
        if (id == 0) {           
            ...
            modal.title("Agregar Legajo");
            modal.open().center();
        }
        else {
            $("#hdIdLegajo").val(id);

            controlador.CargarFormularioLegajo(id);
            ...
            modal.title("Actualizar Legajo");
            modal.open().center();
        }
    }

    this.LegajoJS.prototype.cerrarModalRegistroPersona = function () {
        var modal = $('#divModalRegistroPersona').data('kendoWindow');
        modal.close();
    }

    this.LegajoJS.prototype.CargarFormularioLegajo = function (id) {
        var data_param = new FormData();
        data_param.append('IdLegajo', id);
    ...
    this.LegajoJS.prototype.CargarFormularioCuentasBancarias = function (id) {
    ...
    this.LegajoJS.prototype.BancoDropDownEditor = function (container, options) {
    ...
    this.LegajoJS.prototype.EstadoDropDownEditor = function (container, options) {
        var data = [
    ...
    this.LegajoJS.prototype.CargarFormularioOrdenesServicio = function (id) {
    ...
    this.LegajoJS.prototype.NombreOrdenEditor = function (container, options) {
        $('<textarea required name="' + options.field + '" style="width: ' + (container.width() + 30) + 'px;height:' + (container.height() + 30) + 'px" />')
            .appendTo(container);
    };
    this.LegajoJS.prototype.FechaInicioEditor = function (container, options) {
    ...
    this.LegajoJS.prototype.FechaFinEditor = function (container, options) {
    ...
    this.LegajoJS.prototype.agregarTrabajador = function (e) {
        e.preventDefault();
        ...
        if (frmPersonaValidador.validate()) {
            ...
            if ($("#hdIdEmpleado").val() != 0) {
                ...
                metodo = 'Guardar';
                mensaje = '¿Está seguro de actualizar el trabajador?';
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            if (res.success == 'False') {
                                ...
                                controlador.CargarFormularioLegajo(res.responseText);
    ...
    //this.LegajoJS.prototype.actualizarTrabajadorContacto = function (e) {
    //    e.preventDefault();
    ...
    this.LegajoJS.prototype.cerrarModalEdicion = function () {
        $('#divModalNuevo').data('kendoWindow').close();
    }

    this.LegajoJS.prototype.ingresarDetalleEvaluacion = function (idCad, idEvaluacion) {
    ...
    this.LegajoJS.prototype.abrirModalEliminacion = function (uid) {
        $('#hdnUid').val(uid);
    ...
    this.LegajoJS.prototype.cerrarModalEliminacion = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        $('#hdnUid').val('');
        modal.close();
    }

    this.LegajoJS.prototype.eliminar = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        var grilla = $('#divGrid').data("kendoGrid");
