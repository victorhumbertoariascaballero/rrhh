(function ($) {
    ...
    this.ResumenJS.prototype.inicializarGrid = function () {
        ...
        this.$grid = $("#divGrid").kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "TotalSisper",
                    title: "BOLETAS EN SISPER",
                    attributes: { style: "text-align:right;" },
                    width: "100px"
                },
                {
                    field: "TotalValidado",
                    title: "BOLETAS VALIDADAS",
                    attributes: { style: "text-align:right;" },
                    width: "100px"
                },
                {
                    field: "TotalEnviado",
                    title: "BOLETAS FIRMADAS",
                    attributes: { style: "text-align:right;" },
                    width: "100px"
                },
                //{
                ...
                //},
                {
                    ...
                    template: function (item) {
                        ...
                        if (item.TotalEnviado == 0 && item.TotalValidado > 0) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.firmarPeriodoBoletas(' + item.Anio + ',' + item.Mes + ',' + item.IdPlanilla + ',\'' + item.IdTipoPlanilla + '\')">';
                            controles += '<span class="glyphicon glyphicon-check" aria-hidden="true" data-uib-tooltip="Editar" title="Iniciar proceso de firma para esta planilla"></span>';
                            controles += '</button>';
                        }
    ...
    this.ResumenJS.prototype.firmarPeriodoBoletas = function (anio, mes, planilla, tipoPlanilla) {
        controladorApp.abrirMensajeDeConfirmacion('¿Está seguro de realizar la firma de boletas de pago para la planilla seleccionada?', 'SI', 'NO'
            , function (arg) {
                var data_param = new FormData();
                data_param.append('Anio', anio);
                data_param.append('Mes', mes);
                data_param.append('Planilla', planilla);
                data_param.append('TipoPlanilla', tipoPlanilla);
