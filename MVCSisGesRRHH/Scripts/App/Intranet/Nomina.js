(function ($) {
    var frmPersonaValidador;
    var frmPersonaCeseValidador;
    var strMensajes = '';
    var PERFIL_NOMINA_ABASTECIMIENTO = '45';
    var PERFIL_NOMINA_CONTACTO = '46';
    var PERFIL_NOMINA_CONTABILIDAD = '47';
    var colores = ['#ffc09f', '#ffee93', '#a0ced9', '#adf7b6', '#eac4d5', '#809bce', '#b8e0d2','#7ec4cf','#ff8fab','#ff686b'];

    this.NominaJS = function () { };
    this.NominaJS.prototype.inicializarMaestro = function () {

        /* BUSQUEDA */
        $("#ddlEstado_busqueda").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Nomina/ListarEstados",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlCondicion_busqueda").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdCondicion",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Condicion/ListarCondicion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        data_param.perfil = $("#hdPerfil").val();

                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlSede_busqueda").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlDependencia_busqueda").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        
        /* REGISTRO */
        $("#ddlAnio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Anio",
            dataValueField: "Anio",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Boletas/ListarAnios",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMes").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Boletas/ListarMeses",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        
        /* CONSULTA */

        $("#pnlEmpleado").kendoPanelBar({
            expandMode: "single"
        });

        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaGrupoSan").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarGrupoSanguineo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoIngreso").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdCondicion",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Condicion/ListarTipoIngreso",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                            var data_param = {};
                            data_param.perfil = $("#hdPerfil").val();

                            //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                            return $.toDictionary(data_param);
                        }
                }
            }//,
            //change: function (e) {
            //    var estado = this.value();
            //    if (estado == '5') {
            //        $("#divOrdenes").show();
            //        $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
            //        $("#txtPersonaCargo").removeAttr("required");
            //        $("#txtRUC").attr("required", true);
            //    }
            //    else {
            //        $("#divOrdenes").hide();
            //        $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
            //        $("#txtPersonaCargo").attr("required", true);
            //        $("#txtRUC").removeAttr("required");
            //    } 
            //}
        });
        $("#ddlPersonaCondicion").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdCondicion",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Condicion/ListarCondicion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        data_param.perfil = $("#hdPerfil").val();

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            },
            change: function (e) {
                var estado = this.value();
                if (estado == '5') {
                    $("#divOrdenes").show();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaCargo").removeAttr("required");
                    $("#txtRUC").attr("required", true);
                }
                else {
                    $("#divOrdenes").hide();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaCargo").attr("required", true);
                    $("#txtRUC").removeAttr("required");
                }
            }
        });
        $("#ddlPersonaSede").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstado").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstado",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            },
            change: function (e) {
                debugger;
                var estado = this.value();
                
                if (estado == '1') {
                    //$("#divCese").hide();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaFechaFinLabores").removeAttr("required");
                    $("#txtPersonaFechaFinLabores").val('');
                    //$("#ddlPersonaEstado").data("kendoDropDownList").css({'background-color': 'yellow'});
                }
                if (estado == '0') {
                    //$("#divCese").show();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaFechaFinLabores").attr("required", true);
                    $("#txtPersonaFechaFinLabores").attr("validationmessage", "requerido");
                }
            }
        });
        $("#ddlPersonaDependencia").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlPersonaTipoPension").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDePension",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }//,
            //change: function (e) {
            //    var estado = this.value();
            //    if (estado == '1') {
            //        $("#txtPersonaNombrePension").show();
            //        $("#txtPersonaNombrePension").attr("required", true);
            //    }
            //    else {
            //        $("#txtPersonaNombrePension").hide();
            //        $("#txtPersonaNombrePension").removeAttr("required");
            //    }
            //}
        });
        $("#ddlPersonaTipoAFP").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeAFP",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoComisionAFP").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeComisionAFP",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoContrato").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Contrato/ListarTipoDeIngreso",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }//,
            //change: function (e) {
            //    var estado = this.value();
            //    if (estado == '1') {
            //        $("#txtPersonaNombrePension").show();
            //        $("#txtPersonaNombrePension").attr("required", true);
            //    }
            //    else {
            //        $("#txtPersonaNombrePension").hide();
            //        $("#txtPersonaNombrePension").removeAttr("required");
            //    }
            //}
        });
        $("#ddlPersonaModalidad").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Contrato/ListarModalidadTrabajo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $('#divModalRegistroPersona').kendoWindow({
            draggable: true,
            modal: true,
            pinned: true,
            resizable: false,
            width: '90%',
            height: 'auto',
            title: 'Agregar Trabajador',
            visible: false,
            position: { top: '5%', left: "5%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close", "Maximize"],
            close: function () {
                frmPersonaValidador.hideMessages();

                $("#hdIdEmpleado").val('0');
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value('');
                $("#txtPersonaNumeroDeDocumento").val('');
                $("#chkPersonaDniValidoEnReniec").prop("checked", false);
                $("#txtPersonaNombres").val('');
                $("#txtPersonaApellidoPaterno").val('');
                $("#txtPersonaApellidoMaterno").val('');
                $("#ddlPersonaSexo").data("kendoDropDownList").value('');
                $("#ddlPersonaEstado").data("kendoDropDownList").value('1');
                //$("#ddlPersonaEstado").kendoDropDownList().attr("style", "background-color: yellow");
                //$("#txtPersonaFechaDeNacimiento").data("kendoDatePickerssms").value('');
                //$("#ddlPersonaIdUbigeoNacimiento").data("kendoDropDownList").value('');
                //$("#ddlPersonaIdUbigeoDomicilio").data("kendoDropDownList").value('');
                $("#txtPersonaDireccionDomicilio").val('');

                var panelBar = $("#pnlEmpleado").data("kendoPanelBar");
                if (panelBar != undefined) {
                    panelBar.clearSelection();
                    panelBar.collapse($("#liPersonales"), false);
                    panelBar.collapse($("#liContacto"), false);
                    panelBar.collapse($("#liPlanilla"), false);
                    panelBar.collapse($("#liFamiliares"), false);
                    panelBar.collapse($("#liHistorico"), false);
                }
            }
        }); //.data("kendoWindow"); //AQUI
        //alert('inicio maestro')

        $('#divModalCesePersona').kendoWindow({
            draggable: true,
            modal: true,
            pinned: true,
            resizable: false,
            width: '50%',
            height: 'auto',
            title: 'Confirmar eliminación',
            visible: false,
            position: { top: '25%', left: "25%" },
            actions: ["Close"],
            close: function () {
                frmPersonaCeseValidador.hideMessages();
                $('#hdIdEmpleadoAux').val('');
            }
        }).data("kendoWindow");

        $("#ddlEpsEsSalud").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlSupsRet4taCat").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }//$("#txtNroAutorizacionExoneracion").prop('readonly', false);
            },
            change: function (e) {
                var value = this.value();                
                
                if (value == "1") {
                    $("#txtNroAutorizacionExoneracion").prop('readonly', false);
                }
                else {
                    $("#txtNroAutorizacionExoneracion").prop('readonly', true);
                }
            }
        });
        $("#ddlDsctoJudicial").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        
        //$("#ddlPersonaDependenciaEncargatura").kendoDropDownList({
        //    autoBind: true,
        //    optionLabel: "--Todos--",
        //    dataTextField: "Nombre",
        //    dataValueField: "IdDependencia",
        //    filter: "contains",
        //    minLength: 3,
        //    dataSource: {
        //        serverFiltering: true,
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            },
        //            parameterMap: function ($options, $operation) {
        //                var data_param = {};
        //                if ($options.filter != undefined && $options.filter.filters.length > 0)
        //                    data_param.term = $options.filter.filters[0].value;

        //                //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
        //                return $.toDictionary(data_param);
        //            }
        //        }
        //    }
        //});

        frmPersonaValidador = $("#frmRegistroPersona").kendoValidator().data("kendoValidator");
        frmPersonaCeseValidador = $("#frmRegistroCese").kendoValidator().data("kendoValidator");

        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#txtPersonaFechaInicioLabores").kendoDatePicker({ format: "dd/MM/yyyy" });
        $("#txtPersonaFechaFinLabores").kendoDatePicker({ format: "dd/MM/yyyy" }); // max: new Date()
        $("#txtFechaLlegadaDocumento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#ddlEstado_busqueda").data("kendoDropDownList").value("1");
        $("#txtPersonaFechaFinLaboresElim").kendoDatePicker({ format: "dd/MM/yyyy" }); // max: new Date()

        $("#swDirector").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });
        $("#swFed").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });

        $("#swSindicato").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });
        $("#swDDJJ").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });
        $("#swFFAA").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });
        $("#swDeportista").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });
        $("#swDiscapacidad").kendoSwitch({
            messages: {
                checked: "SI",
                unchecked: "NO"
            }
        });
        //$("#visible-switch").kendoSwitch({
        //    checked: true
        //});

        this.inicializarIndicadores();
        this.inicializarGridMaestro();
    };
    this.NominaJS.prototype.inicializar = function () {

        /* BUSQUEDA */
        $("#ddlEstado_busqueda").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Nomina/ListarEstados",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlCondicion_busqueda").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdCondicion",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Condicion/ListarCondicion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        data_param.perfil = $("#hdPerfil").val();

                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlSede_busqueda").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlDependencia_busqueda").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        //$("#ddlEmpleado_busqueda").kendoDropDownList({
        //    autoBind: true,
        //    cascadeFrom: "ddlDependencia_busqueda",
        //    optionLabel: "--Todos--",
        //    dataTextField: "NombreCompleto",
        //    dataValueField: "IdEmpleado",
        //    filter: "contains",
        //    minLength: 3,
        //    dataSource: {
        //        serverFiltering: true,
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Empleado/ListarEmpleados",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            },
        //            parameterMap: function ($options, $operation) {
        //                var data_param = {};
        //                if ($options.filter != undefined && $options.filter.filters.length > 0)
        //                    data_param.Nombre = $options.filter.filters[0].value;

        //                data_param.IdDependencia = $('#ddlDependencia_busqueda').data("kendoDropDownList").value();
        //                data_param.Estado = -1
        //                data_param.IdEmpleado = 0
        //                data_param.Grilla = {};
        //                data_param.Grilla.RegistrosPorPagina = 0;
        //                data_param.Grilla.OrdenarPor = "Nombre";
        //                data_param.Grilla.OrdenarDeForma = "ASC";

        //                if (data_param.Nombre == data_param.IdDependencia) data_param.Nombre = '';
        //                return $.toDictionary(data_param);
        //            }
        //        }
        //    }
        //});

        /* REGISTRO */
        $("#ddlAnio").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Anio",
            dataValueField: "Anio",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Boletas/ListarAnios",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlMes").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Boletas/ListarMeses",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        /* CONSULTA */
        $('#divModalCesePersona').kendoWindow({
            draggable: true,
            modal: true,
            pinned: true,
            resizable: false,
            width: '30%',
            height: '200px',
            title: 'Confirmar eliminación',
            visible: false,
            position: { top: '5%', left: "35%" },
            actions: ["Close"],
            close: function () {
            }
        }).data("kendoWindow");

        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarTipoDeDocumento",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaSexo").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarSexo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstadoCivil").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstadoCivil",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaGrupoSan").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarGrupoSanguineo",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaCondicion").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdCondicion",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Condicion/ListarCondicion",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        data_param.perfil = $("#hdPerfil").val();

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            },
            change: function (e) {
                var estado = this.value();
                if (estado == '5') {
                    $("#divOrdenes").show();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaCargo").removeAttr("required");
                    $("#txtRUC").attr("required", true);
                }
                else {
                    $("#divOrdenes").hide();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaCargo").attr("required", true);
                    $("#txtRUC").removeAttr("required");
                }
            }
        });
        $("#ddlPersonaSede").kendoDropDownList({
            autoBind: false,
            optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "IdSede",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Sede/ListarSedes",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaEstado").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Seleccione--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Persona/ListarEstado",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            },
            change: function (e) {
                debugger;
                var estado = this.value();

                if (estado == '1') {
                    //$("#divCese").hide();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaFechaFinLabores").removeAttr("required");
                    $("#txtPersonaFechaFinLabores").val('');
                }
                if (estado == '0') {
                    //$("#divCese").show();
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaFechaFinLabores").attr("required", true);
                    $("#txtPersonaFechaFinLabores").attr("validationmessage", "requerido");
                }
            }
        });
        $("#ddlPersonaDependencia").kendoDropDownList({
            autoBind: true,
            optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "IdDependencia",
            filter: "contains",
            minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    },
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($options.filter != undefined && $options.filter.filters.length > 0)
                            data_param.term = $options.filter.filters[0].value;

                        //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
                        return $.toDictionary(data_param);
                    }
                }
            }
        });
        $("#ddlPersonaTipoPension").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDePension",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }//,
            //change: function (e) {
            //    var estado = this.value();
            //    if (estado == '1') {
            //        $("#txtPersonaNombrePension").show();
            //        $("#txtPersonaNombrePension").attr("required", true);
            //    }
            //    else {
            //        $("#txtPersonaNombrePension").hide();
            //        $("#txtPersonaNombrePension").removeAttr("required");
            //    }
            //}
        });
        $("#ddlPersonaTipoAFP").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeAFP",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlPersonaTipoComisionAFP").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoDeComisionAFP",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        $('#divModalRegistroPersona').kendoWindow({
            draggable: true,
            modal: true,
            pinned: true,
            resizable: false,
            width: '90%',
            height: 'auto',
            title: 'Agregar Trabajador',
            visible: false,
            position: { top: '5%', left: "5%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"],
            close: function () {
                frmPersonaValidador.hideMessages();

                $("#hdIdEmpleado").val('0');
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value('');
                $("#txtPersonaNumeroDeDocumento").val('');
                $("#chkPersonaDniValidoEnReniec").prop("checked", false);
                $("#txtPersonaNombres").val('');
                $("#txtPersonaApellidoPaterno").val('');
                $("#txtPersonaApellidoMaterno").val('');
                $("#ddlPersonaSexo").data("kendoDropDownList").value('');
                $("#ddlPersonaEstado").data("kendoDropDownList").value('1');
                //$("#txtPersonaFechaDeNacimiento").data("kendoDatePickerssms").value('');
                //$("#ddlPersonaIdUbigeoNacimiento").data("kendoDropDownList").value('');
                //$("#ddlPersonaIdUbigeoDomicilio").data("kendoDropDownList").value('');
                $("#txtPersonaDireccionDomicilio").val('');

                this.clearSelection();
            }
        }); //.data("kendoWindow");
        alert('inicio ')

        $("#ddlEpsEsSalud").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });
        $("#ddlSupsRet4taCat").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }//$("#txtNroAutorizacionExoneracion").prop('readonly', false);
            },
            change: function (e) {
                var value = this.value();

                if (value == "1") {
                    $("#txtNroAutorizacionExoneracion").prop('readonly', false);
                }
                else {
                    $("#txtNroAutorizacionExoneracion").prop('readonly', true);
                }
            }
        });
        $("#ddlDsctoJudicial").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
                }
            }
        });

        //$("#ddlEsDirector").kendoDropDownList({
        //    autoBind: true,
        //    dataTextField: "Nombre",
        //    dataValueField: "Codigo",
        //    dataSource: {
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            }
        //        }
        //    }
        //});

        //$("#ddlEncargatura").kendoDropDownList({
        //    autoBind: true,
        //    dataTextField: "Nombre",
        //    dataValueField: "Codigo",
        //    dataSource: {
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            }
        //        }
        //    },
        //    change: function (e) {
        //        debugger;
        //        var value = this.value();
        //        //if (value == "1") {
        //        //    $("#divDependenciaEncargatura").show();
        //        //}
        //        //else {
        //        //    $("#divDependenciaEncargatura").hide();
        //        //}
        //    }
        //});

        //$("#ddlFED").kendoDropDownList({
        //    autoBind: true,
        //    dataTextField: "Nombre",
        //    dataValueField: "Codigo",
        //    dataSource: {
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Empleado/ListarBooleanos",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            }
        //        }
        //    }
        //});

        //$("#ddlPersonaDependenciaEncargatura").kendoDropDownList({
        //    autoBind: true,
        //    optionLabel: "--Todos--",
        //    dataTextField: "Nombre",
        //    dataValueField: "IdDependencia",
        //    filter: "contains",
        //    minLength: 3,
        //    dataSource: {
        //        serverFiltering: true,
        //        transport: {
        //            read: {
        //                url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
        //                type: "GET",
        //                dataType: "json",
        //                cache: false
        //            },
        //            parameterMap: function ($options, $operation) {
        //                var data_param = {};
        //                if ($options.filter != undefined && $options.filter.filters.length > 0)
        //                    data_param.term = $options.filter.filters[0].value;

        //                //if (data_param.cod.length = 1) data_param.cod = '0' + data_param.cod;
        //                return $.toDictionary(data_param);
        //            }
        //        }
        //    }
        //});

        frmPersonaValidador = $("#frmRegistroPersona").kendoValidator().data("kendoValidator");

        $("#txtPersonaFechaNacimiento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#txtPersonaFechaInicioLabores").kendoDatePicker({ format: "dd/MM/yyyy" });
        $("#txtPersonaFechaFinLabores").kendoDatePicker({ format: "dd/MM/yyyy" }); // max: new Date()
        $("#txtFechaLlegadaDocumento").kendoDatePicker({ format: "dd/MM/yyyy", max: new Date() });
        $("#ddlEstado_busqueda").data("kendoDropDownList").value("1");

        this.inicializarGrid();
    };

    function LimpiarModalRegistroPersona() {
        var today = new Date();
        var month = today.getMonth();
        if (today.getMonth() > 1) var month = month - 1;
        var year = today.getFullYear();
        $("#txtPersonaFechaNacimiento").kendoDatePicker().data("kendoDatePicker").max(new Date());
        $("#txtPersonaFechaInicioLabores").kendoDatePicker().data("kendoDatePicker"); //.max(new Date());
        //$("#txtPersonaFechaFinLabores").kendoDatePicker().data("kendoDatePicker").max(new Date());
        document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,');

        $("#hdIdEmpleado").val(0);
        $("#txtPersonaNumeroDeDocumento").val('');
        $("#txtPersonaNombres").val('');
        $("#txtPersonaApellidoPaterno").val('');
        $("#txtPersonaApellidoMaterno").val('');
        $("#ddlPersonaSexo").data("kendoDropDownList").value(0);
        $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(0);
        $("#ddlPersonaGrupoSan").data("kendoDropDownList").value(0);
        $("#txtPersonaUbigeo").val('');
        $("#txtPersonaDireccionDomicilio").val('');
        $("#ddlPersonaCondicion").data("kendoDropDownList").value(0);
        $("#ddlPersonaSede").data("kendoDropDownList").value(0);
        $("#txtRUC").val('');
        $("#txtPersonaFechaNacimiento").val('');
        $("#txtPersonaFechaInicioLabores").val('');
        $("#txtPersonaFechaFinLabores").val('');
        $("#ddlPersonaDependencia").data("kendoDropDownList").value(0);
        $("#txtPersonaCargo").val('');
        $("#txtPersonaCorreoElectronicoP").val('');
        $("#txtTelefonoCelularP").val('');
        $("#txtTelefonoFijoP").val('');
        $("#txtPersonaCorreoElectronico").val('');
        $("#txtTelefonoCelular").val('');
        $("#txtTelefonoLaboral").val('');
        $("#txtTelefonoAnexo").val('');
        $('#hdFoto').val('');
        $("#hdIdUbigeoEmpleado").val('');
        
        $("#hdIdTieneContrato").val('');
        $("#txtPersonaAIRHSP").val('');
        $("#txtPersonaMeta").val('');
        $("#txtPersonaContrato").val('');
        $("#txtPersonaProceso").val('');

        $("#divExiste").hide();
        $("#btnBuscarFoto").hide();

        //$("#ddlPersonaTipoPension").data("kendoDropDownList").value(3);
        //OnChangeTieneAFP();

        if ($("#divGridCuentas").data('kendoGrid') != null)
            $("#divGridCuentas").data('kendoGrid').dataSource.data([]);
        //if ($("#divGridOrdenes").data('kendoGrid') != null)
        //    $("#divGridOrdenes").data('kendoGrid').dataSource.data([]);
        if ($("#divGridHistorial").data('kendoGrid') != null)
            $("#divGridHistorial").data('kendoGrid').dataSource.data([]);
        if ($("#divGridEncargaturas").data('kendoGrid') != null)
            $("#divGridEncargaturas").data('kendoGrid').dataSource.data([]);
    }

    //this.NominaJS.prototype.crearCuentaBancaria = function (e) {
    //    if ($('#divGridNudosCriticos').data("kendoGrid").dataSource.data().length >= NudosCriticosLimite) {
    //        controladorApp.notificarMensajeDeAlerta("No se puede agregar más de " + NudosCriticosLimite + " nudos críticos por compromiso.");
    //        e.preventDefault();
    //        return false;
    //    }
    //}
    this.NominaJS.prototype.inicializarIndicadores = function () {
        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Empleado/ObtenerIndicadores',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            //data: data_param,
            success: function (res) {
                debugger;
                //alert(res);
                $("#hActivo").append(res.split('|')[0]); //innerHTML
                $("#hSindicato").append(res.split('|')[1]);
            },
            error: function (res) {
                controladorApp.notificarMensajeDeAlerta(res.responseText);
            }
        });
    }
    this.NominaJS.prototype.inicializarGridMaestro = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarEmpleadosMaestro',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = $("#ddlEstado_busqueda").data("kendoDropDownList").value();
                        data_param.IdDependencia = $("#ddlDependencia_busqueda").data("kendoDropDownList").value();
                        data_param.IdSede = $("#ddlSede_busqueda").data("kendoDropDownList").value();
                        data_param.NroDocumento = $("#txtDNI_busqueda").val();
                        data_param.Nombre = $("#txtEmpleado_busqueda").val().toUpperCase();

                        if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO)
                            data_param.IdCondicion = "5";
                        else
                            data_param.IdCondicion = $("#ddlCondicion_busqueda").data("kendoDropDownList").value();

                        data_param.IdEmpleado = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleado"
                }
            },
            group: {
                field: "NombreOficina", aggregates: [
                    { field: "NombreOficina", aggregate: "count" }
                ]
            },
            aggregate: [
                { field: "NombreCompleto", aggregate: "count" },
                { field: "NombreOficina", aggregate: "count" }
            ]
        });
        debugger;
        
            debugger;
            this.$grid = $("#divGrid").kendoGrid({
                toolbar: ["excel",], //{ template: kendo.template('<div>Esto es una prueba ' + this.$dataSource.total() + ' </div>') }
                excel: {
                    fileName: "Listado de Empleados.xlsx",
                    filterable: false
                },
                dataSource: this.$dataSource,
                autoBind: true,
                selectable: true,
                scrollable: false,
                sortable: false,
                pageable: false,
                groupable: true,

                dataType: 'json',
                columns: [
                    //{
                    //    field: "IdEmpleado",
                    //    title: "ID",
                    //    attributes: { style: "text-align:right;" },
                    //    width: "30px"                    
                    //},
                    {
                        field: "NroDocumento",
                        title: "DNI",
                        attributes: { style: "text-align:center;" },
                        width: "30px"
                    },
                    {
                        field: "NombreCompleto",
                        title: "NOMBRE",
                        width: "200px",
                        aggregates: ["count"],
                        footerTemplate: "Total: #= count#"
                    },
                    {
                        field: "CondicionLaboral",
                        title: "CONDICIÓN",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "TipoIngreso",
                        title: "FORMA DE INGRESO",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "FechaInicio",
                        title: "INICIO DE LABORES",
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            return kendo.toString(kendo.parseDate(item.FechaInicio), 'dd/MM/yyyy');
                        },
                        width: "50px"
                    },
                    {
                        field: "NroContrato",
                        title: "Nro CONTRATO",
                        attributes: { style: "text-align:center;" },
                        width: "50px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "Sede",
                        title: "SEDE",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "CorreoElectronicoLaboral",
                        title: "EMAIL",
                        //attributes: { style: "text-align:center;" },
                        width: "50px"
                    },
                    {
                        field: "NombreOficina",
                        title: "DEPENDENCIA",
                        width: "300px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "NombreCargo",
                        title: "CARGO",
                        width: "300px"
                    },
                    {
                        //field: "FechaInicio",
                        title: "",
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            var cadena = '';
                            if (item.TieneJefatura == 1)
                                cadena = '<h6><span class="badge badge-light" style="background-color:#ffc09f">&nbsp;&nbsp;J&nbsp;&nbsp;</span></h6>';
                            if (item.TieneEncargatura == 1)
                                cadena += '<h6><span class="badge badge-light" style="background-color:#ffee93">&nbsp;&nbsp;E&nbsp;&nbsp;</span></h6>';

                            return cadena;
                        },
                        //
                        width: "50px"
                    },
                    {
                        field: "EstadoNombre",
                        title: "ESTADO",
                        attributes: { style: "text-align:center;" },
                        width: "30px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        title: '',
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            var controles = "";
                            controles += '<button type="button" class="btn btn-light btn-xs" onclick="controlador.generarFormatoFicha(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                            controles += '<span class="glyphicon glyphicon-file" aria-hidden="true" data-uib-tooltip="Editar" title="Exportar información del empleado"></span>';
                            controles += '</button>';
                            
                            return controles;
                        },
                        width: '30px'
                    },
                    {
                        //INGRESAR DETALLE DE LA EVALUACION
                        title: '',
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            var controles = "";
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalRegistroPersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                            controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del empleado"></span>';
                            controles += '</button>';

                            return controles;
                        },
                        width: '30px'
                    }
                ],
                excelExport: function (e) {
                    var sheet = e.workbook.sheets[0];
                    var template = kendo.template(this.columns[4].template);

                    //alert(template);
                    for (var i = 1; i < sheet.rows.length; i++) {
                        debugger;
                        var row = sheet.rows[i];
                        if (row.cells[5] != undefined) {
                            if (row.cells[5].value != undefined) {
                                if (row.cells[5].value.toString().startsWith('/Date')) {
                                    var dataItem = { FechaInicio: row.cells[5].value };
                                    row.cells[5].value = template(dataItem);
                                }
                                else {
                                    if (row.cells[4].value != undefined) {
                                        if (row.cells[4].value.toString().startsWith('/Date')) {
                                            var dataItem2 = { FechaInicio: row.cells[4].value };
                                            row.cells[4].value = template(dataItem2);
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }).data();
        //}
    };
    this.NominaJS.prototype.generarFormatoFicha = function (idEmpleado, nroDNI) {
        window.open(controladorApp.obtenerRutaBase() + "Nomina/FichaPersonal?IdEmpleado=" + idEmpleado + "&NroDocumento=" + nroDNI, "_blank");
    }

    this.NominaJS.prototype.inicializarGrid = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarEmpleados',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        data_param.Estado = $("#ddlEstado_busqueda").data("kendoDropDownList").value();
                        data_param.IdDependencia = $("#ddlDependencia_busqueda").data("kendoDropDownList").value();
                        data_param.IdSede = $("#ddlSede_busqueda").data("kendoDropDownList").value();
                        data_param.NroDocumento = $("#txtDNI_busqueda").val();
                        data_param.Nombre = $("#txtEmpleado_busqueda").val().toUpperCase();

                        if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO) 
                            data_param.IdCondicion = "5";
                        else
                            data_param.IdCondicion = $("#ddlCondicion_busqueda").data("kendoDropDownList").value();

                        data_param.IdEmpleado = 0; //$("#ddlEmpleado_busqueda").data("kendoDropDownList").value();
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblTotal").html(this.total());
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleado"
                }
            },
            group: {
                field: "NombreOficina", aggregates: [
                   { field: "NombreOficina", aggregate: "count" }
                ]
            },
            aggregate: [
                    { field: "NombreCompleto", aggregate: "count" },
                    { field: "NombreOficina", aggregate: "count" }
            ]
        });
        debugger;
        if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO) {
            
            this.$grid = $("#divGrid").kendoGrid({
                toolbar: ["excel", ],
                excel: {
                    fileName: "Listado de Empleados.xlsx",
                    filterable: false
                },
                dataSource: this.$dataSource,
                autoBind: true,
                selectable: true,
                scrollable: false,
                sortable: false,
                pageable: false,
                groupable: true,

                dataType: 'json',
                columns: [
                    //{
                    //    field: "IdEmpleado",
                    //    title: "ID",
                    //    attributes: { style: "text-align:right;" },
                    //    width: "30px"                    
                    //},
                    {
                        field: "NroDocumento",
                        title: "DNI",
                        attributes: { style: "text-align:center;" },
                        width: "30px"
                    },
                    {
                        field: "NombreCompleto",
                        title: "EMPLEADO",
                        width: "200px",
                        aggregates: ["count"],
                        footerTemplate: "Total: #= count#"
                    },
                    {
                        field: "CondicionLaboral",
                        title: "CONDICIÓN",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "Sede",
                        title: "SEDE",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "NombreOficina",
                        title: "DEPENDENCIA",
                        width: "300px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "NroOrden",
                        title: "Nro OS",
                        attributes: { style: "text-align:center;" },
                        width: "30px"
                    },
                    {
                        field: "NombreOrden",
                        title: "NOMBRE OS",
                        width: "200px"
                    },
                    {
                        field: "DuracionOrden",
                        title: "DURACIÓN",
                        attributes: { style: "text-align:right;" },
                        width: "30px"
                    },
                    {
                        field: "InicioOrden",
                        title: "INICIO",
                        attributes: { style: "text-align:center;" },
                        width: "30px"
                    },
                    {
                        field: "FinOrden",
                        title: "FIN",
                        attributes: { style: "text-align:center;" },
                        width: "30px"
                    },
                    {
                        field: "MontoOrden",
                        title: "MONTO",
                        attributes: { style: "text-align:right;" },
                        format: "{0:c}",
                        width: "30px"
                    },
                    {
                        //INGRESAR DETALLE DE LA EVALUACION
                        title: '',
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            var controles = "";
                            if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO) {
                                if (item.IdCondicion == 5) {
                                    controles += '<button type="button" class="btn btn-default btn-sm" onclick="controlador.abrirModalRegistroPersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                                    controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del empleado"></span>';
                                    controles += '</button>';
                                }
                            }
                            else {
                                controles += '<button type="button" class="btn btn-default btn-sm" onclick="controlador.abrirModalRegistroPersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                                controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del empleado"></span>';
                                controles += '</button>';
                            }

                            return controles;
                        },
                        width: '30px'
                    }
                ]
            }).data();
        }
        else {
            debugger;
            this.$grid = $("#divGrid").kendoGrid({
                toolbar: ["excel", ], //{ template: kendo.template('<div>Esto es una prueba ' + this.$dataSource.total() + ' </div>') }
                excel: {
                    fileName: "Listado de Empleados.xlsx",
                    filterable: false
                },
                dataSource: this.$dataSource,
                autoBind: true,
                selectable: true,
                scrollable: false,
                sortable: false,
                pageable: false,
                groupable: true,

                dataType: 'json',
                columns: [
                    //{
                    //    field: "IdEmpleado",
                    //    title: "ID",
                    //    attributes: { style: "text-align:right;" },
                    //    width: "30px"                    
                    //},
                    {
                        field: "NroDocumento",
                        title: "DNI",
                        attributes: { style: "text-align:center;" },
                        width: "30px"                    
                    },
                    {
                        field: "NombreCompleto",
                        title: "EMPLEADO",
                        width: "200px",
                        aggregates: ["count"],
                        footerTemplate: "Total: #= count#"
                    },
                    {
                        field: "CondicionLaboral",
                        title: "CONDICIÓN",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"                    
                    },
                    {
                        //field: "FechaInicio",
                        title: "INICIO DE LABORES",
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            return kendo.toString(kendo.parseDate(item.FechaInicio), 'dd/MM/yyyy');
                        },
                        //
                        width: "50px"
                    },
                    {
                        field: "Sede",
                        title: "SEDE",
                        width: "100px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "NombreOficina",
                        title: "DEPENDENCIA",
                        width: "300px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        field: "CorreoElectronicoLaboral",
                        title: "EMAIL LABORAL",
                        //attributes: { style: "text-align:center;" },
                        width: "50px"
                    },
                    {
                        field: "NombreCargo",
                        title: "CARGO",
                        width: "300px"
                    },
                    {
                        field: "EstadoNombre",
                        title: "ESTADO",
                        attributes: { style: "text-align:center;" },
                        width: "30px",
                        aggregates: ["count"],
                        groupHeaderTemplate: "#= value # (Total: #= count#)"
                    },
                    {
                        //INGRESAR DETALLE DE LA EVALUACION
                        title: '',
                        attributes: { style: "text-align:center;" },
                        template: function (item) {
                            var controles = "";
                            if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO) {
                                if (item.IdCondicion == 5) {
                                    controles += '<button type="button" class="btn btn-info btn-sm" onclick="controlador.abrirModalRegistroPersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                                    controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del empleado"></span>';
                                    controles += '</button>';
                                }                            
                            }
                            else {
                                controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalRegistroPersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                                controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del empleado"></span>';
                                controles += '</button>';
                            }
                        
                            return controles;
                        },
                        width: '30px'
                    }
                ]
            }).data();
        }
    };
    
    this.NominaJS.prototype.buscar = function (e) {
        e.preventDefault();

        var grilla = $('#divGrid').data("kendoGrid");
        grilla.dataSource._sort = undefined;
        grilla.dataSource.page(1);

        //$("#lblTotal").html(grilla.dataSource.total());
    };

    this.NominaJS.prototype.abrirModalRegistroPersona = function (id, documento) {
        var modal = $('#divModalRegistroPersona').data('kendoWindow');
        debugger;
        //$("#divContrato").hide();
        LimpiarModalRegistroPersona();

        if (id == 0) {           
            $("#ddlPersonaEstado").data("kendoDropDownList").value('1');
            $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value('01');
            //$('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();

            $("#divCese1").hide();
            $("#divCese2").hide();
            //$('#_tab4').unbind();
            //$("#_tab4").removeAttr("data-toggle");
            //$('#_tab4').click(function () {
            //    controladorApp.notificarMensajeDeAlerta('Primero debe guardar los datos del trabajador');
            //})

            $("#btnGuardarContacto").hide();

            $("#divActivo").show();
            $("#divInactivo").hide();

            //$('#tab2').removeClass('in active');
            //$('#tab3').removeClass('in active');
            //$('#tab4').removeClass('in active');
            //$('#tab5').removeClass('in active');
            //$('#tab6').removeClass('in active');
            //$('#tab7').removeClass('in active');
            //$('#tab1').addClass('in active');
            //$('#liTab2').removeClass('active');
            //$('#liTab3').removeClass('active');
            //$('#liTab4').removeClass('active');
            //$('#liTab5').removeClass('active');
            //$('#liTab6').removeClass('active');
            //$('#liTab7').removeClass('active');
            //$('#liTab1').addClass('active');
            
            //if ($("#hdPerfil").val() == PERFIL_NOMINA_CONTACTO) {
            //    $('#txtPersonaCorreoElectronicoP').prop('readonly', true);
            //    $('#txtTelefonoCelularP').prop('readonly', true);
            //    $('#txtTelefonoFijoP').prop('readonly', true);
            //    $('#txtPersonaCorreoElectronico').prop('readonly', false);
            //    $('#txtTelefonoCelular').prop('readonly', false);
            //    $('#txtTelefonoLaboral').prop('readonly', false);
            //    $('#txtTelefonoAnexo').prop('readonly', false);
            //}
            //else {
            //    $('#txtPersonaCorreoElectronicoP').prop('readonly', false);
            //    $('#txtTelefonoCelularP').prop('readonly', false);
            //    $('#txtTelefonoFijoP').prop('readonly', false);
            //    $('#txtPersonaCorreoElectronico').prop('readonly', true);
            //    $('#txtTelefonoCelular').prop('readonly', true);
            //    $('#txtTelefonoLaboral').prop('readonly', true);
            //    $('#txtTelefonoAnexo').prop('readonly', true);
            //}

            controlador.CargarFormularioCuentasBancarias(-1);
            controlador.CargarFormularioDesplazamiento(-1);
            controlador.CargarFormularioEncargaturas(-1);
            controlador.CargarFormularioFamiliares(-1);
            controlador.CargarFormularioEstudios(-1);
            //controlador.CargarFormularioOrdenesServicio(-1);
            //if ($("#ddlPersonaCondicion").data("kendoDropDownList").value() == '5'){
            //    $("#divOrdenes").show();
            //    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
            //    $("#txtPersonaCargo").removeAttr("required");
            //    $("#txtRUC").attr("required", true);
            //} 
            //else {
                //$("#divOrdenes").hide();
                //$('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                $("#txtPersonaCargo").attr("required", true);
                $("#txtRUC").removeAttr("required");
            //}

            if ($("#btnGuardarAdmin") != null) $("#btnGuardarAdmin").text("Guardar");
            if ($("#btnGuardarAbastecimiento") != null) $("#btnGuardarAbastecimiento").text("Guardar");
            if ($("#btnGuardarContacto") != null) $("#btnGuardarContacto").text("Guardar");

            //$("#pnlEmpleado").kendoPanelBar({
            //    expandMode: "single"
            //});
            var panelBar = $("#pnlEmpleado").data("kendoPanelBar");
            if (panelBar != undefined) {
                panelBar.clearSelection();
                panelBar.expand($("#liPersonales"), false);
                panelBar.select("#liPersonales");
            }

            modal.title("Agregar Trabajador");
            modal.open(); //.center();
        }
        else {
            $("#hdIdEmpleado").val(id);

            controlador.CargarFormularioTrabajador(id);
            controlador.CargarFormularioCuentasBancarias(id);
            controlador.CargarFormularioDesplazamiento(documento);
            controlador.CargarFormularioEncargaturas(documento);
            controlador.CargarFormularioFamiliares(documento);
            controlador.CargarFormularioEstudios(documento);
            //controlador.CargarFormularioOrdenesServicio(id);

            //$("#pnlEmpleado").kendoPanelBar({
            //    expandMode: "single"
            //});
            var panelBar = $("#pnlEmpleado").data("kendoPanelBar");
            if (panelBar != undefined) {
                panelBar.clearSelection();
                panelBar.expand($("#liPersonales"), false);
                panelBar.select("#liPersonales");
            }
            modal.title("Actualizar Información de Trabajador");
            modal.open(); //.center();
        }
    }

    this.NominaJS.prototype.cerrarModalRegistroPersona = function () {
        var modal = $('#divModalRegistroPersona').data('kendoWindow');
        modal.close();
    }

    this.NominaJS.prototype.CargarFormularioTrabajador = function (id) {
        debugger;
        var data_param = new FormData();
        data_param.append('IdEmpleado', id);
        
        if (($("#hdPerfil").val() == PERFIL_NOMINA_CONTACTO) || ($("#hdPerfil").val() == PERFIL_NOMINA_CONTABILIDAD)) {
            //if ($("#hdPerfil").val() == PERFIL_NOMINA_CONTACTO) {
            //    $('#tab1').removeClass('in active');
            //    $('#tab2').removeClass('in active');
            //    $('#tab4').removeClass('in active');
            //    $('#tab5').removeClass('in active');
            //    $('#tab3').addClass('in active');
            //    $('#liTab1').removeClass('active');
            //    $('#liTab2').removeClass('active');
            //    $('#liTab5').removeClass('active');
            //    $('#liTab3').addClass('active');
            //}
            //if ($("#hdPerfil").val() == PERFIL_NOMINA_CONTABILIDAD) {
            //    $('#tab1').removeClass('in active');
            //    $('#tab2').removeClass('in active');
            //    $('#tab3').removeClass('in active');
            //    $('#tab4').addClass('in active');
            //    $('#tab5').removeClass('in active');
            //    $('#liTab1').removeClass('active');
            //    $('#liTab2').removeClass('active');
            //    $('#liTab4').addClass('active');
            //    $('#liTab5').removeClass('active');
            //}
            $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").readonly();
            $('#txtPersonaNumeroDeDocumento').prop('readonly', true);
            $('#btnBuscarP').prop('disabled', true);
            $('#ddlPersonaSexo').data("kendoDropDownList").readonly();
            $('#ddlPersonaEstadoCivil').data("kendoDropDownList").readonly();
            $('#ddlPersonaGrupoSan').data("kendoDropDownList").readonly();
            $('#ddlPersonaEstado').data("kendoDropDownList").readonly();
            $('#ddlPersonaCondicion').data("kendoDropDownList").readonly();
            $('#ddlPersonaSede').data("kendoDropDownList").readonly();
            $('#txtPersonaFechaNacimiento').data("kendoDatePicker").readonly();
            $('#txtPersonaFechaInicioLabores').data("kendoDatePicker").readonly();
            $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
            $('#ddlPersonaDependencia').data("kendoDropDownList").readonly();
            $('#txtPersonaCargo').prop('readonly', true);
            $('#txtRUC').prop('readonly', true);

            $('#txtPersonaCorreoElectronicoP').prop('readonly', true);
            $('#txtTelefonoCelularP').prop('readonly', true);
            $('#txtTelefonoFijoP').prop('readonly', true);
            $('#txtPersonaCorreoElectronico').prop('readonly', true);
            $('#txtTelefonoCelular').prop('readonly', true);
            $('#txtTelefonoLaboral').prop('readonly', true);
            $('#txtTelefonoAnexo').prop('readonly', true);
            $('#ddlEpsEsSalud').data("kendoDropDownList").readonly();
            $('#ddlSupsRet4taCat').data("kendoDropDownList").readonly();
            $('#ddlDsctoJudicial').data("kendoDropDownList").readonly();
        }
        else {
            //$('#tab2').removeClass('in active');
            //$('#tab3').removeClass('in active');
            //$('#tab4').removeClass('in active');
            //$('#tab5').removeClass('in active');
            //$('#tab6').removeClass('in active');
            //$('#tab7').removeClass('in active');
            //$('#tab1').addClass('in active');
            //$('#liTab2').removeClass('active');
            //$('#liTab3').removeClass('active');
            //$('#liTab4').removeClass('active');
            //$('#liTab5').removeClass('active');
            //$('#liTab6').removeClass('active');
            //$('#liTab7').removeClass('active');
            //$('#liTab1').addClass('active');
            $('#ddlPersonaTipoDeDocumento').data("kendoDropDownList").enable(true);
            $('#txtPersonaNumeroDeDocumento').prop('readonly', false);
            $('#btnBuscarP').prop('disabled', false);
            $('#ddlPersonaSexo').data("kendoDropDownList").enable(true);
            $('#ddlPersonaEstadoCivil').data("kendoDropDownList").enable(true);
            $('#ddlPersonaGrupoSan').data("kendoDropDownList").enable(true);
            $('#ddlPersonaEstado').data("kendoDropDownList").enable(true);
            $('#ddlPersonaCondicion').data("kendoDropDownList").enable(true);
            $('#ddlPersonaSede').data("kendoDropDownList").enable(true);
            $('#txtPersonaFechaNacimiento').data("kendoDatePicker").enable(true);
            $('#txtPersonaFechaInicioLabores').data("kendoDatePicker").enable(true);
            $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
            $('#ddlPersonaDependencia').data("kendoDropDownList").enable(true);
            $('#txtPersonaCargo').prop('readonly', false);
            $('#txtRUC').prop('readonly', false);

            $('#txtPersonaCorreoElectronicoP').prop('readonly', false);
            $('#txtTelefonoCelularP').prop('readonly', false);
            $('#txtTelefonoFijoP').prop('readonly', false);
            $('#txtPersonaCorreoElectronico').prop('readonly', false);
            $('#txtTelefonoCelular').prop('readonly', false);
            $('#txtTelefonoLaboral').prop('readonly', false);
            $('#txtTelefonoAnexo').prop('readonly', false);
            //$('#ddlEpsEsSalud').data("kendoDropDownList").enable(true);;

            //if ($("#hdPerfil").val() == PERFIL_NOMINA_ABASTECIMIENTO)
            //    $("#txtPersonaCargo").removeAttr("required");
            //else {
            //    $("#txtPersonaCargo").attr("required", true);
            //    $("#txtPersonaCargo").attr("validationmessage", "requerido");
            //}
        }

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Empleado/ObtenerParaEditar',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                debugger;
                $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value(res.TipoDocumento);
                $("#txtPersonaNumeroDeDocumento").val(res.NroDocumento);
                $("#txtPersonaNombres").val(res.Nombre);
                $("#txtPersonaApellidoPaterno").val(res.Paterno);
                $("#txtPersonaApellidoMaterno").val(res.Materno);
                $("#ddlPersonaSexo").data("kendoDropDownList").value(res.IdGenero);
                $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value(res.IdEstadoCivil);
                $("#ddlPersonaGrupoSan").data("kendoDropDownList").value(res.IdGrupoSanguineo);
                $("#txtPersonaDireccionDomicilio").val(res.Domicilio);
                $("#txtPersonaUbigeo").val(res.DescripcionUbigeo);
                $("#txtPersonaCorreoElectronicoP").val(res.CorreoElectronico);
                $("#txtTelefonoCelularP").val(res.Celular);
                $("#txtTelefonoFijoP").val(res.Telefono);
                $("#txtPersonaCorreoElectronico").val(res.CorreoElectronicoLaboral);
                $("#txtTelefonoCelular").val(res.CelularLaboral);
                $("#txtTelefonoLaboral").val(res.TelefonoLaboral);
                $("#txtTelefonoAnexo").val(res.AnexoLaboral);
                $("#ddlPersonaEstado").data("kendoDropDownList").value(res.Estado);
                $("#ddlPersonaCondicion").data("kendoDropDownList").value(res.IdCondicion);
                //$("#ddlEsDirector").data("kendoDropDownList").value(res.Director);
                $("#swDirector").data("kendoSwitch").check((res.Director == 1 ? true : false));
                $("#swFed").data("kendoSwitch").check(res.FED);
                $("#swSindicato").data("kendoSwitch").check(res.Sindicato);
                $("#swDDJJ").data("kendoSwitch").check(res.DDJJ);
                $("#swFFAA").data("kendoSwitch").check(res.FFAA);
                $("#swDeportista").data("kendoSwitch").check(res.Deportista);
                $("#swDiscapacidad").data("kendoSwitch").check(res.Discapacidad);
                if (res.Estado == 1) {
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaFechaFinLabores").removeAttr("required");
                    $("#txtPersonaFechaFinLabores").val('');
                    $("#divActivo").show();
                    $("#divInactivo").hide();
                    $("#divCese1").hide();
                    $("#divCese2").hide();
                }//$("#divCese").hide();
                if (res.Estado == 0) {
                    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                    $("#txtPersonaFechaFinLabores").attr("required", true);
                    $("#txtPersonaFechaFinLabores").attr("validationmessage", "requerido");
                    $("#txtPersonaFechaFinLabores").data("kendoDatePicker").value(res.FechaCese);
                    $("#divInactivo").show();
                    $("#divActivo").hide();
                    $("#divCese1").show();
                    $("#divCese2").show();
                }//$("#divCese").show();

                //if (res.IdCondicion == '5') {
                //    $("#divOrdenes").show();
                //    $('#txtPersonaFechaFinLabores').data("kendoDatePicker").enable(true);
                //    $("#txtPersonaCargo").removeAttr("required");
                //    $("#txtRUC").attr("required", true);
                //}
                //else {
                //  $("#divOrdenes").hide();
                //  $('#txtPersonaFechaFinLabores').data("kendoDatePicker").readonly();
                    $("#txtPersonaCargo").attr("required", true);
                    $("#txtRUC").removeAttr("required");
                //} 

                //if (res.TieneContrato == 1) {
                //    //$("#divContrato").show();
                //    $("#hdIdTieneContrato").val('1');
                //    $("#hdIdContrato").val(res.IdContrato);
                //    $("#txtPersonaAIRHSP").val(res.NroAIRHSP);
                //    $("#txtPersonaMeta").val(res.Meta);
                //    $("#txtPersonaContrato").val(res.NroContrato);
                //    $("#txtPersonaProceso").val(res.NombreProceso);
                //}
                //else {
                //    //$("#divContrato").hide();
                //    $("#hdIdContrato").val('0');
                //    $("#hdIdTieneContrato").val('0');
                //}

                $("#ddlPersonaSede").data("kendoDropDownList").value(res.IdSede);
                $("#ddlPersonaDependencia").data("kendoDropDownList").value(res.IdDependencia);
                debugger;
                $("#hdIdDependencia").val(res.IdDependencia);
                $("#txtPersonaCargo").val(res.NombreCargo);
                $("#txtRUC").val(res.RUC);
                $("#hdIdUbigeoEmpleado").val(res.Ubigeo);
                
                $("#txtPersonaFechaNacimiento").data("kendoDatePicker").value(kendo.parseDate(res.FechaNacimiento));
                $("#txtPersonaFechaInicioLabores").data("kendoDatePicker").value(res.FechaInicio);

                $("#ddlPersonaTipoPension").data("kendoDropDownList").value(res.IdTipoPension);
                $("#ddlPersonaTipoAFP").data("kendoDropDownList").value(res.IdTipoAfp);
                $("#txtPersonaCodigoAfp").val(res.CodigoAfp);
                $("#ddlPersonaTipoComisionAFP").data("kendoDropDownList").value(res.IdTipoComisionAfp);

                $("#ddlPersonaTipoIngreso").data("kendoDropDownList").value(res.IdTipoIngreso);
                $("#ddlPersonaTipoContrato").data("kendoDropDownList").value(res.IdTipoContrato);
                $("#txtPersonaAIRHSP").val(res.NroAIRHSP);
                $("#txtPersonaProceso").val(res.DocIngreso);
                $("#txtPersonaCese").val(res.DocCese);
                $("#txtPersonaMeta").val(res.Meta);
                $("#txtPersonaContrato").val(res.NroContrato);
                $("#txtPersonaSalario").val(res.Remuneracion);
                $("#ddlPersonaModalidad").data("kendoDropDownList").value(res.IdTipoModalidad);

                if (res.Foto != '') {
                    document.getElementById('imgFoto').setAttribute('src', 'data:image/png;base64,' + res.Foto);
                    $('#hdFoto').val(res.Foto);
                }
                else {
                    if (($("#hdPerfil").val() != PERFIL_NOMINA_CONTACTO) && ($("#hdPerfil").val() != PERFIL_NOMINA_CONTABILIDAD)) 
                        $("#btnBuscarFoto").show();
                }
                
                if (res.EPSEsSalud==true) $("#ddlEpsEsSalud").data("kendoDropDownList").value('1');
                else $("#ddlEpsEsSalud").data("kendoDropDownList").value('0');
                
                if (res.SupsRet4taCat == true) {
                    $("#ddlSupsRet4taCat").data("kendoDropDownList").value('1');
                    $("#txtNroAutorizacionExoneracion").prop('readonly', false);
                }
                else {
                    $("#ddlSupsRet4taCat").data("kendoDropDownList").value('0');
                    $("#txtNroAutorizacionExoneracion").prop('readonly', true);
                }
                if (res.DsctoJudicial == true) {
                    $("#ddlDsctoJudicial").data("kendoDropDownList").value('1');
                    $("#liTab6").show();
                }
                else {
                    $("#ddlDsctoJudicial").data("kendoDropDownList").value('0');
                    $("#liTab6").hide();
                }


                controlador.CargarFormularioDesplazamiento(res.NroDocumento);
                controlador.CargarFormularioEncargaturas(res.NroDocumento);
                controlador.CargarFormularioFamiliares(res.NroDocumento);
                controlador.CargarFormularioEstudios(res.NroDocumento);
                OnChangeTieneAFP();
                debugger;
                if (res.lstDescuentoJudicialBeneficiario_Registro != null && res.lstDescuentoJudicialBeneficiario_Registro.length > 0) {
                    $("#ddlBancos").kendoDropDownList({
                        autoBind: true,
                        optionLabel: "--Seleccione--",
                        dataTextField: "Nombre",
                        dataValueField: "IdBanco",
                        dataSource: {
                            serverFiltering: true,
                            transport: {
                                read: {
                                    url: controladorApp.obtenerRutaBase() + "DescuentoJudicial/ListarBancos",
                                    type: "GET",
                                    dataType: "json",
                                    cache: false
                                }

                            }
                        },
                        change: function (e) {
                            var value = this.value();

                            if (value > 1) {
                                $('#txtNumeroCuenta').prop('readonly', false);
                            }
                            else {
                                $('#txtNumeroCuenta').prop('readonly', true);
                            }

                        }

                    });

                    $("#ddlFormaPago").kendoDropDownList({
                        autoBind: true,
                        optionLabel: "--Seleccione--",
                        dataTextField: "NomFormaPago",
                        dataValueField: "CodFormaPago",
                        dataSource: {
                            serverFiltering: true,
                            transport: {
                                read: {
                                    url: controladorApp.obtenerRutaBase() + "DescuentoJudicial/ListarFormaPago",
                                    type: "GET",
                                    dataType: "json",
                                    cache: false
                                }

                            }
                        }
                    });

                    $("#ddlTipoRetencion").kendoDropDownList({
                        autoBind: true,
                        optionLabel: "--Seleccione--",
                        dataTextField: "NomTipoRetencion",
                        dataValueField: "CodTipoRetencion",
                        dataSource: {
                            serverFiltering: true,
                            transport: {
                                read: {
                                    url: controladorApp.obtenerRutaBase() + "DescuentoJudicial/ListarTipoRetencion",
                                    type: "GET",
                                    dataType: "json",
                                    cache: false
                                }

                            }
                        },
                        change: function (e) {
                            var value = this.value();

                            if (value > 1) {
                                //$('#divPorcentaje').show();
                                //$('#divMonto').hide();
                                $('#txtImporteFijo').val('');
                                $('#txtValorPorcentaje').prop('readonly', false);
                                $('#txtImporteFijo').prop('readonly', true);
                            }
                            else {
                                //$('#divPorcentaje').hide();
                                //$('#divMonto').show();
                                $('#txtValorPorcentaje').val('');
                                $('#txtImporteFijo').prop('readonly', false);
                                $('#txtValorPorcentaje').prop('readonly', true);
                            }
                        }
                    });

                    var cantBene = res.lstDescuentoJudicialBeneficiario_Registro.length;
                    debugger;
                    $("#lblNroBeneficiario").html(cantBene);
                    this.$grid = $("#divGridBeneficiarios").kendoGrid({
                        //toolbar: ["excel", ],
                        //excel: {
                        //    fileName: "Exportar Beneficiarios.xlsx",
                        //    filterable: false
                        //},
                        dataSource: res.lstDescuentoJudicialBeneficiario_Registro,
                        autoBind: true,
                        selectable: true,
                        scrollable: false,
                        sortable: false,
                        pageable: false,
                        groupable: false,
                        dataType: 'json',
                        columns: [
                            {
                                field: "iCodJudicial",
                                title: "COD. JUDICIAL",
                                width: "30px",
                                attributes: { style: "text-align:left;" },
                                hidden: true
                            },
                            {
                                field: "iCodJudicialDetalle",
                                title: "COD. DET JUDICIAL",
                                width: "30px",
                                attributes: { style: "text-align:left;" },
                                hidden: true
                            },
                            {
                                field: "vDniBeneficiario",
                                title: "Nª DNI",
                                width: "30px",
                                attributes: { style: "text-align:center;" }
                            },
                            {
                                field: "vNombreBeneficiario",
                                title: "BENEFICIARIO(A)",
                                width: "150px",
                                attributes: { style: "text-align:left;" }
                            },

                            {
                                field: "iCodigoBanco",
                                title: "COD.BANCO",
                                width: "10px",
                                hidden: true,
                                attributes: { style: "text-align:center;" }

                            },
                            {
                                field: "vNombreBanco",
                                title: "BANCO",
                                width: "100px",
                                attributes: { style: "text-align:center;" }
                            },
                            {
                                field: "vNumeroCuenta",
                                title: "NRO. CUENTA",
                                width: "50px",
                                attributes: { style: "text-align:center;" }
                            },
                            {
                                field: "iCodTipoRetencion",
                                title: "COD. RETENCION",
                                width: "20px",
                                attributes: { style: "text-align:center;" },
                                hidden: true

                            },
                            {
                                field: "vNombreRetencion",
                                title: "TIPO RETENCION",
                                width: "50px",
                                attributes: { style: "text-align:center;" }
                            },
                            {
                                field: "iCodFormaPago",
                                title: "COD. FORMA PAGO",
                                width: "20px",
                                attributes: { style: "text-align:center;" },
                                hidden: true

                            },
                            {
                                field: "vNombreFormaPago",
                                title: "FORMA PAGO",
                                width: "50px",
                                attributes: { style: "text-align:center;" }
                            },
                            {
                                field: "dValorPorcentaje",
                                title: " (%) ",
                                width: "40px", format: "{0:n2}",
                                attributes: { style: "text-align:right;" }
                            },
                            {
                                field: "dMontoRetencion",
                                title: "MONTO",
                                width: "40px", format: "{0:n2}",
                                attributes: { style: "text-align:right;" }
                            },
                            {
                                field: "sFechaLlegadaDoc",
                                title: "F. DOCUMENTO",
                                width: "40px",
                                attributes: { style: "text-align:center;" }
                            },
                            {
                                //ACTUALIZAR VALORES DE BENRFICAIRIOS
                                title: 'Editar',
                                attributes: { style: "text-align:center;" },
                                template: function (item) {
                                    var controles = "";
                                    var aItems = [item.vDniBeneficiario, item.vNombreBeneficiario, item.iCodigoBanco, item.vNombreBanco, item.vNumeroCuenta, item.iCodTipoRetencion, item.vNombreRetencion, item.dValorPorcentaje, item.dMontoRetencion, item.iCodFormaPago, item.sFechaLlegadaDoc];
                                    controles += '<button type="button" class="btn btn-info btn-sm" onclick="controlador.EditarBeneficiarioTemporal(\'' + aItems + '\')">';
                                    controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar datos del Beneficiario"></span>';
                                    controles += '</button>';
                                    return controles;
                                },
                                width: '30px'
                            },
                            {
                                //title: 'Eliminar',
                                title: 'Quitar',
                                attributes: { style: "text-align:center;" },
                                template: function (item) {
                                    var controles = "";                                    
                                    controles += '<button type="button" class="btn btn-default btn-sm" onclick="controlador.QuitarBeneficiarioJudicialTemporal(\'' + item.iCodJudicialDetalle + '\')">';
                                    controles += '<span class="glyphicon glyphicon-trash" aria-hidden="true" data-uib-tooltip="Ver" title="Quitar Beneficiario"></span>';
                                    controles += '</button>';
                                    return controles;
                                },
                                width: '30px'
                            },
                        ]
                    }).data();
                }
            },
            error: function (res) {
                debugger;
            }
        });
    }

    this.NominaJS.prototype.EditarBeneficiarioTemporal = function (item) {

        var _item = item.split(',');
        var data_param = new FormData();

        debugger;
        
        $("#btnAgregarBeneficiario").text('Actualizar Beneficiario');
        $("#txtNumeroDocumentoBeneficiario").val(_item[0]);
        $("#txtNombreBeneficiario").val(_item[1]);
        $("#ddlBancos").data("kendoDropDownList").value(_item[2]);
        $("#txtNumeroCuenta").val(_item[4]);
        $("#ddlTipoRetencion").data("kendoDropDownList").value(_item[5]);
        $("#txtValorPorcentaje").val(_item[7]);
        $("#txtImporteFijo").val(_item[8]);
        
        $("#ddlFormaPago").data("kendoDropDownList").value(_item[9]);
        $("#txtFechaLlegadaDocumento").val(_item[10]);
        //$("#txtObservacion").val(_item[9]);
        $("#hdIdJudicialDetalle").val('1');  /* Valor 1 cuando es EDICION */


        $("#txtNumeroDocumentoBeneficiario").prop('readonly', true);
        $("#txtNombreBeneficiario").prop('readonly', true);
        $("#txtNumeroCuenta").prop('readonly', false);
        $("#txtFechaLlegadaDocumento").prop('readonly', false);

        if (_item[5] == "1") {
            $('#divPorcentaje').hide();
            $('#divMonto').show();
            $('#txtImporteFijo').prop('readonly', false);

        }
        if (_item[5] == "2") {
            $('#divPorcentaje').show();
            $('#divMonto').hide();
            $('#txtValorPorcentaje').prop('readonly', false);
        }

    };

    this.NominaJS.prototype.CargarFormularioCuentasBancarias = function (id) {
        this.$dataSourceCuentas = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarCuentasEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarCuentaEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarCuentaEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarCuentaEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.IdEmpleado = id;
                            data_param.Estado = 0;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            //if (frmCompromiso.validate()) {                                
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdBanco = $options.Banco.IdBanco;
                            data_param.CCI = $options.CCI;
                            data_param.NroCuenta = $options.NroCuenta;
                            data_param.IdEstado = $options.Estado.Codigo;
                            //}
                            break;
                        case "update":
                            data_param.IdEmpleadoBanco = $options.IdEmpleadoBanco;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdBanco = $options.Banco.IdBanco;
                            data_param.CCI = $options.CCI;
                            data_param.NroCuenta = $options.NroCuenta;
                            data_param.IdEstado = $options.Estado.Codigo;
                            break;
                        case "destroy":
                            data_param.IdEmpleadoBanco = $options.IdEmpleadoBanco;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdBanco = $options.Banco.IdBanco;
                            data_param.CCI = $options.CCI;
                            data_param.NroCuenta = $options.NroCuenta;
                            data_param.IdEstado = 0;
                            break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                switch (e.type) {
                    case "create": case "update": case "destroy":
                        var grilla = $('#divGridCuentas').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleadoBanco",
                    fields: {
                        IdEmpleadoBanco: { editable: false, nullable: true },
                        NroCuenta: { validation: { required: true } },
                        CCI: { validation: { required: true } },
                        Banco: {
                            validation: {
                                required: true,
                                bancovalidation: function (input) {
                                    if (input.is("[name='Banco']") && input.val() == "") {
                                        input.attr("data-bancovalidation-msg", "Entidad bancaria es requerida");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { IdBanco: 8, Nombre: "Nombre" }
                        },
                        Estado: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Estado']") && input.val() == "") {
                                        input.attr("data-estadovalidation-msg", "El estado es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ACTIVO" }
                        },
                    }
                }
            },
        });

        this.divGridCuentas = $("#divGridCuentas").kendoGrid({
            toolbar: ["excel", "create"],
            excel: {
                fileName: "Listado de cuentas bancarias del trabajador.xlsx",
                filterable: false
            },
            dataSource: this.$dataSourceCuentas,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            //dataBound: function (e) {
            //    $('.k-grid-add').unbind("click");
            //    $('.k-grid-add').bind("click", function () {
            //        if ($("#hdIdEmpleado").val() == 0) {
            //            controladorApp.notificarMensajeDeAlerta('Primero debe completar el registro del trabajador 123');
            //            return false;
            //        }
            //    });
            //},
            columns: [
                {
                    field: "Banco",
                    title: "Entidad Bancaria",
                    width: "200px",
                    editor: controlador.BancoDropDownEditor,
                    template: "#=Banco.Nombre#"
                },
                {
                    field: "NroCuenta",
                    title: "Número de Cuenta ",
                    attributes: { style: "text-align:left;" },

                    width: "200px"
                },
                {
                    field: "CCI",
                    title: "Código de Cuenta Interbancaria (CCI)",
                    attributes: { style: "text-align:left;" },

                    width: "200px"
                },
                {
                    field: "Estado",
                    title: "Estado",
                    width: "50px",
                    editor: controlador.EstadoDropDownEditor,
                    template: "#=Estado.Nombre#"
                },
                { title: "Acciones", command: ["edit", "destroy"], width: "250px" },
            ],
            editable: "inline"
        }).data();
    }

    this.NominaJS.prototype.BancoDropDownEditor = function (container, options) {
        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "IdBanco",
                optionLabel: "-- Seleccione --",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Persona/ListarBancos",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = 0;
                            data_param.Grilla.OrdenarPor = "Nombre";
                            data_param.Grilla.OrdenarDeForma = "ASC";

                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.EstadoDropDownEditor = function (container, options) {
        var data = [
                        { Nombre: "ACTIVO", Codigo: "1" },
                        { Nombre: "INACTIVO", Codigo: "0" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };
    this.NominaJS.prototype.OpcionDropDownEditor = function (container, options) {
        var data = [
            { Nombre: "SI", Codigo: "1" },
            { Nombre: "NO", Codigo: "0" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };

    this.NominaJS.prototype.CargarFormularioOrdenesServicio = function (id) {
        this.$dataSourceOrdenes = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarOrdenesEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarOrdenEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarOrdenEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarOrdenEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.IdEmpleado = id;
                            data_param.Estado = 0;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.NroOrden = $options.NroOrden;
                            data_param.NroSIAF = $options.NroSIAF;
                            data_param.Nombre = $options.Nombre.toUpperCase();
                            data_param.Duracion = $options.Duracion;
                            data_param.FechaInicio = $options.FechaInicio;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.Monto = $options.Monto;
                            data_param.IdEstado = $options.Estado.Codigo;
                            break;
                        case "update":
                            data_param.IdEmpleadoOrden = $options.IdEmpleadoOrden;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.NroOrden = $options.NroOrden;
                            data_param.NroSIAF = $options.NroSIAF;
                            data_param.Nombre = $options.Nombre.toUpperCase();
                            data_param.Duracion = $options.Duracion;
                            data_param.FechaInicio = $options.FechaInicio;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.Monto = $options.Monto;
                            data_param.IdEstado = $options.Estado.Codigo;
                            break;
                        case "destroy":
                            data_param.IdEmpleadoOrden = $options.IdEmpleadoOrden;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.NroOrden = $options.NroOrden;
                            data_param.Nombre = $options.Nombre.toUpperCase();
                            data_param.Duracion = $options.Duracion;
                            data_param.FechaInicio = $options.FechaInicio;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.IdEstado = 0;
                            break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                switch (e.type) {
                    case "create": case "update": case "destroy":
                        var grilla = $('#divGridOrdenes').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleadoOrden",
                    fields: {
                        IdEmpleadoOrden: { editable: false, nullable: true },
                        NroOrden: { validation: { required: true, maxlength: "15" } },
                        NroSIAF: { validation: { required: true, maxlength: "15" } },
                        Duracion: { validation: { required: true, type: "number" } },
                        Nombre: { validation: { required: true } },
                        FechaInicio: {
                            validation: {
                                required: true,
                                type: "date",
                                format: "dd/MM/yyyy",
                                fechainiciovalidation: function (input) {
                                    if (input.is("[name='FechaInicio']") && input.val() == "") {
                                        input.attr("data-fechainiciovalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        FechaFin: {
                            validation: {
                                required: true,
                                type: "date",
                                format: "dd/MM/yyyy",
                                fechafinvalidation: function (input) {
                                    if (input.is("[name='FechaFin']") && input.val() == "") {
                                        input.attr("data-fechafinvalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        Monto: {
                            validation: {
                                required: true,
                                type: "number",
                                montovalidation: function (input) {
                                    if (input.is("[name='Monto']") && input.val() == "") {
                                        input.attr("data-montovalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        Estado: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Estado']") && input.val() == "") {
                                        input.attr("data-estadovalidation-msg", "El estado es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ACTIVO" }
                        },
                    }
                }
            },
        });

        this.divGridOrdenes = $("#divGridOrdenes").kendoGrid({
            toolbar: ["excel", "create"], //, { text: "Add New Email", className: "k-grid-addEmail", imageClass: "k-add" }, { text: "Add New Cell Text Message", className: "k-grid-addText", imageClass: "k-add" }
            excel: {
                fileName: "Listado de ordenes de servicio del trabajador.xlsx",
                filterable: false
            },
            dataSource: this.$dataSourceOrdenes,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            dataBound: function (e) {
                $('.k-grid-add').unbind("click");
                $('.k-grid-add').bind("click", function () {
                    if ($("#hdIdEmpleado").val() == 0) {
                        controladorApp.notificarMensajeDeAlerta('Primero debe completar el registro del trabajador');
                        return false;
                    }
                });
            },
            columns: [
                {
                    field: "NroOrden",
                    title: "Nro Orden",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "NroSIAF",
                    title: "Nro SIAF",
                    attributes: { style: "text-align:center;" },
                    width: "30px"
                },
                {
                    field: "Nombre",
                    title: "Nombre de la Orden de Servicio",
                    attributes: { style: "text-align:left;" },
                    editor: controlador.NombreOrdenEditor,
                    width: "200px"
                },
                {
                    field: "Duracion",
                    title: "Plazo",
                    attributes: { style: "text-align:center;" },

                    width: "30px"
                },
                {
                    field: "FechaInicio",
                    title: "Fecha Inicio",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaInicioEditor,
                    width: "70px"
                },
                {
                    field: "FechaFin",
                    title: "Fecha Fin",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaFinEditor,
                    width: "80px"
                },
                {
                    field: "Monto",
                    title: "Monto",
                    attributes: { style: "text-align:right;" },
                    format: "{0:c}",
                    width: "50px"
                },
                {
                    field: "Estado",
                    title: "Estado",
                    width: "30px",
                    editor: controlador.EstadoDropDownEditor,
                    template: "#=Estado.Nombre#"
                },
                { title: "Acciones", command: ["edit", "destroy"], width: "50px" },
            ],
            editable: "inline"
        }).data();
                
    }
     

    this.NominaJS.prototype.NombreOrdenEditor = function (container, options) {
        $('<textarea required name="' + options.field + '" style="width: ' + (container.width() + 30) + 'px;height:' + (container.height() + 30) + 'px" />')
            .appendTo(container);
    };
    this.NominaJS.prototype.FechaInicioEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width: ' + container.width() + 'px" data-bind="value:FechaInicio">')
            .appendTo(container);
    };
    this.NominaJS.prototype.FechaFinEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width: ' + container.width() + 'px" data-bind="value:FechaFin">')
            .appendTo(container);
    };
    this.NominaJS.prototype.agregarTrabajador = function (e) {
        e.preventDefault();
        var metodo = 'Registrar';
        var mensaje = '¿Está seguro de agregar el trabajador?';
        var resultado = 'Trabajador registrado correctamente';
        debugger;
        if (frmPersonaValidador.validate()) {
            var modal = $('#divModalRegistroPersona').data('kendoWindow');
            var esValido = true;
            var mensajeValidacion = '';

            //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid($('#hdnUid').val());
            var data_param = new FormData();
            if ($("#hdIdEmpleado").val() != 0) {
                data_param.append('IdEmpleado', $("#hdIdEmpleado").val());
                metodo = 'Guardar';
                mensaje = '¿Está seguro de actualizar los datos del trabajador?';
                resultado = 'Trabajador actualizado correctamente';

                data_param.append('IdTipoPension', $("#ddlPersonaTipoPension").data("kendoDropDownList").value());
                data_param.append('IdTipoAfp', $("#ddlPersonaTipoAFP").data("kendoDropDownList").value());
                data_param.append('CodigoAfp', $("#txtPersonaCodigoAfp").val());
                data_param.append('IdTipoComisionAfp', $("#ddlPersonaTipoComisionAFP").data("kendoDropDownList").value()); 

                //if ($("#hdIdTieneContrato").val() == '1') {
                
                //}
                //else
                //    data_param.append('NroAIRHSP', '');
            }
            else {
                data_param.append('IdTipoPension', 3); //SIN REGIMEN
                data_param.append('IdTipoAfp', 0); //SIN REGIMEN
                data_param.append('CodigoAfp', ''); //SIN REGIMEN
                data_param.append('IdTipoComisionAfp', 0); //SIN REGIMEN                
            }

            data_param.append('TipoDocumento', $("#ddlPersonaTipoDeDocumento").data("kendoDropDownList").value());
            data_param.append('NroDocumento', $("#txtPersonaNumeroDeDocumento").val());
            data_param.append('Nombre', $("#txtPersonaNombres").val());
            data_param.append('Paterno', $("#txtPersonaApellidoPaterno").val());
            data_param.append('Materno', $("#txtPersonaApellidoMaterno").val());
            data_param.append('IdGenero', $("#ddlPersonaSexo").data("kendoDropDownList").value());
            data_param.append('IdEstadoCivil', $("#ddlPersonaEstadoCivil").data("kendoDropDownList").value());
            data_param.append('IdGrupoSanguineo', $("#ddlPersonaGrupoSan").data("kendoDropDownList").value());
            data_param.append('DescripcionUbigeo', $("#txtPersonaUbigeo").val());
            data_param.append('Domicilio', $("#txtPersonaDireccionDomicilio").val());
            data_param.append('Foto', $('#hdFoto').val());
            data_param.append('Estado', $("#ddlPersonaEstado").data("kendoDropDownList").value());
            data_param.append('IdCondicion', $("#ddlPersonaCondicion").data("kendoDropDownList").value());
            data_param.append('IdSede', $("#ddlPersonaSede").data("kendoDropDownList").value());
            data_param.append('FechaNacimiento', kendo.toString(kendo.parseDate($("#txtPersonaFechaNacimiento").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('FechaInicio', kendo.toString(kendo.parseDate($("#txtPersonaFechaInicioLabores").data("kendoDatePicker").value()), 'dd/MM/yyyy'));

            if ($("#txtPersonaFechaFinLabores").data("kendoDatePicker").value() != '')
                data_param.append('FechaCese', kendo.toString(kendo.parseDate($("#txtPersonaFechaFinLabores").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            
            data_param.append('IdDependencia', $("#ddlPersonaDependencia").data("kendoDropDownList").value());
            data_param.append('NombreCargo', $("#txtPersonaCargo").val().toUpperCase());
            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('Celular', $("#txtTelefonoCelularP").val());
            data_param.append('Telefono', $("#txtTelefonoFijoP").val());
            data_param.append('CorreoElectronicoLaboral', $("#txtPersonaCorreoElectronico").val());
            data_param.append('CelularLaboral', $("#txtTelefonoCelular").val());
            data_param.append('TelefonoLaboral', $("#txtTelefonoLaboral").val());
            data_param.append('AnexoLaboral', $("#txtTelefonoAnexo").val());
            data_param.append('DescripcionUbigeo', $("#txtPersonaUbigeo").val());
            data_param.append('RUC', $("#txtRUC").val());
            data_param.append('Ubigeo', $("#hdIdUbigeoEmpleado").val());            
            data_param.append('EPSEsSalud', $("#ddlEpsEsSalud").data("kendoDropDownList").value() == '1' ? true : false);
            data_param.append('SupsRet4taCat', $("#ddlSupsRet4taCat").data("kendoDropDownList").value() == '1' ? true : false);
            data_param.append('sNroAutorizacionExoneracion', $("#txtNroAutorizacionExoneracion").val());
            data_param.append('DsctoJudicial', $("#ddlDsctoJudicial").data("kendoDropDownList").value() == '1' ? true : false);
            //data_param.append('Director', $("#ddlEsDirector").data("kendoDropDownList").value());
            //data_param.append('Encargatura', $("#ddlEncargatura").data("kendoDropDownList").value() == '1' ? true : false);
            //data_param.append('FED', $("#ddlFED").data("kendoDropDownList").value() == '1' ? true : false);
            data_param.append('Director', ($("#swDirector").data("kendoSwitch").check() ? 1: 0));
            data_param.append('Encargatura', false);
            data_param.append('FED', $("#swFed").data("kendoSwitch").check());
            data_param.append('Sindicato', $("#swSindicato").data("kendoSwitch").check());
            data_param.append('DDJJ', $("#swDDJJ").data("kendoSwitch").check());
            data_param.append('FFAA', $("#swFFAA").data("kendoSwitch").check());
            data_param.append('Deportista', $("#swDeportista").data("kendoSwitch").check());
            data_param.append('Discapacidad', $("#swDiscapacidad").data("kendoSwitch").check());

            data_param.append('IdContrato', $("#hdIdContrato").val());
            data_param.append('IdTipoIngreso', $("#ddlPersonaTipoIngreso").data("kendoDropDownList").value());
            data_param.append('IdTipoContrato', $("#ddlPersonaTipoContrato").data("kendoDropDownList").value());
            data_param.append('NroAIRHSP', $("#txtPersonaAIRHSP").val());
            data_param.append('Meta', $("#txtPersonaMeta").val());
            data_param.append('NroContrato', $("#txtPersonaContrato").val());
            data_param.append('DocIngreso', $("#txtPersonaProceso").val());
            data_param.append('DocCese', $("#txtPersonaCese").val());
            data_param.append('Remuneracion', $("#txtPersonaSalario").val());
            data_param.append('IdTipoModalidad', $("#ddlPersonaModalidad").data("kendoDropDownList").value());

            debugger;
            var nacimiento = kendo.toString(kendo.parseDate($("#txtPersonaFechaNacimiento").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            var inicio = kendo.toString(kendo.parseDate($("#txtPersonaFechaInicioLabores").data("kendoDatePicker").value()), 'dd/MM/yyyy');
            var fin = kendo.toString(kendo.parseDate($("#txtPersonaFechaFinLabores").data("kendoDatePicker").value()), 'dd/MM/yyyy');

            if (nacimiento == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de nacimiento no es válida');
                $("#txtPersonaFechaNacimiento").focus();
                return;
            }
            if (inicio == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de inicio de labores no es válida');
                $("#txtPersonaFechaInicioLabores").focus();
                return;
            }
            if (inicio != null && fin != null) {
                if (Date(inicio) > Date(fin)) {
                    controladorApp.notificarMensajeDeAlerta('La fecha de cese de labores debe ser mayor que la fecha de inicio de labores');
                    $("#txtPersonaFechaFinLabores").focus();
                    return;
                }
            }
            
            controladorApp.abrirMensajeDeConfirmacion(
                mensaje, 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Empleado/' + metodo,
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                controladorApp.notificarMensajeSatisfactorio(resultado);

                                // REFRESCAR INFORMACION DEL TRABAJADOR

                                $("#hdIdEmpleado").val(res.responseText.IdEmpleado);

                                $("#_tab4").attr("data-toggle", "tab");
                                $('#_tab4').prop("onclick", null).off("click");

                                controlador.CargarFormularioTrabajador(res.responseText.IdEmpleado);
                                controlador.CargarFormularioCuentasBancarias(res.responseText.IdEmpleado);
                                controlador.CargarFormularioDesplazamiento($("#txtPersonaNumeroDeDocumento").val());
                                controlador.CargarFormularioEncargaturas($("#txtPersonaNumeroDeDocumento").val());
                                controlador.CargarFormularioFamiliares($("#txtPersonaNumeroDeDocumento").val());
                                controlador.CargarFormularioEstudios($("#txtPersonaNumeroDeDocumento").val());
                                if (res.responseText.ExisteDirector) {
                                    controladorApp.notificarMensajeDeAlerta("No se asignó como director, porque aún existe un director en el área");
                                }
                                //controlador.CargarFormularioOrdenesServicio(res.responseText);

                                modal.title("Actualizar Trabajador");
                                //modal.close();
                                $('#divGrid').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }

    }

    this.NominaJS.prototype.actualizarTrabajadorContacto = function (e) {
        e.preventDefault();
        
        if (frmPersonaValidador.validate()) {
            var modal = $('#divModalRegistroPersona').data('kendoWindow');
            var esValido = true;
            var mensajeValidacion = '';

            //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid($('#hdnUid').val());
            var data_param = new FormData();
            if ($("#hdIdEmpleado").val() != 0) 
                data_param.append('IdEmpleado', $("#hdIdEmpleado").val());
            
            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('Celular', $("#txtTelefonoCelularP").val());
            data_param.append('Telefono', $("#txtTelefonoFijoP").val());
            data_param.append('CorreoElectronicoLaboral', $("#txtPersonaCorreoElectronico").val());
            data_param.append('CelularLaboral', $("#txtTelefonoCelular").val());
            data_param.append('TelefonoLaboral', $("#txtTelefonoLaboral").val());
            data_param.append('AnexoLaboral', $("#txtTelefonoAnexo").val());
            
            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de actualizar los datos del trabajador?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Empleado/GuardarContacto',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {
                                if (($("#hdPerfil").val() == PERFIL_NOMINA_CONTACTO) || ($("#hdPerfil").val() == PERFIL_NOMINA_CONTABILIDAD)) {
                                    modal.close();
                                    $('#divGrid').data("kendoGrid").dataSource.page(1);
                                }
                                else {
                                    controladorApp.notificarMensajeSatisfactorio("Trabajador registrado correctamente");

                                    // REFRESCAR INFORMACION DEL TRABAJADOR
                                    LimpiarModalRegistroPersona();

                                    $("#hdIdEmpleado").val(res.responseText);

                                    $("#_tab4").attr("data-toggle", "tab");
                                    $('#_tab4').prop("onclick", null).off("click");

                                    controlador.CargarFormularioTrabajador(res.responseText);
                                    controlador.CargarFormularioCuentasBancarias(res.responseText);
                                    controlador.CargarFormularioDesplazamiento($("#txtPersonaNumeroDeDocumento").val());
                                    controlador.CargarFormularioEncargaturas($("#txtPersonaNumeroDeDocumento").val());
                                    controlador.CargarFormularioFamiliares($("#txtPersonaNumeroDeDocumento").val());
                                    controlador.CargarFormularioEstudios($("#txtPersonaNumeroDeDocumento").val());
                                    modal.title("Actualizar Trabajador");
                                    $('#divGrid').data("kendoGrid").dataSource.page(1);
                                }
                            }
                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }

    }

    this.NominaJS.prototype.cerrarModalEdicion = function () {
        $('#divModalNuevo').data('kendoWindow').close();
    }

    this.NominaJS.prototype.ingresarDetalleEvaluacion = function (idCad, idEvaluacion) {
        //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
        window.open(controladorApp.obtenerRutaBase() + 'EvaluacionDetalle/?IdCad=' + idCad + "&IdEvaluacion=" + idEvaluacion, '_blank');
    }

    this.NominaJS.prototype.abrirModalCesePersona = function (id, documento) {
        $('#hdIdEmpleadoAux').val(id);

        var modal = $('#divModalCesePersona').data('kendoWindow');
        //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid(id);
        modal.title("Registrar cese de labores");
        $("#txtPersonaFechaFinLaboresElim").kendoDatePicker().data("kendoDatePicker"); //.max(new Date());
        $("#txtPersonaCeseElim").val('');

        modal.open();
    }

    this.NominaJS.prototype.cerrarModalEliminacion = function () {
        var modal = $('#divModalCesePersona').data('kendoWindow');
        $('#hdIdEmpleadoAux').val('');
        modal.close();
    }

    this.NominaJS.prototype.cesarTrabajador = function (e) {
        e.preventDefault();
        var modal = $('#divModalCesePersona').data('kendoWindow');

        if (frmPersonaCeseValidador.validate()) {
            var data_param = new FormData();
            data_param.append('IdEmpleado', $('#hdIdEmpleadoAux').val());
            data_param.append('FechaCese', kendo.toString(kendo.parseDate($("#txtPersonaFechaFinLaboresElim").data("kendoDatePicker").value()), 'dd/MM/yyyy'));
            data_param.append('DocCese', $("#txtPersonaCeseElim").val().toUpperCase());

            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de registrar el cese del trabajador?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Empleado/RegistrarCese',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            controladorApp.notificarMensajeSatisfactorio("Registro de cese se hizo correctamente");

                            var grilla = $('#divGridDesplazamiento').data("kendoGrid");
                            grilla.dataSource.page(1);

                            var grilla2 = $('#divGridEncargaturas').data("kendoGrid");
                            grilla2.dataSource.page(1);

                            modal.close();
                        },
                        error: function (res) {

                        }
                    });
                }, data_param);
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }

        
    }

    this.NominaJS.prototype.AgregarBeneficiarioTemporal = function (e) {
        e.preventDefault();
        debugger;
        //var btn = $("#btnAgregarBeneficiario").text();
        //if (btn == 'Actualizar Beneficiario') {
        //    controlador.EditarBeneficiario();
        //}
        //else {
        //    controlador.AgregarBeneficiario();
        //}
        debugger;
        var resultado = 'La operación se ha realizado correctamente';
        var dniBeneficiario = $("#txtNumeroDocumentoBeneficiario").val();
        var nomBeneficiario = $("#txtNombreBeneficiario").val();
        var codBanco = $("#ddlBancos").data("kendoDropDownList").value();
        var nomBanco = $("#ddlBancos").data("kendoDropDownList").text();
        var nroCuenta = $("#txtNumeroCuenta").val();
        var tipoRetencion = $("#ddlTipoRetencion").data("kendoDropDownList").value();
        var nombRetencion = $("#ddlTipoRetencion").data("kendoDropDownList").text();
        var porcRetencion = $("#txtValorPorcentaje").val();
        var dmontoFijo = $("#txtImporteFijo").val();
        var vObservacion = $("#txtObservacion").val();
        var codFormaPago = $("#ddlFormaPago").data("kendoDropDownList").value();
        var nombFormaPago = $("#ddlFormaPago").data("kendoDropDownList").text();
        var FechaLlegadaDocumento = $("#txtFechaLlegadaDocumento").val();
        //
        if (tipoRetencion == "1") {
            porcRetencion = "0";
            dmontoFijo = $("#txtImporteFijo").val();
        }

        if (tipoRetencion == "2") {
            dmontofijo = "0";
            porcRetencion = $("#txtValorPorcentaje").val();
        }

        //if (dniBeneficiario == "" || nomBeneficiario == "") {
        //    alert("Falta Datos del Beneficiario");
        //    return false;
        //}

        if (codBanco == "") {
            alert("Debe seleccionar el Banco");
            return false;
        }

        if (tipoRetencion == "") {
            alert("Debe seleccionar Tipo de Retención");
            return false;
        }
        if (tipoRetencion == "1") {

            if (dmontoFijo == "") {
                alert("Ingresar Monto de Retención");
                return false;
            }

        }
        if (tipoRetencion == "2") {

            if (porcRetencion == "") {
                alert("Ingresar valor del Porcentaje (%)");
                return false;
            }

        }
        if (codFormaPago == "") {
            alert("Debe seleccionar Forma de Pago");
            return false;
        }
        if (FechaLlegadaDocumento == "") {
            alert("Debe ingresar fecha de la llegado del documento");
            return false;
        }

        /*****************************************************/
        /*    Enviar para agregar la fila en la GRILLA        */
        /*****************************************************/
        var id = $("#hdIdJudicialDetalle").val();
        debugger;
        //if (id == "0") {
            $("#hdCambioDetalle").val('0');

            /* VALIDAR SI EXISTE BENEFICIARIO CON MONTO RETENCIOB */
            //var gridBenef = $("#divGridBeneficiarios").data().kendoGrid.dataSource.view();
            //if (gridBenef.length > 0) {
            //    for (var i = 0; i < gridBenef.length; i++) {
            //        debugger;
            //        if (gridBenef[i].vDniBeneficiario == dniBeneficiario) {
            //            controladorApp.notificarMensajeDeAlerta("El Beneficiario ya ha sido agregado en la relación, validar...");
            //            return;
            //        }

            //    }
            //}
            var data_param = new FormData();
            data_param.append('iCodTrabajador', $("#hdIdEmpleado").val());
            data_param.append('vDniBeneficiario', dniBeneficiario);
            data_param.append('vNombreBeneficiario', nomBeneficiario);
            data_param.append('iCodigoBanco', codBanco);
            data_param.append('vNombreBanco', nomBanco);
            data_param.append('vNumeroCuenta', nroCuenta);
            data_param.append('iCodTipoRetencion', tipoRetencion);
            data_param.append('vNombreRetencion', nombRetencion);
            data_param.append('dValorPorcentaje', porcRetencion);
            data_param.append('dMontoRetencion', dmontoFijo);
            data_param.append('iCodFormaPago', codFormaPago);
            data_param.append('vNombreFormaPago', nombFormaPago);
            data_param.append('sFechaLlegadaDoc', FechaLlegadaDocumento);
            mensaje = '¿Está seguro de actualizar la Retención Judicial ?';
            controladorApp.abrirMensajeDeConfirmacion(
                mensaje, 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'DescuentoJudicial/' + 'Registrar_Beneficiario',

                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        data: arg,
                        success: function (res) {
                            debugger;
                            if (res.success == 'False') {
                                controladorApp.notificarMensajeDeAlerta(res.responseText);
                            }
                            else {

                                

                                    controladorApp.notificarMensajeSatisfactorio(resultado);

                                    // REFRESCAR INFORMACION DEL CONCEPTO TRABAJADOR POR LA PLANILLA APERTURADA
                                    //$("#hdIdConcepto").val(res.responseText);

                                    //controlador.CargarFormularioDescuentoJudicialTrabajador(res.responseText);
                                    controlador.CargarBeneficiarios($("#hdIdEmpleado").val());
                                    controlador.LimpiarBeneficiarios();
                                    modal.title("Actualizar Retención Judicial del Trabajador");
                                    modal.close();

                                    $("#hdIdJudicialTrabajador").val('0');
                                    $("#txtNumeroDocumentoTrabajadorJudicial").val('');
                                    $("#txtNombreTrabajadorJudicial").val('');
                                    //$("#txtMontoRetencionTotalJudicial").val('');
                                    //$('#ddlPlanillaAperturadaJudicial').data("kendoDropDownList").value('');

                                    //$("#ddlPlanillaAperturada_busquedaJudicial").data("kendoDropDownList").value(listaPlanilla);
                                    //$("#btnBuscarPlanillaAperturadaJudicial").click();
                                    //$('#divGridDescuentoJudicial').data("kendoGrid").dataSource.page(1);
                                

                            }

                        },
                        error: function (res) {
                            //alert(res);
                        }
                    });
                }, data_param);


            debugger;

            //this.ListarGrillaBeneficiarios(this.$dataSource);

        //}

        //
        $("#txtNumeroDocumentoBeneficiario").val('');
        $("#txtNombreBeneficiario").val('');
        $("#ddlBancos").data("kendoDropDownList").value('');
        $("#txtNumeroCuenta").val('');
        $("#ddlTipoRetencion").data("kendoDropDownList").value('');
        $("#ddlFormaPago").data("kendoDropDownList").value('');
        $("#txtImporteFijo").val('');
        $("#txtValorPorcentaje").val('');
        $("#txtFechaLlegadaDocumento").val('');

    };    
    
    this.NominaJS.prototype.LimpiarBeneficiarios = function () {
        $("#btnAgregarBeneficiario").text('Agregar Beneficiario');
        $("#txtNumeroDocumentoBeneficiario").val('');
        $("#txtNombreBeneficiario").val('');
        $("#ddlBancos").data("kendoDropDownList").value('');
        $("#txtNumeroCuenta").val('');
        $("#ddlTipoRetencion").data("kendoDropDownList").value('');
        $("#txtImporteFijo").val('');
        $("#txtValorPorcentaje").val('');
        $("#ddlFormaPago").data("kendoDropDownList").value('');
        $("#txtNumeroDocumentoBeneficiario").prop('readonly', false);
        $("#txtNombreBeneficiario").prop('readonly', false);
        $("#txtFechaLlegadaDocumento").val('');
        //

    }
    
    this.NominaJS.prototype.CargarBeneficiarios = function (id) {
        debugger;
        this.$dataSource = [];


        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ObtenerBeneficiarios',
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};

                    if ($operation === "read") {
                        //
                        data_param.IdEmpleado = id
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
                        if ($options !== undefined && $options.sort !== undefined) {
                            data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                            data_param.Grilla.OrdenarPor = $options.sort[0].field;
                        }
                    }

                    return $.toDictionary(data_param);
                }
            },
            change: function (e) {
                $("#lblNroBeneficiario").html(this.total());
                debugger;
            },
            schema: {
                total: function (response) {
                    //debugger;
                    //var TotalDeRegistros = 0;
                    //if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return response.length; // TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleado",
                    fields: {
                        IdEmpleado: { type: "number" }
                    }
                }
            }
        });

        this.$grid = $("#divGridBeneficiarios").kendoGrid({
            dataSource: this.$dataSource,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "iCodJudicial",
                    title: "COD. JUDICIAL",
                    width: "30px",
                    attributes: { style: "text-align:left;" },
                    hidden: true
                },
                {
                    field: "iCodJudicialDetalle",
                    title: "COD. DET JUDICIAL",
                    width: "30px",
                    attributes: { style: "text-align:left;" },
                    hidden: true
                },
                {
                    field: "vDniBeneficiario",
                    title: "Nª DNI",
                    width: "30px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "vNombreBeneficiario",
                    title: "BENEFICIARIO(A)",
                    width: "150px",
                    attributes: { style: "text-align:left;" }
                },

                {
                    field: "iCodigoBanco",
                    title: "COD.BANCO",
                    width: "10px",
                    hidden: true,
                    attributes: { style: "text-align:center;" }

                },
                {
                    field: "vNombreBanco",
                    title: "BANCO",
                    width: "100px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "vNumeroCuenta",
                    title: "NRO. CUENTA",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "iCodTipoRetencion",
                    title: "COD. RETENCION",
                    width: "20px",
                    attributes: { style: "text-align:center;" },
                    hidden: true

                },
                {
                    field: "vNombreRetencion",
                    title: "TIPO RETENCION",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "iCodFormaPago",
                    title: "COD. FORMA PAGO",
                    width: "20px",
                    attributes: { style: "text-align:center;" },
                    hidden: true

                },
                {
                    field: "vNombreFormaPago",
                    title: "FORMA PAGO",
                    width: "50px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    field: "dValorPorcentaje",
                    title: " (%) ",
                    width: "40px", format: "{0:n2}",
                    attributes: { style: "text-align:right;" }
                },
                {
                    field: "dMontoRetencion",
                    title: "MONTO",
                    width: "40px", format: "{0:n2}",
                    attributes: { style: "text-align:right;" }
                },
                {
                    field: "sFechaLlegadaDoc",
                    title: "F. DOCUMENTO",
                    width: "40px",
                    attributes: { style: "text-align:center;" }
                },
                {
                    //ACTUALIZAR VALORES DE BENRFICAIRIOS
                    title: 'Editar',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        var aItems = [item.vDniBeneficiario, item.vNombreBeneficiario, item.iCodigoBanco, item.vNombreBanco, item.vNumeroCuenta, item.iCodTipoRetencion, item.vNombreRetencion, item.dValorPorcentaje, item.dMontoRetencion, item.iCodFormaPago, item.sFechaLlegadaDoc];
                        controles += '<button type="button" class="btn btn-info btn-sm" onclick="controlador.EditarBeneficiarioTemporal(\'' + aItems + '\')">';
                        controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar datos del Beneficiario"></span>';
                        controles += '</button>';
                        return controles;
                    },
                    width: '30px'
                },
                {
                    //title: 'Eliminar',
                    title: 'Quitar',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        controles += '<button type="button" class="btn btn-default btn-sm" onclick="controlador.QuitarBeneficiarioJudicialTemporal(\'' + item.iCodJudicialDetalle + '\')">';
                        controles += '<span class="glyphicon glyphicon-trash" aria-hidden="true" data-uib-tooltip="Ver" title="Quitar Beneficiario"></span>';
                        controles += '</button>';
                        return controles;
                    },
                    width: '30px'
                },
            ]
        }).data();
    };

    this.NominaJS.prototype.QuitarBeneficiarioJudicialTemporal = function (id) {
        
        var resultado = 'La operación se ha realizado correctamente';        
        
        var data_param = new FormData();
        data_param.append('iCodJudicialDetalle', id);        
        mensaje = '¿Está seguro de quitar de la lista al benerficiario?';
        controladorApp.abrirMensajeDeConfirmacion(
            mensaje, 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'DescuentoJudicial/' + 'Quitar_Beneficiario',

                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            controladorApp.notificarMensajeSatisfactorio(resultado);

                            // REFRESCAR INFORMACION DEL CONCEPTO TRABAJADOR POR LA PLANILLA APERTURADA                            
                            controlador.CargarBeneficiarios($("#hdIdEmpleado").val());
                            controlador.LimpiarBeneficiarios();                           
                        }
                    },
                    error: function (res) {
                        //alert(res);
                    }
                });
            }, data_param);


        debugger;

        ////this.ListarGrillaBeneficiarios(this.$dataSource);

        ////}

        ////
        //$("#txtNumeroDocumentoBeneficiario").val('');
        //$("#txtNombreBeneficiario").val('');
        //$("#ddlBancos").data("kendoDropDownList").value('');
        //$("#txtNumeroCuenta").val('');
        //$("#ddlTipoRetencion").data("kendoDropDownList").value('');
        //$("#ddlFormaPago").data("kendoDropDownList").value('');
        //$("#txtImporteFijo").val('');
        //$("#txtValorPorcentaje").val('');

        
    };

    this.NominaJS.prototype.CargarFormularioDesplazamiento = function (documento) {
        debugger;
        //console.log(parseInt($("#hdIdDependencia").val()));
        this.$dataSourceDesplazamiento = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarDesplazamientoEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                //update: {
                //    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarEncargaturasEmpleado",
                //    type: 'POST',
                //    dataType: 'json',
                //    cache: false
                //},
                //destroy: {
                //    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarEncargaturasEmpleado",
                //    type: 'POST',
                //    dataType: 'json',
                //    cache: false
                //},
                //create: {
                //    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarEncargaturasEmpleado",
                //    type: 'POST',
                //    dataType: 'json',
                //    cache: false
                //},

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.TipoDocumento = 1;
                            data_param.NroDocumento = documento;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        //case "create":
                        //    //if (frmCompromiso.validate()) {                                
                        //    data_param.IdMaestroDetalle = $options.IdMaestroDetalle;
                        //    data_param.IdEmpleado = $("#hdIdEmpleado").val();
                        //    data_param.IdDependenciaEncargatura = $options.Dependencia.IdDependencia;
                        //    data_param.FechaIni = $options.FechaIni;
                        //    data_param.IdEstado = $options.Estado.Codigo;
                        //    //}
                        //    break;
                        //case "update":
                        //    data_param.IdMaestroDetalle = $options.IdMaestroDetalle;
                        //    data_param.IdEmpleadoEncargatura = $options.IdEmpleadoEncargatura;
                        //    data_param.IdEmpleado = $("#hdIdEmpleado").val();
                        //    data_param.IdDependenciaEncargatura = $options.Dependencia.IdDependencia;
                        //    data_param.FechaIni = $options.FechaIni;
                        //    data_param.FechaFin = $options.FechaFin;
                        //    data_param.IdEstado = $options.Estado.Codigo;
                        //    break;
                        //case "destroy":
                        //    data_param.IdEmpleadoBanco = $options.IdEmpleadoBanco;
                        //    data_param.IdEmpleado = $("#hdIdEmpleado").val();
                        //    data_param.IdBanco = $options.Banco.IdBanco;
                        //    data_param.CCI = $options.CCI;
                        //    data_param.NroCuenta = $options.NroCuenta;
                        //    data_param.IdEstado = 0;
                        //    break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                if (e.response.success != null) {
                    if (e.response.success == 'False') {
                        controladorApp.notificarMensajeDeAlerta(e.response.responseText);
                    }
                    else {
                        controladorApp.notificarMensajeSatisfactorio("Se registró con éxito");
                    }
                }
                switch (e.type) {
                    case "create": case "update": //case "destroy":
                        var grilla = $('#divGridDesplazamiento').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleado",
                    fields: {
                        IdEmpleado: { editable: false, nullable: true },
                        Dependencia: {
                            validation: {
                                required: true,
                                dependenciavalidation: function (input) {
                                    if (input.is("[name='Dependencia']") && input.val() == "") {
                                        input.attr("data-dependenciavalidation-msg", "Dependencia es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                IdDependencia: parseInt($("#hdIdDependencia").val()),
                                Nombre: "Nombre"
                            }
                        },
                        FechaIni: { editable: true, nullable: true, type: "date" },
                        FechaFin: { editable: true, nullable: true, type: "date" },
                        Estado: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Estado']") && input.val() == "") {
                                        input.attr("data-estadovalidation-msg", "El estado es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ACTIVO" }
                        },
                    }
                }
            },
        });

        this.divGridDesplazamiento = $("#divGridDesplazamiento").kendoGrid({
            toolbar: ["excel"], //, "create"
            //excel: {
            //    fileName: "Listado de encargaturas del trabajador.xlsx",
            //    filterable: false
            //},
            dataSource: this.$dataSourceDesplazamiento,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columns: [
                {
                    field: "IdMaestroDetalle",
                    title: "Item",
                    attributes: { style: "text-align:center;" },
                    width: "20px",
                    template: function (item) {
                        return '<h6><span class="badge badge-light" style="background-color:' + colores[item.IdMaestroDetalle - 1] + '">' + item.IdMaestroDetalle + '</span></h6>';
                    }
                },
                //{
                //    field: "NroDocumento",
                //    title: "DNI",
                //    attributes: { style: "text-align:center;" },
                //    width: "30px"
                //},
                {
                    field: "CondicionLaboral",
                    title: "CONDICIÓN",
                    width: "50px",
                    aggregates: ["count"],
                    //groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    field: "DocIngreso",
                    title: "DOCUMENTO INGRESO",
                    width: "50px",
                },
                {
                    field: "TipoIngreso",
                    title: "TIPO DE INGRESO",
                    width: "50px",
                },
                {
                    field: "NroContrato",
                    title: "CONTRATO",
                    attributes: { style: "text-align:center;" },
                    width: "50px",
                },
                {
                    field: "Remuneracion",
                    title: "REMUNERACIÓN",
                    attributes: { style: "text-align:right;" },
                    width: "50px",
                },
                {
                    //field: "FechaInicio",
                    title: "INICIO DE LABORES",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        return kendo.toString(kendo.parseDate(item.FechaInicio), 'dd/MM/yyyy');
                    },
                    //
                    width: "50px"
                },
                {
                    //field: "FechaInicio",
                    title: "FIN DE LABORES",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        return kendo.toString(kendo.parseDate(item.FechaCese), 'dd/MM/yyyy');
                    },
                    //
                    width: "50px"
                },
                {
                    field: "NombreOficina",
                    title: "DEPENDENCIA",
                    width: "200px",
                    aggregates: ["count"],
                    groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                //{
                //    field: "CorreoElectronicoLaboral",
                //    title: "EMAIL LABORAL",
                //    //attributes: { style: "text-align:center;" },
                //    width: "50px"
                //},
                {
                    field: "NombreCargo",
                    title: "CARGO",
                    width: "200px"
                },
                {
                    field: "EstadoNombre",
                    title: "ESTADO",
                    attributes: { style: "text-align:center;" },
                    width: "30px",
                    template: function (item) {
                        var controles = '<h6><span class="badge badge-light">' + item.EstadoNombre + '</span></h6>';
                        if (item.Estado == 1)
                            controles = '<h6><span class="badge badge-info">' + item.EstadoNombre + '</span></h6>'
                        if (item.Estado == 0)
                            controles = '<h6><span class="badge badge-secondary">' + item.EstadoNombre + '</span></h6>'

                        return controles;
                    }
                    //aggregates: ["count"],
                    //groupHeaderTemplate: "#= value # (Total: #= count#)"
                },
                {
                    //INGRESAR DETALLE DE LA EVALUACION
                    title: '',
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = "";
                        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalRegistroPersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                        controles += '<span class="glyphicon glyphicon-edit" aria-hidden="true" data-uib-tooltip="Editar" title="Actualizar información del trabajador"></span>';
                        controles += '</button>&nbsp;';

                        if (item.Estado == 1) {
                            controles += '<button type="button" class="btn btn-secondary btn-xs" onclick="controlador.abrirModalCesePersona(\'' + item.IdEmpleado + '\',\'' + item.NroDocumento + '\')">';
                            controles += '<span class="glyphicon glyphicon-ban-circle" aria-hidden="true" data-uib-tooltip="Eliminar" title="Dar de baja al trabajador"></span>';
                            controles += '</button>&nbsp;';
                        }
                        if (item.Estado == 0) {
                            controles += '<button type="button" class="btn btn-danger btn-xs" onclick="controlador.abrirModalEliminarPersona(\'' + item.IdMaestro + '\',\'' + item.IdMaestroDetalle + '\')">';
                            controles += '<span class="glyphicon glyphicon-remove" aria-hidden="true" data-uib-tooltip="Editar" title="Eliminar movimiento laboral del trabajador"></span>';
                            controles += '</button>';
                        }
                        
                        return controles;
                    },
                    width: '80px'
                }
                /*{ title: "Acciones", command: ["edit"], width: "250px" },*/
            ],
            editable: "inline",
            cancel: function (e) {
                $("#divGridDesplazamiento").data("kendoGrid").refresh();
            }
        }).data();
    };
    this.NominaJS.prototype.CargarFormularioEncargaturas = function (documento) {
        debugger;
        $("#hdNroDocumento").val(documento);

        this.$dataSourceEncargaturas = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarEncargaturasEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarEncargaturasEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarEncargaturasEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarEncargaturasEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.TipoDocumento = 1;
                            data_param.NroDocumento = documento;
                            //data_param.IdDependenciaEncargatura = $options.Dependencia.IdDependencia;
                            //data_param.Estado = 0;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            //if (frmCompromiso.validate()) {                                
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdDependenciaEncargatura = $options.Dependencia.IdDependencia;
                            data_param.FechaIni = $options.FechaIni;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.IdEstado = $options.Estado.Codigo;
                            data_param.IdDDJJ = $options.DDJJ.Codigo;
                            data_param.DocEncargatura = $options.DocEncargatura;
                            data_param.DocEncargaturaFin = $options.DocEncargaturaFin;
                            //}
                            break;
                        case "update":
                            data_param.IdEmpleadoEncargatura = $options.IdEmpleadoEncargatura;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdDependenciaEncargatura = $options.Dependencia.IdDependencia;
                            data_param.FechaIni = $options.FechaIni;
                            data_param.FechaFin = $options.FechaFin;
                            data_param.IdEstado = $options.Estado.Codigo;
                            data_param.IdDDJJ = $options.DDJJ.Codigo;
                            data_param.DocEncargatura = $options.DocEncargatura;
                            data_param.DocEncargaturaFin = $options.DocEncargaturaFin;
                            break;
                        //case "destroy":
                        //    data_param.IdEmpleadoBanco = $options.IdEmpleadoBanco;
                        //    data_param.IdEmpleado = $("#hdIdEmpleado").val();
                        //    data_param.IdBanco = $options.Banco.IdBanco;
                        //    data_param.CCI = $options.CCI;
                        //    data_param.NroCuenta = $options.NroCuenta;
                        //    data_param.IdEstado = 0;
                        //    break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                if (e.response.success != null) {
                    if (e.response.success == 'False') {
                        controladorApp.notificarMensajeDeAlerta(e.response.responseText);
                    }
                    else {
                        controladorApp.notificarMensajeSatisfactorio("Se registró con éxito");
                    }
                }
                switch (e.type) {
                    case "create": case "update": //case "destroy":
                        var grilla = $('#divGridEncargaturas').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        //controlador.CargarFormularioEncargaturas($("#hdNroDocumento").val());

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEmpleado",
                    fields: {
                        IdMaestroDetalle: { editable: false, nullable: false
                            //validation: {
                            //    required: true,
                            //    detallevalidation: function (input) {
                            //        if (input.is("[name='IdMaestroDetalle']") && input.val() == "") {
                            //            input.attr("data-detallevalidation-msg", "Item detalle es requerido");
                            //            return false;
                            //        }
                            //        return true;
                            //    }
                            //}
                        },
                        IdEmpleado: { editable: false, nullable: true },                        
                        DocEncargatura: { editable: true },
                        DocEncargaturaFin: { editable: true },
                        Dependencia: {
                            validation: {
                                required: true,
                                dependenciavalidation: function (input) {
                                    if (input.is("[name='Dependencia']") && input.val() == "") {
                                        input.attr("data-dependenciavalidation-msg", "Dependencia es requerida");
                                        return false;
                                    }                                    
                                    return true;
                                }
                            }, defaultValue: {
                                IdDependencia: parseInt($("#hdIdDependencia").val()),
                                Nombre: "Nombre"
                            }
                        },
                        FechaIni: {
                            editable: true,
                            //nullable: false,
                            type: "date",
                            format: "dd/MM/yyyy",
                            validation: {
                                required: true,
                                min: new Date(),
                                fechainivalidation: function (input) {
                                    if (input.is("[name='FechaIni']") && input.val() == "") {
                                        input.attr("data-fechainivalidation-msg", "fecha de inicio requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        FechaFin: {
                            //editable: true,
                            //nullable: true,
                            type: "date",
                            format: "dd/MM/yyyy",
                            //validation: {
                            //    fechafinvalidation: function (input) {
                            //        if (input.is("[name='FechaFin']") && input.val() != "") {
                            //            if (input.is("[name='FechaIni']") && input.val() != "") {

                            //                return false;
                            //            }
                            //        }

                            //        return true;
                            //    }
                            //}
                        },
                        Estado: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Estado']") && input.val() == "") {
                                        input.attr("data-estadovalidation-msg", "El estado es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ACTIVO" }
                        },
                        DDJJ: {
                            validation: {
                                required: true,
                                ddjjvalidation: function (input) {
                                    if (input.is("[name='DDJJ']") && input.val() == "") {
                                        input.attr("data-ddjjvalidation-msg", "DDJJ es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 0, Nombre: "NO" }
                        },
                    }
                }
            },
        });

        this.divGridEncargaturas = $("#divGridEncargaturas").kendoGrid({
            toolbar: ["create"], //"excel", 
            //excel: {
            //    fileName: "Listado de encargaturas del trabajador.xlsx",
            //    filterable: false
            //},
            dataSource: this.$dataSourceEncargaturas,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            dataBound: function (e) {
                $('.k-grid-add').unbind("click");
                $('.k-grid-add').bind("click", function () {
                    if ($("#hdIdEmpleado").val() == 0) {
                        controladorApp.notificarMensajeDeAlerta('Primero debe completar el registro del trabajador');
                        return false;
                    }
                });
            },
            columns: [
                {
                    field: "IdMaestroDetalle",
                    title: "Item",
                    attributes: { style: "text-align:center;" },
                    width: "20px",
                    editor: controlador.ItemMaestroDropDownEditor,
                    template: function (item) {
                        return '<h6><span class="badge badge-light" style="background-color:' + colores[item.IdMaestroDetalle - 1] + '">' + item.IdMaestroDetalle + '</span></h6>';
                    }
                },
                {
                     field: "Dependencia",
                     title: "DEPENDENCIA ENCARGADA",
                     width: "150px",
                     editor: controlador.DependenciaDropDownEditor,
                     template: "#=Dependencia.Nombre#"
                },
                {
                    title: "INICIO",
                    columns: [
                        {
                            field: "DocEncargatura",
                            title: "DOC. SUSTENTO",
                            width: "50px"
                        },
                        {
                            field: "FechaIni",
                            title: "FECHA",
                            attributes: { style: "text-align:center;" },
                            format: "{0: dd/MM/yyyy}",
                            width: "50px"
                        }
                    ]
                },
                {
                    title: "FIN",
                    columns: [
                        {
                            field: "DocEncargaturaFin",
                            title: "DOC. SUSTENTO",
                            width: "50px"
                        },
                        {
                            field: "FechaFin",
                            title: "FECHA",
                            attributes: { style: "text-align:center;" },
                            format: "{0: dd/MM/yyyy}",
                            width: "50px"
                        }
                    ]
                },
                {
                    field: "DDJJ",
                    title: "¿PRESENTA DDJJ?",
                    width: "50px",
                    editor: controlador.OpcionDropDownEditor,
                    template: "#=DDJJ.Nombre#",
                    attributes: { style: "text-align:center;" },
                },
                {
                    field: "Estado",
                    title: "ESTADO",
                    width: "50px",
                    editor: controlador.EstadoDropDownEditor,
                    //template: "#=Estado.Nombre#",
                    attributes: { style: "text-align:center;" },
                    template: function (item) {
                        var controles = '<h6><span class="badge badge-light">' + item.Estado.Nombre + '</span></h6>';
                        if (item.Estado.Codigo == 1)
                            controles = '<h6><span class="badge badge-info">' + item.Estado.Nombre + '</span></h6>'
                        if (item.Estado.Codigo == 0)
                            controles = '<h6><span class="badge badge-secondary">' + item.Estado.Nombre + '</span></h6>'

                        return controles;
                    }
                },
                { title: "Acciones", command: ["edit"], width: "100px" },
            ],
            editable: "inline",
            cancel: function (e) {                
                $("#divGridEncargaturas").data("kendoGrid").refresh();
            }
        }).data();
    };
    this.NominaJS.prototype.CargarFormularioFamiliares = function (documento) {
        debugger;
        $("#hdNroDocumento").val(documento);

        this.$dataSourceFamiliares = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarFamiliaresEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarFamiliarEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarFamiliarEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarFamiliarEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },

                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            //data_param.TipoDocumento = 1;
                            //data_param.NroDocumento = documento;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.Estado = 1;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            //if (frmCompromiso.validate()) {                                
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.Nombre = $options.Nombre.toUpperCase(); //Banco.IdBanco;
                            data_param.IdParentesco = $options.Parentesco.Codigo;
                            data_param.NroDocumento = $options.NroDocumento;
                            data_param.Edad = $options.Edad;
                            data_param.FechaNacimiento = kendo.toString(kendo.parseDate($options.FechaNacimiento), 'dd/MM/yyyy');
                            data_param.Ocupacion = $options.Ocupacion;
                            data_param.IdSexo = $options.Sexo.Nombre;
                            //}
                            break;
                        case "update":
                            //data_param.IdEmpleadoFamiliar = $options.IdEmpleadoFamiliar;
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdFamiliar = $options.IdFamiliar;
                            data_param.Nombre = $options.Nombre.toUpperCase(); //Banco.IdBanco;
                            data_param.IdParentesco = $options.Parentesco.Codigo;
                            data_param.NroDocumento = $options.NroDocumento;
                            data_param.Edad = $options.Edad;
                            data_param.FechaNacimiento = kendo.toString(kendo.parseDate($options.FechaNacimiento), 'dd/MM/yyyy');
                            data_param.Ocupacion = $options.Ocupacion;
                            data_param.IdSexo = $options.Sexo.Nombre;
                            break;
                        case "destroy":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdFamiliar = $options.IdFamiliar;
                            data_param.IdEstado = 0;
                            break;
                    }

                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                if (e.response.success != null) {
                    if (e.response.success == 'False') {
                        controladorApp.notificarMensajeDeAlerta(e.response.responseText);
                    }
                    else {
                        controladorApp.notificarMensajeSatisfactorio("Se registró con éxito");
                    }
                }
                switch (e.type) {
                    case "create": case "update": case "destroy":
                        var grilla = $('#divGridFamiliares').data("kendoGrid");
                        grilla.dataSource._sort = undefined;
                        grilla.dataSource.page(1);

                        break;
                }
            },
            schema: {
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdFamiliar",
                    fields: {
                        IdEmpleado: { editable: false, nullable: true },
                        IdFamiliar: { editable: false, nullable: false },
                        Nombre: { validation: { required: true, style: "text-transform: uppercase" } }, //, class: "text-uppercase"
                        Edad: { validation: { required: true, type: "number" } },
                        DNI: { validation: { required: true, maxlength: "8" } },
                        Parentesco: {
                            validation: {
                                required: true,
                                parentescovalidation: function (input) {
                                    if (input.is("[name='Parentesco']") && input.val() == "") {
                                        input.attr("data-parentescovalidation-msg", "Parentesco es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 1, Nombre: "ESPOSO (A)" }
                        },
                        FechaNacimiento: {
                            validation: {
                                required: true,
                                type: "date",
                                format: "dd/MM/yyyy",
                                fechanacimientovalidation: function (input) {
                                    if (input.is("[name='FechaNacimiento']") && input.val() == "") {
                                        input.attr("data-fechanacimientovalidation-msg", "requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }
                        },
                        Sexo: {
                            validation: {
                                required: true,
                                estadovalidation: function (input) {
                                    if (input.is("[name='Sexo']") && input.val() == "") {
                                        input.attr("data-sexovalidation-msg", "El sexo es requerido");
                                        return false;
                                    }

                                    return true;
                                }
                            }, defaultValue: { Codigo: 0, Nombre: "F" }
                        },
                    }
                }
            },
        });

        this.divGridFamiliares = $("#divGridFamiliares").kendoGrid({
            toolbar: ["create"], //"excel", 
            //excel: {
            //    fileName: "Listado de encargaturas del trabajador.xlsx",
            //    filterable: false
            //},
            dataSource: this.$dataSourceFamiliares,
            autoBind: true,

            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            dataBound: function (e) {
                $('.k-grid-add').unbind("click");
                $('.k-grid-add').bind("click", function () {
                    if ($("#hdIdEmpleado").val() == 0) {
                        controladorApp.notificarMensajeDeAlerta('Primero debe completar el registro del trabajador');
                        return false;
                    }
                });
            },
            columns: [
                {
                    field: "Parentesco",
                    title: "Parentesco",
                    width: "80px",
                    editor: controlador.ParentescoDropDownEditor,
                    template: "#=Parentesco.Nombre#"
                },
                {
                    field: "Nombre",
                    title: "Apellidos y Nombres",
                    attributes: { style: "text-align:left;" },
                    width: "150px"
                },
                {
                    field: "NroDocumento",
                    title: "DNI",
                    attributes: { style: "text-align:right;" },
                    width: "70px"
                },
                {
                    field: "FechaNacimiento",
                    title: "Fec Nacimiento",
                    width: "30px",
                    attributes: { style: "text-align:center;" },
                    editor: controlador.FechaNacimientoEditor
                },
                {
                    field: "Edad",
                    title: "Edad",
                    attributes: { style: "text-align:right;" },
                    width: "50px"
                },
                {
                    field: "Ocupacion",
                    title: "Ocupación: Trabajo/Estudios",
                    attributes: { style: "text-align:left;" },
                    width: "150px"
                },
                {
                    field: "Sexo",
                    title: "Sexo",
                    width: "30px",
                    editor: controlador.SexoDropDownEditor,
                    template: "#=Sexo.Nombre#"
                },
                { title: "Acciones", command: ["edit", "destroy"], width: "150px" },
            ],
            editable: "inline",
            cancel: function (e) {
                $("#divGridEncargaturas").data("kendoGrid").refresh();
            }
        }).data();
    };

    this.NominaJS.prototype.abrirModalEliminarPersona = function (maestro, detalle) {
        controladorApp.abrirMensajeDeConfirmacion(
            '¿Está seguro de eliminar el movimiento laboral del trabajador?', 'SI', 'NO'
            , function (arg) {
                $.ajax({
                    url: controladorApp.obtenerRutaBase() + 'Empleado/EliminarHistorialEmpleado',
                    type: 'POST',
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    data: arg,
                    success: function (res) {
                        debugger;
                        if (res.success == 'False') {
                            controladorApp.notificarMensajeDeAlerta(res.responseText);
                        }
                        else {
                            var grilla = $('#divGridDesplazamiento').data("kendoGrid");
                            grilla.dataSource.page(1);

                            var grilla2 = $('#divGridEncargaturas').data("kendoGrid");
                            grilla2.dataSource.page(1);

                            //modal.close();
                        }
                    },
                    error: function (res) {
                        alert(res);
                    }
                });
            }, data_param);

        if (frmPersonaValidador.validate()) {
            var modal = $('#divModalRegistroPersona').data('kendoWindow');
            var esValido = true;
            var mensajeValidacion = '';

            //var dr = $("#divGrid").data("kendoGrid").dataSource.getByUid($('#hdnUid').val());
            var data_param = new FormData();
            if ($("#hdIdEmpleado").val() != 0)
                data_param.append('IdEmpleado', $("#hdIdEmpleado").val());

            data_param.append('CorreoElectronico', $("#txtPersonaCorreoElectronicoP").val());
            data_param.append('Celular', $("#txtTelefonoCelularP").val());
            data_param.append('Telefono', $("#txtTelefonoFijoP").val());
            data_param.append('CorreoElectronicoLaboral', $("#txtPersonaCorreoElectronico").val());
            data_param.append('CelularLaboral', $("#txtTelefonoCelular").val());
            data_param.append('TelefonoLaboral', $("#txtTelefonoLaboral").val());
            data_param.append('AnexoLaboral', $("#txtTelefonoAnexo").val());

            
        }
        else {
            controladorApp.notificarMensajeDeAlerta('Completar los campos requeridos');
        }

    }


    this.NominaJS.prototype.ParentescoDropDownEditor = function (container, options) {
        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                optionLabel: "-- Seleccione --",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Empleado/ListarParentesco",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = 0;
                            data_param.Grilla.OrdenarPor = "Nombre";
                            data_param.Grilla.OrdenarDeForma = "ASC";

                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.SexoDropDownEditor = function (container, options) {
        var data = [
            { Nombre: "F", Codigo: "0" },
            { Nombre: "M", Codigo: "1" }
        ];

        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                //optionLabel: "-- Seleccione --",
                dataSource: data
            });
    };

    this.NominaJS.prototype.FechaNacimientoEditor = function (container, options) {
        $('<input data-role="datepicker" data-format="dd/MM/yyyy" type="date" name="' + options.field + '" style="width_: ' + container.width() + 'px" data-bind="value:FechaNacimiento">')
            .appendTo(container);
    };

    this.NominaJS.prototype.DependenciaDropDownEditor = function (container, options) {
        /*$('<input required name="' + options.field + '" style="width: ' + (container.width() + 200) + 'px; "/>')*/
        $('<input required name="' + options.field + '" style="width: 100%; "/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "IdDependencia",
                optionLabel: "-- Seleccione --",
                filter: "contains",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            if ($options.filter != undefined && $options.filter.filters.length > 0)
                                data_param.term = $options.filter.filters[0].value;                            
                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.NivelEducativoDropDownEditor = function (container, options) {
        /*$('<input required name="' + options.field + '" style="width: ' + (container.width() + 200) + 'px; "/>')*/
        $('<input required name="' + options.field + '" style="width: 100%; "/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "IdDependencia",
                optionLabel: "-- Seleccione --",
                filter: "contains",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Dependencia/ListarDependencias",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            if ($options.filter != undefined && $options.filter.filters.length > 0)
                                data_param.term = $options.filter.filters[0].value;
                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.NivelAlcanzadoDropDownEditor = function (container, options) {
        /*$('<input required name="' + options.field + '" style="width: ' + (container.width() + 200) + 'px; "/>')*/
        $('<input required name="' + options.field + '" style="width: 100%; "/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "strDescripcion",
                dataValueField: "iCodNivel",
                optionLabel: "-- Seleccione --",
                filter: "contains",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Perfiles/ListarNivelEducativo",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            if ($options.filter != undefined && $options.filter.filters.length > 0)
                                data_param.term = $options.filter.filters[0].value;
                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.Meses = function (container, options) {
        /*$('<input required name="' + options.field + '" style="width: ' + (container.width() + 200) + 'px; "/>')*/
        $('<input required name="' + options.field + '" style="width: 100%; "/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Nombre",
                dataValueField: "Codigo",
                optionLabel: "-- Seleccione --",
                filter: "contains",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Nomina/ListarMeses",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            if ($options.filter != undefined && $options.filter.filters.length > 0)
                                data_param.term = $options.filter.filters[0].value;
                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.Anios = function (container, options) {
        /*$('<input required name="' + options.field + '" style="width: ' + (container.width() + 200) + 'px; "/>')*/
        $('<input required name="' + options.field + '" style="width: 100%; "/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "Anio",
                dataValueField: "Anio",
                optionLabel: "-- Seleccione --",
                filter: "contains",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Nomina/ListarAnioPostulacion",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            if ($options.filter != undefined && $options.filter.filters.length > 0)
                                data_param.term = $options.filter.filters[0].value;
                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    }
                }
            });
    };

    this.NominaJS.prototype.ItemMaestroDropDownEditor = function (container, options) {
        $('<input required name="' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                autoBind: true,
                dataTextField: "IdMaestroDetalle",
                dataValueField: "IdMaestroDetalle",
                optionLabel: "-- Seleccione --",
                filter: "contains",
                dataSource: {
                    transport: {
                        read: {
                            url: controladorApp.obtenerRutaBase() + "Empleado/ListarDesplazamientoEmpleado",
                            type: "GET",
                            dataType: "json",
                            cache: false
                        },
                        parameterMap: function ($options, $operation) {
                            var data_param = {};
                            data_param.TipoDocumento = 1;
                            data_param.NroDocumento = $("#hdNroDocumento").val();;

                            return $.toDictionary(data_param);
                        }
                    },
                    requestEnd: function (e) {

                    },
                    filter: [
                        { field: "Estado", operator: "equals", value: 1 }
                    ]
                }
            });
    };

    this.NominaJS.prototype.CargarFormularioEstudios = function (documento) {
        debugger;
        $("#hdNroDocumento").val(documento);
        this.$dataSourceEstudios = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Empleado/ListarEstudiosEmpleado',
                    type: 'GET',
                    dataType: 'json',
                    cache: false,
                },
                update: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/ActualizarEstudioEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                destroy: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/EliminarEstudioEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false
                },
                create: {
                    url: controladorApp.obtenerRutaBase() + "Empleado/RegistrarEstudioEmpleado",
                    type: 'POST',
                    dataType: 'json',
                    cache: false,
                },
                parameterMap: function ($options, $operation) {
                    var data_param = {};
                    switch ($operation) {
                        case "read":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.Estado = 1;
                            data_param.Grilla = {};
                            data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                            data_param.Grilla.PaginaActual = $options.page
                            if ($options !== undefined && $options.sort !== undefined) {
                                data_param.Grilla.OrdenarDeForma = $options.sort[0].dir;
                                data_param.Grilla.OrdenarPor = $options.sort[0].field;
                            }
                            break;
                        case "create":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.Especialidad = $options.Especialidad.toUpperCase();
                            data_param.NivelAlcanzado = $options.IdNivel;
                            data_param.Institucion = $options.Institucion.toUpperCase();
                            data_param.IdNivel = $options.NivelAlcanzado.iCodNivel;

                            data_param.IdMes = $options.Obtencion_Estudio_Mes.Codigo;
                            data_param.IdAnio = $options.Obtencion_Estudio_Anio.Anio;

                            data_param.IdFechaInicioMes = $options.Inicio_Estudio_Mes.Codigo;
                            data_param.IdFechaInicioAnio = $options.Inicio_Estudio_Anio.Anio;

                            data_param.IdFechaFinMes = $options.Fin_Estudio_Mes.Codigo;
                            data_param.IdFechaFinAnio = $options.Fin_Estudio_Anio.Anio;

                            data_param.Ciudad = $options.Ciudad.toUpperCase();
                            break;
                        case "update":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdEstudio = $options.IdEstudio;
                            data_param.Especialidad = $options.Especialidad.toUpperCase();
                            data_param.NivelAlcanzado = $options.IdNivel;
                            data_param.Institucion = $options.Institucion.toUpperCase();
                            data_param.IdNivel = +$options.NivelAlcanzado.iCodNivel;

                            data_param.IdMes = +$options.Obtencion_Estudio_Mes.Codigo;
                            data_param.IdAnio = +$options.Obtencion_Estudio_Anio.Anio;

                            data_param.IdFechaInicioMes = +$options.Inicio_Estudio_Mes.Codigo;
                            data_param.IdFechaInicioAnio = +$options.Inicio_Estudio_Anio.Anio;

                            data_param.IdFechaFinMes = +$options.Fin_Estudio_Mes.Codigo;
                            data_param.IdFechaFinAnio = +$options.Fin_Estudio_Anio.Anio;

                            data_param.Ciudad = $options.Ciudad.toUpperCase();
                            break;
                        case "destroy":
                            data_param.IdEmpleado = $("#hdIdEmpleado").val();
                            data_param.IdEstudio = $options.IdEstudio;
                            break;
                    }
                    return $.toDictionary(data_param);
                }
            },
            requestEnd: function (e) {
                console.log(e.response);
                var grilla = $('#divGridEstudios').data("kendoGrid");
                console.log("Datos después de la operación:", grilla.dataSource.data());

                if (e.response.success != null) {
                    if (e.response.success == 'False') {
                        controladorApp.notificarMensajeDeAlerta(e.response.responseText);
                    }
                    else if(e.type != "read") {
                        controladorApp.notificarMensajeSatisfactorio("Se registró con éxito");
                    }
                }
                if (e.type === "create" || e.type === "update" || e.type === "destroy") {
                    grilla.dataSource.page(1); // Asegurarse de que los datos se recargan desde la primera página
                    grilla.dataSource.read();
                    $('#divGridEstudios').data('kendoGrid').refresh();
                }
            },
            schema: {
                data: "data",
                total: function (response) {
                    var TotalDeRegistros = 0;
                    if (response.length > 0) TotalDeRegistros = response[0].Grilla.TotalDeRegistros;
                    return TotalDeRegistros;
                },
                model: {
                    id: "IdEstudio",
                    fields: {
                        IdEmpleado: { editable: false, nullable: true },
                        IdEstudio: { editable: false, nullable: false },
                        NivelAlcanzado: {
                            validation: {
                                required: true,
                                nivelalcanzadovalidation: function (input) {
                                    if (input.is("[name='NivelAlcanzado']") && input.val() == "") {
                                        input.attr("data-nivelalcanzadovalidation-msg", "nivel es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                iCodNivel: 0,
                                strDescripcion: "strDescripcion"
                            }
                        },
                        Obtencion_Estudio_Mes: {
                            validation: {
                                required: true,
                                obtencion_estudio_mesvalidation: function (input) {
                                    if (input.is("[name='Obtencion_Estudio_Mes']") && input.val() == "") {
                                        input.attr("data-obtencion_estudio_mes-msg", "mes obtención de estudio es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                Codigo: "0",
                                Nombre: "Nombre"
                            }
                        },
                        Obtencion_Estudio_Anio: {
                            validation: {
                                required: true,
                                obtencion_estudio_aniovalidation: function (input) {
                                    if (input.is("[name='Obtencion_Estudio_Anio']") && input.val() == "") {
                                        input.attr("data-obtencion_estudio_anio-msg", "año de obtención de estudio es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                Anio: "Anio",
                            }
                        },
                        Inicio_Estudio_Mes: {
                            validation: {
                                required: true,
                                inicio_estudio_mesvalidation: function (input) {
                                    if (input.is("[name='Inicio_Estudio_Mes']") && input.val() == "") {
                                        input.attr("data-inicio_estudio_mes-msg", "mes obtención de estudio es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                Codigo: "0",
                                Nombre: "Nombre"
                            }
                        },
                        Inicio_Estudio_Anio: {
                            validation: {
                                required: true,
                                inicio_estudio_aniovalidation: function (input) {
                                    if (input.is("[name='Inicio_Estudio_Anio']") && input.val() == "") {
                                        input.attr("data-inicio_estudio_anio-msg", "año de obtención de estudio es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                Anio: "Anio",
                            }
                        },
                        Fin_Estudio_Mes: {
                            validation: {
                                required: true,
                                fin_estudio_mesvalidation: function (input) {
                                    if (input.is("[name='Fin_Estudio_Mes']") && input.val() == "") {
                                        input.attr("data-fin_estudio_mes-msg", "mes obtención de estudio es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                Codigo: "0",
                                Nombre: "Nombre"
                            }
                        },
                        Fin_Estudio_Anio: {
                            validation: {
                                required: true,
                                fin_estudio_aniovalidation: function (input) {
                                    if (input.is("[name='Fin_Estudio_Anio']") && input.val() == "") {
                                        input.attr("data-fin_estudio_anio-msg", "año de obtención de estudio es requerida");
                                        return false;
                                    }
                                    return true;
                                }
                            }, defaultValue: {
                                Anio: "Anio",
                            }
                        },
                        Institucion: { validation: { required: true, style: "text-transform: uppercase", nullable: false } }, //, class: "text-uppercase"
                        Especialidad: { validation: { required: true, style: "text-transform: uppercase", nullable: false } },
                        Ciudad: { validation: { required: true, style: "text-transform: uppercase", nullable: false } },
                    }
                }
            },
        });
        this.divGridEstudios = $("#divGridEstudios").kendoGrid({
            toolbar: ["create"], //"excel", 
            //excel: {
            //    fileName: "Listado de encargaturas del trabajador.xlsx",
            //    filterable: false
            //},
            dataSource: this.$dataSourceEstudios,
            autoBind: true,
            scrollable: false,
            sortable: false,
            pageable: false,
            groupable: false,
            dataType: 'json',
            columnMenu: {
                autoSize: true,
            },
            dataBound: function (e) {
                $('.k-grid-add').unbind("click");
                $('.k-grid-add').bind("click", function () {
                    if ($("#hdIdEmpleado").val() == 0) {
                        controladorApp.notificarMensajeDeAlerta('Primero debe completar el registro del trabajador');
                        return false;
                    }
                });
            },
            columns: [
                {
                    field: "NivelAlcanzado",
                    title: "Nivel Alcanzado",
                    editor: controlador.NivelAlcanzadoDropDownEditor,
                    template: "#=NivelAlcanzado.strDescripcion#"
                },
                {
                    field: "Institucion",
                    title: "Institucion",
                    template: "#=Institucion#"
                },
                {
                    field: "Especialidad",
                    title: "Especialidad",
                    attributes: { style: "text-align:left;" },
                },
                {
                    title: "Obtención de Estudio",
                    columns: [
                        {
                            field: "Obtencion_Estudio_Mes",
                            title: "Mes",
                            editor: controlador.Meses,
                            template: "#=Obtencion_Estudio_Mes.Nombre#"
                        },
                        {
                            field: "Obtencion_Estudio_Anio",
                            title: "Año",
                            editor: controlador.Anios,
                            template: "#=Obtencion_Estudio_Anio.Anio#"
                        },
                    ]
                },
                {
                    title: "Fecha inicio de estudio",
                    columns: [
                        {
                            field: "Inicio_Estudio_Mes",
                            title: "Mes",
                            editor: controlador.Meses,
                            template: "#=Inicio_Estudio_Mes.Nombre#"
                        },
                        {
                            field: "Inicio_Estudio_Anio",
                            title: "Año",
                            editor: controlador.Anios,
                            template: "#=Inicio_Estudio_Anio.Anio#"
                        },
                    ]
                },
                {
                    title: "Fecha fin de estudio",
                    columns: [
                        {
                            field: "Fin_Estudio_Mes",
                            title: "Mes",
                            editor: controlador.Meses,
                            template: "#=Fin_Estudio_Mes.Nombre#"
                        },
                        {
                            field: "Fin_Estudio_Anio",
                            title: "Año",
                            editor: controlador.Anios,
                            template: "#=Fin_Estudio_Anio.Anio#"
                        },
                    ]
                },
                {
                    field: "Ciudad",
                    title: "Ciudad/País",
                    attributes: { style: "text-align:right;" },
                },
                { title: "Acciones", command: ["edit", "destroy"] },
            ],
            editable: "inline",
            cancel: function (e) {
                $("#divGridEncargaturas").data("kendoGrid").refresh();
            }
        }).data();
    };

}(jQuery));