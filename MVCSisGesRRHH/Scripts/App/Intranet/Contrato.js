(function ($) {
    ...
    this.ContratoJS = function () { };
    this.ContratoJS.prototype.inicializar = function () {
        ...
        $("#ddlTipoLimite").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
            dataValueField: "Codigo",
            dataSource: data,
            change: function (e) {
                var tipo = $("#ddlTipoLimite").val();
                debugger;
                if (tipo == 1) {
                    $("#divFin").show();
        ...
        $("#ddlTipoPostulante").kendoDropDownList({
            autoBind: true,
            ...
            dataSource: {
                ...
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarTipoPostulante",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
        ...
        $("#ddlPostulante").kendoDropDownList({
            autoBind: true,
            cascadeFrom: "ddlTipoPostulante",
            optionLabel: "--Seleccione--",
            dataTextField: "NombreCompletoProceso",
            dataValueField: "IdRegistro",
            //filter: "contains",
            //minLength: 3,
            dataSource: {
                serverFiltering: true,
                transport: {
                    ...
                    parameterMap: function ($options, $operation) {
                        var data_param = {};
                        if ($operation === "read") {
                            data_param.Tipo = $('#ddlTipoPostulante').data("kendoDropDownList").value();
                            data_param.Estado = 2;
                            data_param.Grilla = {};
            ...
            change: function (e) {
                ...
                if (estado != '') {
                    debugger;
                    $("#hdIdPostulante").val(estado.split("|")[0]);
                    $("#hdIdPostulacion").val(estado.split("|")[1]);
                    $("#hdIdConvocatoria").val(estado.split("|")[2]);
        ...
        $('#divModalFormato').kendoWindow({
            ...
            width: '50%',
            height: 'auto',
            title: 'Actualizar Documento del Contrato',
            visible: false,
    ...
    function LimpiarModalRegistroPersona() {
        ...
        $("#txtPersonaFechaInicioLabores").kendoDatePicker().data("kendoDatePicker").min(new Date());
    ...
    this.ContratoJS.prototype.inicializarGrid = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Contrato/ListarContratos',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    ...
                    if ($operation === "read") {
                        ...
                        data_param.NroContrato = $("#txtContrato_busqueda").val();
        ...
        this.$grid = $("#divGrid").kendoGrid({
            toolbar: ["excel", ], 
            excel: {
                fileName: "Listado de Contratos.xlsx",
                filterable: false
            },
            ...
            columns: [
                ...
                {
                    field: "FechaInicioCadena",
                    title: "INICIO",
                    attributes: { style: "text-align:center;" },
                    width: "50px"
                },
                {
                    field: "FechaCeseCadena",
                    title: "FIN",
                    attributes: { style: "text-align:center;" },
                    width: "50px"
                },
    ...
    this.ContratoJS.prototype.buscar = function (e) {
        e.preventDefault();
    ...
    function CargarFormularioPostulante() {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $("#txtPersonaRemuneracion").val(res.Remuneracion);
                $("#txtPersonaMeta").val(res.Meta);
                $("#txtPersonaAIRHSP").val(res.NroPlanilla);
                ...
                //$("#txtPersonaBancoCCI").val(res.CuentaBancoAfiliado);

                debugger;
                if ($('#ddlTipoPostulante').data("kendoDropDownList").value() == '1') {
                    $('#txtPersonaRemuneracion').prop('readonly', true);
                    $("#txtPersonaProceso").val("CAS N° " + res.NombreProceso);
                    $("#ddlPersonaCondicion").data("kendoDropDownList").value(1);
                    $("#ddlPersonaDependencia").data("kendoDropDownList").value(res.IdUnidadOrganica);
                }
                else {
                    $('#txtPersonaRemuneracion').removeAttr('readonly');
                }   
    ...
    this.ContratoJS.prototype.abrirModalRegistroPersona = function (id) {
        ...
        if (id == 0) {           
            ...
            $("#postulacion1").prop("checked", true);
            modal.title("Ingresar Contrato Laboral");
            modal.open();
        }
        else {
            $("#hdIdPostulante").val(id);
    ...
    this.ContratoJS.prototype.cerrarModalRegistroPersona = function () {
        var modal = $('#divModalRegistroPersona').data('kendoWindow');
        modal.close();
    }

    this.ContratoJS.prototype.CargarFormularioTrabajador = function (id) {
        var data_param = new FormData();
    ...
    this.ContratoJS.prototype.CargarFormularioCuentasBancarias = function (id) {
    ...
    this.ContratoJS.prototype.BancoDropDownEditor = function (container, options) {
    ...
    this.ContratoJS.prototype.EstadoDropDownEditor = function (container, options) {
        var data = [
    ...
    this.ContratoJS.prototype.CargarFormularioOrdenesServicio = function (id) {
    ...
    this.ContratoJS.prototype.NombreOrdenEditor = function (container, options) {
        $('<textarea required name="' + options.field + '" style="width: ' + (container.width() + 30) + 'px;height:' + (container.height() + 30) + 'px" />')
            .appendTo(container);
    };
    this.ContratoJS.prototype.FechaInicioEditor = function (container, options) {
    ...
    this.ContratoJS.prototype.FechaFinEditor = function (container, options) {
    ...
    this.ContratoJS.prototype.agregarContrato = function (e) {
        e.preventDefault();
        ...
        if (frmPersonaValidador.validate()) {
            ...
            data_param.append('Planilla', "");
            data_param.append('Meta', $("#txtPersonaMeta").val());
            data_param.append('IdTipoLimite', $("#ddlTipoLimite").data("kendoDropDownList").value());
            data_param.append('NombreUnidadOrganica', $("#ddlPersonaDependencia").data("kendoDropDownList").text());
            data_param.append('NombreProceso', $("#txtPersonaProceso").val());
            data_param.append('Remuneracion', $("#txtPersonaRemuneracion").val());
            
            if ($('#ddlTipoPostulante').data("kendoDropDownList").value() == '1')
                data_param.append('Estado', 0);
            else
            ...
            if (inicio == null) {
                controladorApp.notificarMensajeDeAlerta('La fecha de inicio de contrato no es válida');
                $("#txtPersonaFechaInicioLabores").focus();
                return;
            }
            if (inicio != null && fin != null) {
                if (Date(inicio) > Date(fin)) {
                    controladorApp.notificarMensajeDeAlerta('La fecha de fin de contrato debe ser mayor que la fecha de inicio');
            ...
            controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de ingresar el contrato?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Contrato/' + metodo,
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        ...
                        success: function (res) {
                            ...
                            if (res.success == 'False') {
                                ...
                                controladorApp.notificarMensajeSatisfactorio("Contrato registrado correctamente");

                                // REFRESCAR INFORMACION DEL TRABAJADOR
                                $("#hdIdContrato").val(res.responseText);
    ...
    this.ContratoJS.prototype.GenerarFormatoContrato = function () {
    ...
    this.ContratoJS.prototype.agregarContratoArchivo = function (e) {
        e.preventDefault();
    ...
    this.ContratoJS.prototype.actualizarTrabajadorContacto = function (e) {
        e.preventDefault();
    ...
    this.ContratoJS.prototype.cerrarModalFormato = function () {
        $('#divModalFormato').data('kendoWindow').close();
    }

    this.ContratoJS.prototype.ingresarDetalleEvaluacion = function (idCad, idEvaluacion) {
    ...
    this.ContratoJS.prototype.abrirModalFormato = function (IdContrato) {
        $("#fileFormato").data("kendoUpload").clearAllFiles();
        $('#hdIdContrato').val(IdContrato);
    ...
    this.ContratoJS.prototype.cerrarModalEliminacion = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        $('#hdnUid').val('');
        modal.close();
    }

    this.ContratoJS.prototype.eliminar = function () {
        var modal = $('#divModalEliminacion').data('kendoWindow');
        var grilla = $('#divGrid').data("kendoGrid");
