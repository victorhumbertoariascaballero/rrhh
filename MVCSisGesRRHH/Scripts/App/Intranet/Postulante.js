(function ($) {
    ...
    this.PostulanteJS.prototype.inicializar = function () {
        ...
        $("#ddlEstado_busqueda").kendoDropDownList({
            ...
            dataSource: {
                transport: {
                    read: {
                        url: controladorApp.obtenerRutaBase() + "Postulante/ListarEstados",
                        type: "GET",
                        dataType: "json",
                        cache: false
                    }
        ...
        $("#ddlEstado_busquedaDes").kendoDropDownList({
            autoBind: false,
            //optionLabel: "--Todos--",
            dataTextField: "Nombre",
            dataValueField: "Codigo",
        ...
        $('#divModalNotificacion').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
        ...
        $('#divModalEvaluacion').kendoWindow({
            draggable: true,
            modal: true,
            pinned: false,
            resizable: false,
            width: '80%',
            height: 'auto',
            title: 'Evaluación de documentos enviados por el postulante',
            visible: false,
            position: { top: '10%', left: "10%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"],
        ...
        $('#divModalGanador').kendoWindow({
            ...
            width: '80%',
            height: 'auto',
            title: 'Documentos enviados por el postulante ganador del proceso',
            visible: false,
            position: { top: '10%', left: "10%" },
            //actions: ["Minimize", "Maximize", "Close"],
            actions: ["Close"],
        ...
        this.inicializarGridGanadores();
        this.inicializarGrid();
        this.inicializarGridDesignados();
    };
    this.PostulanteJS.prototype.inicializarIngresar = function () {
        /* REGISTRO */
        ...
        $("#txtPersonaNacionalidad").val("PERUANO");
    ...
    this.PostulanteJS.prototype.inicializarEditar = function () {
        /* REGISTRO */
        $("#ddlPersonaTipoDeDocumento").kendoDropDownList({
            autoBind: true,
            dataTextField: "Nombre",
    ...
    this.PostulanteJS.prototype.inicializarGridGanadores = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/ListarGanadores',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    ...
                    if ($operation === "read") {
                        data_param.IdPostulante = 0;
                        data_param.NroDocumento = $("#txtDNI_busquedaG").val();
                        data_param.NombreProceso = $("#txtProceso_busquedaG").val().toUpperCase();
                        data_param.Nombre = $("#txtPostulante_busquedaG").val().toUpperCase();
                        data_param.Estado = 1; //$("#ddlEstado_busqueda").data("kendoDropDownList").value();
                        //data_param.Tipo = 1;
                        data_param.Grilla = {};
        ...
        this.$grid = $("#divGridG").kendoGrid({
            ...
            columns: [
                ...
                {
                    field: "NombreUnidadOrganica",
                    title: "UNIDAD ORGÁNICA",
                    width: "250px"
                },
                //{
                //    field: "FechaMaximaContrato",
                //    title: "FECHA MÁXIMA PARA FIRMAR",
                //    attributes: { style: "text-align:center;" },
                //    width: "80px"
                //},
                {
                    ...
                    template: function (item) {
                        var respuesta = '';
                        if (item.Estado == 1) respuesta = "REGISTRO";
                        if (item.Estado == 2) respuesta = "PENDIENTE DE FIRMA";
                        if (item.Estado == 3) respuesta = "CON CONTRATO FIRMADO";
                ...
                },
                {
                    ...
                    template: function (item) {
                        var controles = "";
                        //if (item.Estado == 1) {
                        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalGanadores(' + item.IdPostulante + ',' + item.IdPostulacion + ',' + item.IdConvocatoria + ')">';
                        controles += '<span class="glyphicon glyphicon-search" aria-hidden="true" data-uib-tooltip="Validar" title="Documentos presentados por el postulante"></span>';
                        controles += '</button>';
                        //}
    ...
    this.PostulanteJS.prototype.inicializarGrid = function () {
        this.$dataSource = [];
        this.$dataSource = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            ...
            transport: {
                read: {
                    url: controladorApp.obtenerRutaBase() + 'Postulante/Listar',
                    type: 'GET',
                    dataType: 'json',
                    cache: false
                },
                parameterMap: function ($options, $operation) {
                    ...
                    if ($operation === "read") {
                        ...
                        data_param.NombreProceso = $("#txtProceso_busqueda").val().toUpperCase();
                        data_param.Nombre = $("#txtPostulante_busqueda").val().toUpperCase();
            ...
            columns: [
                ...
                },
                {
                    ...
                    template: function (item) {
                        var controles = "";
                        //if (item.Estado == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalEvaluacion(' + item.IdPostulante + ',' + item.IdPostulacion + ',' + item.IdConvocatoria + ')">';
                            controles += '<span class="glyphicon glyphicon-search" aria-hidden="true" data-uib-tooltip="Validar" title="Documentos presentados por el postulante"></span>';
                            controles += '</button>';
                        //}
                ...
                },
                {
                    ...
                    template: function (item) {
                        var controles = "";
                        //if (item.IdPostulacion != 0 && item.IdConvocatoria != 0) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalNotificacion(' + item.IdPostulante + ',' + item.IdPostulacion + ',' + item.IdConvocatoria + ')">';
                            controles += '<span class="glyphicon glyphicon-menu-hamburger" aria-hidden="true" data-uib-tooltip="Validar" title="Bandeja de notificaciones al postulante"></span>';
                            controles += '</button>';
                        //}
                ...
                },
                {
                    ...
                    template: function (item) {
                        var controles = "";
                        if (item.Estado == 1) {
                            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.validarPostulante(' + item.IdPostulante + ',' + item.IdPostulacion + ',' + item.IdConvocatoria + ')">';
                            controles += '<span class="glyphicon glyphicon-ok" aria-hidden="true" data-uib-tooltip="Validar" title="Validar ficha del postulante"></span>';
                            controles += '</button>';
                        }
    ...
    this.PostulanteJS.prototype.inicializarGridDesignados = function () {
        this.$dataSourceDes = [];
        this.$dataSourceDes = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                ...
                parameterMap: function ($options, $operation) {
                    ...
                    if ($operation === "read") {
                        data_param.IdPostulante = 0;
                        data_param.NroDocumento = $("#txtDNI_busquedaDes").val();
                        data_param.NombreProceso = $("#txtProceso_busquedaDes").val().toUpperCase();
                        data_param.Nombre = $("#txtPostulante_busquedaDes").val().toUpperCase();
                        data_param.Estado = $("#ddlEstado_busquedaDes").data("kendoDropDownList").value();
                        data_param.Tipo = 2;
                        data_param.Grilla = {};
                        data_param.Grilla.RegistrosPorPagina = $options.pageSize;
                        data_param.Grilla.PaginaActual = $options.page
        ...
        this.$grid = $("#divGridDes").kendoGrid({
            toolbar: ["excel"], //{ template: kendo.template('<div>Esto es una prueba ' + this.$dataSource.total() + ' </div>') }
            excel: {
                fileName: "Listado postulantes por otra modalidad.xlsx",
                filterable: false
            },
            dataSource: this.$dataSourceDes,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
            ...
            columns: [
                ...
                //        if (item.Estado == 1) {
                //            controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalEvaluacion(' + item.IdPostulante + ',' + item.IdPostulacion + ',' + item.IdConvocatoria + ')">';
                //            controles += '<span class="glyphicon glyphicon-search" aria-hidden="true" data-uib-tooltip="Validar" title="Validar ficha del postulante"></span>';
                //            controles += '</button>';
                //        }
                ...
                //        //if (item.IdPostulacion != 0 && item.IdConvocatoria != 0) {
                //        controles += '<button type="button" class="btn btn-info btn-xs" onclick="controlador.abrirModalNotificacion(' + item.IdPostulante + ',' + item.IdPostulacion + ',' + item.IdConvocatoria + ')">';
                //        controles += '<span class="glyphicon glyphicon-menu-hamburger" aria-hidden="true" data-uib-tooltip="Validar" title="Validar ficha del postulante"></span>';
                //        controles += '</button>';
                //        //}
    ...
    this.PostulanteJS.prototype.inicializarGridNotificacion = function (IdPostulante, IdPostulacion, IdConvocatoria) {
        this.$dataSourceNotificacion = [];
        this.$dataSourceNotificacion = new kendo.data.DataSource({
            serverPaging: true,
            serverSorting: true,
            batch: false,
            transport: {
                ...
                parameterMap: function ($options, $operation) {
                    ...
                    if ($operation === "read") {
                        ...
                        data_param.IdPostulacion = IdPostulacion;
        ...
        this.$gridNotificacion = $("#divGridNotificacion").kendoGrid({
            toolbar: ["excel"], //{ template: kendo.template('<div>Esto es una prueba ' + this.$dataSource.total() + ' </div>') }
            excel: {
                fileName: "Listado de Notificaciones.xlsx",
                filterable: false
            },
            dataSource: this.$dataSourceNotificacion,
            autoBind: true,
            selectable: true,
            scrollable: false,
            sortable: false,
    ...
    this.PostulanteJS.prototype.buscar = function (e) {
        e.preventDefault();
    ...
    this.PostulanteJS.prototype.buscarGanador = function (e) {
        e.preventDefault();

        var grilla = $('#divGridG').data("kendoGrid");
        grilla.dataSource._sort = undefined;
        grilla.dataSource.page(1);

        $("#lblTotalG").html(grilla.dataSource.total());
    };

    this.PostulanteJS.prototype.buscarDes = function (e) {
        e.preventDefault();

        var grilla = $('#divGridDes').data("kendoGrid");
        grilla.dataSource._sort = undefined;
        grilla.dataSource.page(1);

        $("#lblTotalDes").html(grilla.dataSource.total());
    };
    ...
    this.PostulanteJS.prototype.CargarFormularioPostulante = function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                //$("#txtPersonaNombreAfp").val(res.NombreAFPAfiliada);
    ...
    this.PostulanteJS.prototype.NombreOrdenEditor = function (container, options) {
    ...
    this.PostulanteJS.prototype.registrarPostulante = function (e) {
        ...
        if (frmPersonaValidador.validate()) {
            ...
            if ($("#tienepension1").prop("checked")) data_param.append('IdEstaAfiliadoPensiones', 1);
            else data_param.append('IdEstaAfiliadoPensiones', 0);
    ...
    this.PostulanteJS.prototype.cerrarModalEdicion = function () {
        $('#divModalNuevo').data('kendoWindow').close();
    }
    this.PostulanteJS.prototype.abrirRegistroPostulante = function () {
        //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
        window.open(controladorApp.obtenerRutaBase() + 'Postulante/Ingresar', '_blank');
    }
    ...
    this.PostulanteJS.prototype.cerrarModalPostulante = function () {
        //window.location = controladorApp.obtenerRutaBase() + 'PropuestaDetalle/?IdCad=' + idCad + "&IdPropuesta=" + idPropuesta + "&IdVersion=" + idVersion;
        window.location = controladorApp.obtenerRutaBase() + 'Postulante/';
    }
    ...
    this.PostulanteJS.prototype.editarFormatoFicha = function (IdPostulante, IdPostulacion, IdConvocatoria) {
        window.open(controladorApp.obtenerRutaBase() + "Postulante/Editar?IdPostulante=" + IdPostulante + "&IdPostulacion=" + IdPostulacion + "&IdConvocatoria=" + IdConvocatoria, "_blank");
    }
    this.PostulanteJS.prototype.validarPostulante = function (IdPostulante, IdPostulacion, IdConvocatoria) {
        var data_param = new FormData();
        ...
        data_param.append('Estado', 2);

        controladorApp.abrirMensajeDeConfirmacion(
                '¿Está seguro de validar la información del postulante?', 'SI', 'NO'
                , function (arg) {
                    $.ajax({
                        url: controladorApp.obtenerRutaBase() + 'Postulante/Validar',
                        type: 'POST',
                        dataType: 'json',
                        contentType: false,
                        processData: false,
                        ...
                        success: function (res) {
                            ...
                            if (res.success == 'False') {
                                ...
                                controladorApp.notificarMensajeSatisfactorio("Postulante validado correctamente");
                                $('#divGrid').data("kendoGrid").dataSource.page(1);
                                $('#divGridDes').data("kendoGrid").dataSource.page(1);
                            }
                        },
                        error: function (res) {
                            //alert(res);
    ...
    this.PostulanteJS.prototype.abrirModalNotificacion = function (IdPostulante, IdPostulacion, IdConvocatoria) {
        $('#hdIdPostulante').val(IdPostulante);
        $('#hdIdPostulacion').val(IdPostulacion);
        $('#hdIdConvocatoria').val(IdConvocatoria);

        var modal = $('#divModalNotificacion').data('kendoWindow');
        modal.title("Bandeja de Notificaciones");

        this.inicializarGridNotificacion(IdPostulante, IdPostulacion, IdConvocatoria);

        modal.open();
    }
    this.PostulanteJS.prototype.abrirModalEvaluacion = function (IdPostulante, IdPostulacion, IdConvocatoria) {
        $('#hdIdPostulante').val(IdPostulante);
        $('#hdIdPostulacion').val(IdPostulacion);
        $('#hdIdConvocatoria').val(IdConvocatoria);

        var modal = $('#divModalEvaluacion').data('kendoWindow');
        //modal.title("Bandeja de Notificaciones");
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.IdTieneHojaVida == 1) {
                    ...
                    $("#btnActualFoto").attr("href", newUrl);
                    $("#btnActualFoto").show();
                }
                else
                    $("#btnActualFoto").hide();
                ...
                if (res.IdTieneDDJJ == 1) {
                    ...
                    newUrl = controladorApp.obtenerRutaBase() + 'Postulante/FichaPersonal/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria
                    $("#btnActualDDJJOrig").attr("href", newUrl);
                    $("#btnActualDDJJOrig").show();
                }
                else {
                    $("#btnActualDDJJ").hide();
                    $("#btnActualDDJJOrig").hide();
                }   
                ...
                newUrl = controladorApp.obtenerRutaBase() + 'Postulante/HojaVida/?idPostulante=' + res.IdPostulante + '&idPostulacion=' + res.IdPostulacion + '&idConvocatoria=' + res.IdConvocatoria 
                $("#btnActualHojaVidaOrig").attr("href", newUrl);
                $("#btnActualHojaVidaOrig").show();
        ...
        this.$gridDocumento = $(".divGridDocumento").kendoGrid({
            dataSource: {
        ...
        this.$gridEstudio = $(".divGridEstudio").kendoGrid({
            dataSource: {
                //filter: { field: "EmployeeID", operator: "eq", value: e.data.EmployeeID }
                serverPaging: false,
                serverSorting: true,
        ...
        this.$gridCapacitacion = $(".divGridCapacitacion").kendoGrid({
            dataSource: {
                //filter: { field: "EmployeeID", operator: "eq", value: e.data.EmployeeID }
                serverPaging: false,
                serverSorting: true,
        ...
        this.$gridExperiencia = $(".divGridExperiencia").kendoGrid({
            dataSource: {
                serverPaging: false,
                serverSorting: true,
                batch: false,
    ...
    this.PostulanteJS.prototype.abrirModalGanadores = function (IdPostulante, IdPostulacion, IdConvocatoria) {
        $('#hdIdPostulante').val(IdPostulante);
        $('#hdIdPostulacion').val(IdPostulacion);
        $('#hdIdConvocatoria').val(IdConvocatoria);

        var modal = $('#divModalGanador').data('kendoWindow');
        modal.open();
        //modal.title("Bandeja de Notificaciones");
        ...
        this.$gridDocumento = $(".divGridDocumentoG").kendoGrid({
            dataSource: {
                //filter: { field: "EmployeeID", operator: "eq", value: e.data.EmployeeID }
                serverPaging: false,
                serverSorting: true,
        ...
        this.$gridEstudio = $(".divGridEstudioG").kendoGrid({
            dataSource: {
                //filter: { field: "EmployeeID", operator: "eq", value: e.data.EmployeeID }
                serverPaging: false,
                serverSorting: true,
        ...
        this.$gridCapacitacion = $(".divGridCapacitacionG").kendoGrid({
            dataSource: {
                //filter: { field: "EmployeeID", operator: "eq", value: e.data.EmployeeID }
                serverPaging: false,
                serverSorting: true,
        ...
        this.$gridExperiencia = $(".divGridExperienciaG").kendoGrid({
            dataSource: {
                serverPaging: false,
                serverSorting: true,
                batch: false,
    ...
    this.PostulanteJS.prototype.guardarNotificacion = function () {
        var IdPostulante = $('#hdIdPostulante').val();
        var IdPostulacion = $('#hdIdPostulacion').val(); 
        var IdConvocatoria = $('#hdIdConvocatoria').val();
        ...
        data_param.append('IdPostulacion', IdPostulacion);
        data_param.append('IdConvocatoria', IdConvocatoria);
        data_param.append('Descripcion', $('#txtDesNotificacion').val());
        data_param.append('IdOrigen', 2);

        $.ajax({
            url: controladorApp.obtenerRutaBase() + 'Postulante/RegistrarNotificacion',
            type: 'POST',
            dataType: 'json',
            contentType: false,
            processData: false,
            data: data_param,
            success: function (res) {
                controladorApp.notificarMensajeSatisfactorio("Notificacion enviada correctamente");
                $('#txtDesNotificacion').val('');
                controlador.inicializarGridNotificacion(IdPostulante, IdPostulacion, IdConvocatoria);
            },
            error: function (res) {
    ...
    this.PostulanteJS.prototype.MostarRangoFechas = function (suma) {
    ...
    this.PostulanteJS.prototype.abrirModalVisor = function (idPostulacion, idDetalle, idTipo) {
