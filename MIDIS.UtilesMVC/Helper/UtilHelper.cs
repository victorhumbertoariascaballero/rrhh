...
namespace MIDIS.UtilesMVC.Helper
{
    public static class UtilHtmlHelper
    {
        ...
        public static MvcHtmlString HelperMenu(this HtmlHelper helper, List<T_genm_opcion> listaOpcion,string nombreLista)
        {
            ...
            foreach (T_genm_opcion item in (from x in listaOpcion
                                            where x.Id_grupo_opcion == 0
                                            select x).ToList())
            {
                var itemLista = new TagBuilder("li");
                var link = new TagBuilder("a") { InnerHtml = item.Nombre_opcion };
                UrlHelper urlHelper = new UrlHelper(helper.ViewContext.RequestContext);
                if (!string.IsNullOrEmpty(item.Url) && item.Url != "#")
                {
                    string[] ruta = item.Url.Split('/');
                    switch (ruta.Count())
                    {
                        case 1:
        ...
        private static void LlenarOpcionMenu(TagBuilder itemLista, T_genm_opcion itemOpcion,List<T_genm_opcion> listaOpcion, UrlHelper urlHelper)
        {
            if ((from x in listaOpcion
                 where x.Id_grupo_opcion == itemOpcion.Id_opcion
                 select x).Count() > 0)
            {
